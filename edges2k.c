...
void plotfspec(int, double *, double *, double *, double *, double, int, int, char *,
               char *, char *);
void outfspec(int, double *, double *, double *, double *, double, int, int, char *);
void outcal(int, double *, complex double *, double *, double *, double *, double *,
            double *, double *, char *);
void outsim(int, double *, double *, int, int, int, int, int);
void readcal(int *, double *, complex double *, double *, double *, double *, double *,
             double *, double *);
...
double polyfitr(int, int, double *, double *, double *, double *, double *, long double *,
                double *);
double polyfitrc(int, int, double *, double *, double *, double *, double *,
                 long double *, double *, int);
double polyfitr2(int, int, double *, double *, double *, double *, double *,
...
double w3p(complex double, complex double, double, double, double, double, double,
           double);
double w3pinv(complex double, complex double, double, double, double, double, double,
              double);
void fittp(int, double *, complex double *, double *, int, double *, complex double *,
           int, double *, double *, long double *, double *, int, char *);
void plotvna(int, double *, complex double *, double *, int, double *, complex double *,
             double *, double *, int, double, double, int, char *);
void wavefit(int, int, int, double *, double *, double *, complex double *,
             complex double *, double, double, double, double *, double *, double *,
...
complex double cabl2(double, double, complex double, int, complex double *,
                     complex double *, double);
double antloss(double, double *, double *, long double *, double *, double, int);
double lossinv(double, double, double);
double beamcorr(double, int, double, double, double *, double *, long double *, double *,
                int, int, double *, double *, double *, int, double *, double, double,
                int, int);
double beamcorr2(double, int, double, double, double *, double *, long double *, double *,
                 int, int, double *, double *, double *, int, double *, double, double,
                 int);
double gst(double);
...
int main(int argc, char *argv[]) {
  ...
  static double freqant[NDATA], freqopen[NDATA], freqshort[NDATA], freqhot[NDATA],
      freqamb[NDATA], freqs11ant[NDATA];
  static double freqs11lna[NDATA], freqs11hot[NDATA], freqs11amb[NDATA],
      freqs11cab1[NDATA], freqs11cab2[NDATA], freqs11rig[NDATA];
  static double wtant[NDATA], wtopen[NDATA], wtshort[NDATA], wthot[NDATA], wtamb[NDATA],
      wttant[NDATA], wttlna[NDATA], wtthot[NDATA], wttamb[NDATA], wttcab1[NDATA],
      wttcab2[NDATA], wttrig[NDATA];
  static double spant[NDATA], spopen[NDATA], spshort[NDATA], sphot[NDATA], spamb[NDATA],
      data[NDATA], dataout[NDATA], dataout2[NDATA], fitf[NDATA * NFIT], md[NDATA],
      ltemp[NDATA];
  ...
  static complex double s11ant[NDATA], s11lna[NDATA], s11hot[NDATA], s11amb[NDATA],
      s11cab1[NDATA], s11cab2[NDATA], s11rig[NDATA], s12rig[NDATA], s22rig[NDATA], T;
  static complex double ss11ant[NDATA], ss11lna[NDATA], ss11hot[NDATA], ss11amb[NDATA],
      ss11cab[NDATA], ss11rig[NDATA], ss12rig[NDATA], ss22rig[NDATA], tmp[NDATA],
      tmpsm[NDATA];
  static double bb[NBEAM], bb2[NBEAM];

  double freq, wt, re, re1, re2, re3, re4, re5, re6, re7, re8, re9, re10, re11, re12,
      re13, re14, im, im1, im2, im3, im4, im5, im6, im7, im8, im9, im10, im11, im12, im13,
      im14, bspac;
  double tt, La, Lh, Lhh, rms, rms1, rms2, rms3, fstart, fstop, wfstart, wfstop, fcen,
      eorcen, eorwid, eoramp, atten, dlyrob, days, adb, ldb, gha, dgha, specin, freqr,
      antaz, dscale;
  double spe_ind, spe_inderr, gamma, gammaerr, ion_abs, ion_em, ssun, sunind, ghav,
      ghamax, ghamin, ion, tau, noise, opn, fbstart, fbstop, lr, li, ar, ai;
  double thot, tamb, tcold, tant, tload, tcab, s, sca, s1, s2, ofs, err1, lst, secs,
      aloss, delaylna, delayln, delaycorr, dbcorr, t150, delayant, tant2, freqref;
  int i, j, k, kk, nant, nopen, nshort, nhot, namb, mode, ns11ant, ns11lna, ns11hot,
      ns11amb, ns11cab1, ns11cab2, ns11rig, nfit, nfit1, imp, db, nfit2, nfit3, nfit4,
      iter, mfit, cfit, wfit, skymode, yr, dy, hr, mn, sc, smooth, cal, wtmode, lmode,
      cmb, nline, site, map;
  int ncal, cons, binteg, nter, nocal, bfit, bfit2, mdd, skymod2, test, i1, i2, i3, i4,
      i5, i6, i7, yrh, dyh, hrh, mnh, sch, yrc, dyc, hrc, mnc, scc, yro, dyo, hro, mno,
      sco, yrs, dys, hrs, mns, scs, low, sim, rr;
  static double tcal_sca[NDATA], tcal_ofs[NDATA], tcal_scasm[NDATA], tcal_ofssm[NDATA],
      wtcal[NDATA];
  FILE *file1;
  char fname[256], buf[2048], title[256], hottitle[256], ambtitle[256], opentitle[256],
      shorttitle[256], datfile[256], ambfile[256], hotfile[256], openfile[256],
      shortfile[256], info[256];
  char antfname[256], hotfname[256], ambfname[256], openfname[256], shortfname[256],
      rigfname[256], lnafname[256];
  mode = 0;
  nant = nopen = nshort = nhot = namb = ns11ant = ns11lna = ns11hot = ns11amb = ns11cab1 =
      ns11cab2 = ns11rig = 0;
  wtmode = 0;
  cons = 0;
  nocal = 0;
  ...
  tload = 300.0;  // internal comparison load - value doesn't matter as long as it have
                  // same value as used in calibration
  cal = 0;
  tcab = 0;
  atten = 0;
  Lhh = 1;
  ...
  dlyrob = 0.5 / 3e8;
  smooth = 1;
  Lh = 0.05;  // estimated loss of 6 inches 0.141 stainless steel dB at 100 MHz   thot and
              // tcold have to be correct or both scaled 1K error ~ 10mK
  ...
  for (i = 0; i < argc - 1; i++) {
    ...
    if (strstr(buf, "-hotsparms"))
      mode = 31;  // from /data5/edges/data/CalHotLoadCableData/ but has to be fit if freq
                  // spacing different
    if (strstr(buf, "-cals11_sm33"))
      mode = 33;  // more recent calibrations just using simulator 3
    ...
    if (strstr(buf, "-skymod2")) {
      sscanf(argv[i + 1], "%d", &skymod2);
    }  // use for new slower beamcorrection using new Haslam map
    if (strstr(buf, "-lmode")) {
      sscanf(argv[i + 1], "%d", &lmode);
    }  // -1 = to use aloss 0=for hot load 2=hot+rigloss 1= balun + bean using
       // interpolated antloss 5or6=lowband balun +conn
    if (strstr(buf, "-wtmode")) {
      sscanf(argv[i + 1], "%d", &wtmode);
    }  // sets weighting for fit and > 10 for  calibration 1 = normal zero wt outside
       // wfstart & wfstop
    if (strstr(buf, "-nfit1")) {
    ...
    }  // temperature of cable used for noise wave calibration get set to tcold unless
       // entered
    if (strstr(buf, "-tant")) {
      sscanf(argv[i + 1], "%lf", &tant);
    }  // temperature of antenna used in loss calculation needed even when specal.txt is
       // used
    ...
    }  // loss of hot load at 100 MHz uses tamb for loss calc Lh = -1 for hot load model
       // Lh = -2 to use s12,s22
    ...
    if (mode) {
      ...
      while (fgets(buf, 2048, file1) != 0) {
        if (mode >= 1 && mode < 10) {
          ...
          if (mode == 1 && j == 0) {
            sscanf(buf, "%*s %*s %*s %d %*s %s %d:%d:%d:%d:%d %lf %lf %lf %lf %lf",
                   &nline, datfile, &yr, &dy, &hr, &mn, &sc, &gha, &dgha, &ghav, &ghamax,
                   &ghamin);
          }
          if (mode == 2 && j == 0) {
            sscanf(buf, "%*s %*s %*s %*s %*s %s %d:%d:%d:%d:%d %lf %lf %lf %lf %lf",
                   openfile, &yro, &dyo, &hro, &mno, &sco, &gha, &dgha, &ghav, &ghamax,
                   &ghamin);
          }
          if (mode == 5 && j == 0) {
            sscanf(buf, "%*s %*s %*s %*s %*s %s %d:%d:%d:%d:%d %lf %lf %lf %lf %lf",
                   shortfile, &yrs, &dys, &hrs, &mns, &scs, &gha, &dgha, &ghav, &ghamax,
                   &ghamin);
          }
          if (mode == 3 && j == 0) {
            sscanf(buf, "%*s %*s %*s %*s %*s %s %d:%d:%d:%d:%d %lf %lf %lf %lf %lf",
                   hotfile, &yrh, &dyh, &hrh, &mnh, &sch, &gha, &dgha, &ghav, &ghamax,
                   &ghamin);
          }
          if (mode == 4 && j == 0) {
            sscanf(buf, "%*s %*s %*s %*s %*s %s %d:%d:%d:%d:%d %lf %lf %lf %lf %lf",
                   ambfile, &yrc, &dyc, &hrc, &mnc, &scc, &gha, &dgha, &ghav, &ghamax,
                   &ghamin);
          }
        ...
        if (mode >= 17 && mode < 40) {  // Raul's format for all
          ...
          if (buf[0] != '#') {
            sscanf(buf,
                   ...
                   &freq, &re, &im, &re1, &im1, &re2, &im2, &re3, &im3, &re4, &im4, &re5,
                   &im5, &re6, &im6, &re7, &im7, &re8, &im8, &re9, &im9, &re10, &im10,
      ...
      fclose(file1);
      //    printf("mode %d j %d nant %d nopen %d nhot %d namb %d ns11ant %d ns11lna %d
      //    ns11hot %d ns11amb %d ns11cab1 %d ns11cab2 %d\n",
      //          mode,j,nant,nopen,nhot,namb,ns11ant,ns11lna,ns11hot,ns11amb,ns11cab1,ns11cab2);
    }
    mode = 0;
  }
  if (tcab == 0.0)
    tcab = tcold;  // sets to tcold if not entered
  ...
  for (j = 0; j < ns11lna; j++)
    s11lna[j] = s11lna[j] *
                    cexp(-2.0 * PI * freqs11lna[j] * 1e6 * ((delaylna + delaycorr) * I)) *
                    pow(10.0, 0.05 * (dbcorr + ldb)) +
                lr + li * I;  // corrections for adapters
  for (j = 0; j < ns11ant; j++) {
    if (opn)
      s11ant[j] = opn;
    else
      s11ant[j] = s11ant[j] *
                      cexp(-2.0 * PI * freqs11ant[j] * 1e6 * (delaycorr + delayant) * I) *
                      pow(10.0, 0.05 * (dbcorr + adb)) +
                  ar + ai * I;
  }
  for (j = 0; j < ns11hot; j++)
    s11hot[j] = s11hot[j] * cexp(-2.0 * PI * freqs11hot[j] * 1e6 * (delaycorr)*I) *
                pow(10.0, 0.05 * dbcorr);
  for (j = 0; j < ns11amb; j++)
    s11amb[j] = s11amb[j] * cexp(-2.0 * PI * freqs11amb[j] * 1e6 * (delaycorr)*I) *
                pow(10.0, 0.05 * dbcorr);
  for (j = 0; j < ns11cab1; j++)
    s11cab1[j] = s11cab1[j] * cexp(-2.0 * PI * freqs11cab1[j] * 1e6 * (delaycorr)*I) *
                 pow(10.0, 0.05 * dbcorr);
  for (j = 0; j < ns11cab2; j++)
    s11cab2[j] = s11cab2[j] * cexp(-2.0 * PI * freqs11cab2[j] * 1e6 * (delaycorr)*I) *
                 pow(10.0, 0.05 * dbcorr);
  if (yr)
  ...
  if (skymode >= 0)
    bfit =
        beamcorr(0, bfit, antaz, secs, mcalc, fitf, aarr, bbrr, skymode, binteg, &lst, bb,
                 &bspac, cmb, &freqref, fbstart, fbstop, site, map);  // fit polynomial
  if (skymod2 >= 0)
    bfit = beamcorr2(0, bfit, antaz, secs, mcalc, fitf, aarr, bbrr, skymod2, binteg, &lst,
                     bb, &bspac, cmb, &freqref, fbstart, fbstop, site);  // fit polynomial
  if (skymod2 <= -128)
    bfit2 = beamcorr(0, bfit, antaz, secs, mcalc, fitf, aarr, bbrr, -skymod2, binteg,
                     &lst, bb2, &bspac, cmb, &freqref, fbstart, fbstop, site,
                     map);  // fit polynomial
  if (skymode >= 0)
  ...
  if (nant && (nopen || nshort) && nhot && namb && ns11ant && ns11lna && ns11hot &&
      ns11amb && ns11cab1 && !nocal) {  // continue as all data is present
    //      L = pow(10.0,-0.1*atten);  // L not used if atten needed for cable use R
    //      factor method of memo 98

    // fit s11 with series

    printf(
        "nopen %d nshort %d nhot %d namb %d ns11hot %d ns11amb %d ns11cab1 %d ns11cab2 "
        "%d freq %f "
        "%f %f %f\n",
        nopen, nshort, nhot, namb, ns11hot, ns11amb, ns11cab1, ns11cab2, freqant[0],
        freqant[nant - 1], freqs11ant[0], freqs11ant[ns11ant - 1]);

    // 27 or higher for antenna
    printf("nant %d ns11ant %d ns11lna %d\n", nant, ns11ant,
           ns11lna);  // period under 100 MHz
    fittp(ns11lna, freqs11lna, s11lna, wttlna, namb, freqamb, ss11lna, nfit3, fitf, mcalc,
          aarr, bbrr, 1, lnafname);
    fittp(ns11hot, freqs11hot, s11hot, wtthot, namb, freqhot, ss11hot, nfit2, fitf, mcalc,
          aarr, bbrr, 2, hotfname);
    fittp(ns11amb, freqs11amb, s11amb, wttamb, namb, freqamb, ss11amb, nfit2, fitf, mcalc,
          aarr, bbrr, 3, ambfname);
    if (nopen)
      fittp(ns11cab1, freqs11cab1, s11cab1, wttcab1, namb, freqamb, ss11cab, nfit2, fitf,
            mcalc, aarr, bbrr, 4, openfname);
    if (nshort)
      fittp(ns11cab2, freqs11cab2, s11cab2, wttcab2, namb, freqamb, &ss11cab[namb], nfit2,
            fitf, mcalc, aarr, bbrr, 5, shortfname);
    if (ns11rig) {
      fittp(ns11rig, freqs11rig, s11rig, wttrig, namb, freqamb, ss11rig, nfit2, fitf,
            mcalc, aarr, bbrr, 7, rigfname);
      fittp(ns11rig, freqs11rig, s12rig, wttrig, namb, freqamb, ss12rig, nfit2, fitf,
            mcalc, aarr, bbrr, 8, rigfname);
      fittp(ns11rig, freqs11rig, s22rig, wttrig, namb, freqamb, ss22rig, nfit2, fitf,
            mcalc, aarr, bbrr, 9, rigfname);
    }
    if (!nopen) {
      for (i = 0; i < namb; i++) {
    ...
    for (iter = 0; iter < nter; iter++) {  // needs 4 for accurate convergence
      for (i = 0; i < namb; i++) {
        ...
        // invert with scale error and offset and changed S11 (i.e. with changes it will
        // analyzed with wrong S11)
        s1 = w3pinv(ss11amb[i], ss11lna[i], sspamb[i], tload, tamb, tlna0[i], tlna1[i],
                    tlna2[i]);
        s = w3pinv(ss11hot[i], ss11lna[i], ssphot[i], tload, tamb, tlna0[i], tlna1[i],
                   tlna2[i]);
        if (Lh >= 0) Lhh = loss(ss11hot[i], Lh * sqrt(freqhot[i] / 100.0));
        if (Lh == -1)
          Lhh = lossmodel(freqhot[i], ss11hot[i], 0);  // mode = 0 for cable + adapter
        if (Lh == -2)
          Lhh = rigloss(ss11hot[i], ss11rig[i], ss12rig[i],
        ...
        if (nopen)
          sspopen[nopen + i] = (spshort[i] - tamb) * tcal_sca[i] + tamb - tcal_ofs[i];
        ssphot[i] = (sphot[i] - tamb) * tcal_sca[i] + tamb - tcal_ofs[i];
        sspamb[i] = (spamb[i] - tamb) * tcal_sca[i] + tamb - tcal_ofs[i];
        if (wtopen[i] == 0 || wthot[i] == 0 || wtamb[i] == 0 || freqamb[i] < wfstart ||
            freqamb[i] > wfstop)
          wtcal[i] = 0;
        else
          wtcal[i] = 1;
        if (wtshort[i] == 0 || wtcal[i] == 0)
          ...
          wtcal[namb + i] = 1;
        if ((wtmode % 10) == 1 && freqamb[i] >= 55.0 && freqamb[i] <= 195)
          wtcal[i] = wtcal[namb + i] = 1;  // allow wideband cal
        if ((wtmode % 10) == 2 && freqamb[i] >= 80.0 && freqamb[i] <= 195)
          wtcal[i] = wtcal[namb + i] = 1;  // allow wideband cal
        if ((wtmode % 10) == 3) {
          if (wtopen[i] == 0 || wthot[i] == 0 || wtamb[i] == 0 || freqamb[i] < 80.0 ||
              freqamb[i] > 195.0)
            wtcal[i] = 0;
          else
            wtcal[i] = 1;
          if (wtshort[i] == 0 || wthot[i] == 0 || wtamb[i] == 0 || freqamb[i] < 80.0 ||
              freqamb[i] > 195.0)
            wtcal[namb + i] = 0;
          else
            wtcal[namb + i] = 1;
        ...
        if ((wtmode % 10) == 4) {
          if (wtopen[i] == 0 || wthot[i] == 0 || wtamb[i] == 0 || freqamb[i] < 50.0 ||
              freqamb[i] > 100.0)
            wtcal[i] = 0;
          else
            wtcal[i] = 1;
          if (wtshort[i] == 0 || wthot[i] == 0 || wtamb[i] == 0 || freqamb[i] < 50.0 ||
              freqamb[i] > 100.0)
            wtcal[namb + i] = 0;
          else
            wtcal[namb + i] = 1;
        ...
        if (!(i % (nopen / 10)) && iter >= nter - 1)
          printf(
              "iter %d freq %7.2f sca %7.2f s1 %7.2f s %7.2f s2 %7.2f tcold %7.2f thot "
              "%7.2f ofs "
              "%5.2f sca %5.2f tcal_ofs %5.2f tcal_sca %5.2f tlnau %5.2f\n",
              iter, freqhot[i], sca, s1, s, s2, tcold, thot, ofs, sca, tcal_ofs[i],
              tcal_sca[i], tlna0[i]);
      }
      //    for(i=0;i<namb;i++) Tll[i]=Tll2[i]=s11lna[i]; // test of fitting
      wavefit(nopen, nshort, wfit, sspopen, wtcal, freqopen, ss11cab, ss11lna, tcab,
              tload, tamb, tlna0, tlna1, tlna2, dataout, mcalc, fitf, aarr, bbrr,
              &delayln);
    }

    plotfspec(nopen + nshort, freqopen, sspopen, dataout, wtcal, dscale, 0, 3, openfile,
              shortfile,
              info);  // check on fit to open cable
    ...
    for (i = 0; i < namb; i++) {
      data[i] = thot;
      s2 = w3pinv(ss11hot[i], ss11lna[i], ssphot[i], tload, tamb, tlna0[i], tlna1[i],
                  tlna2[i]);
      if (Lh < 0)
    ...
    for (i = 0; i < namb; i++) {
      data[i] = tcold;
      dataout[i] = w3pinv(ss11amb[i], ss11lna[i], sspamb[i], tload, tamb, tlna0[i],
                          tlna1[i], tlna2[i]);
    }
    ...
    if (cfit > 0) {
      ...
      printf("rms_tcal_ofs %f K cfit %d\n", rmscalc(namb, tcal_ofs, tcal_ofssm, wtcal),
             cfit);
      //   for(i=0;i<namb;i++) printf("freq %f tcal_ofs %f tcal_ofssm %f diff %f wtcal
    ...
    outcal(namb, freqamb, ss11lna, tcal_scasm, tcal_ofssm, tlna0, tlna1, tlna2, wtcal,
           "cal_data");
    cal = 1;
    ...
    for (i = 0; i < namb; i++) {
      ...
      if (Lh == -2)
        Lhh =
            rigloss(ss11hot[i], ss11rig[i], ss12rig[i], ss22rig[i]);  // rigid cable loss
  ...
  if (cal == 0 || nant != namb) {
    ...
    if (ncal == 0) return 0;
    for (i = 0; i < ncal; i++)
      tmp[i] = ss11lna[i] * cexp(-2.0 * PI * freqcal[i] * 1e6 * (delaylna * I)) *
                   pow(10.0, 0.05 * ldb) +
               lr + li * I;
    fittp(ncal, freqcal, tmp, wtcal, nant, freqant, tmpsm, nfit1, fitf, mcalc, aarr, bbrr,
          6, fname);
  ...
  if (wtmode > 10)
    for (i = 0; i < nant; i++)
      if (freqs11ant[i] < wfstart - 1 || freqs11ant[i] > wfstop + 1)
        wttant[i] = 0;  // useful for lower order fit
  kk = 0;
  for (j = 0; j < ns11ant; j++)
    if (wttant[j]) kk++;
  if (nfit4 >= kk) nfit4 = kk - 1;
  fittp(ns11ant, freqs11ant, s11ant, wttant, nant, freqant, ss11ant, nfit4, fitf, mcalc,
        aarr, bbrr, 0, antfname);
  if (lmode == 2) {
    fittp(ns11rig, freqs11rig, s11rig, wttrig, nant, freqant, ss11rig, nfit2, fitf, mcalc,
          aarr, bbrr, 7, rigfname);
    fittp(ns11rig, freqs11rig, s12rig, wttrig, nant, freqant, ss12rig, nfit2, fitf, mcalc,
          aarr, bbrr, 8, rigfname);
    fittp(ns11rig, freqs11rig, s22rig, wttrig, nant, freqant, ss22rig, nfit2, fitf, mcalc,
          aarr, bbrr, 9, rigfname);
  }
  ...
  for (i = 0; i < nant; i++) {
    ...
    dataout[i] = w3pinv(ss11ant[i], ss11lna[i], sspant[i], tload, tamb, tlna0[i],
                        tlna1[i], tlna2[i]);

    if (lmode < 0)
      La =
          loss(ss11ant[i],
               atten + aloss * sqrt(freqant[i] /
                                    100.0));  // aloss > 0 for simple atten or cross-check
    else {                                    // 6 = no antenna or ground plane loss
      if (lmode == 2)
        La = rigloss(ss11ant[i], ss11rig[i], ss12rig[i],
                     ss22rig[i]);  // hot load - can only use if calibration being done
                                   // otherwise ss11rig=0
      else
        La = lossmodel(freqant[i], ss11ant[i], lmode) *
             antloss(freqant[i], mcalc, fitf, aarr, bbrr, aloss,
                     lmode);  // lmode 0=hot load 1=balun+bend 5or6=balun+bend lowband
    }
    if (wtant[i] && (La < 0.1 || La > 1.0))
      printf("check loss freq %f La %f\n", freqant[i], La);
    ...
    if (!(i % (nant / 10)))
      printf(
          "freqant %7.2f tlnau %7.2f tlnacc %7.2f tlnacs %7.2f tlnac %7.2f phase %7.2f "
          "tsky %7.2f "
          "md %7.2f loss %5.2f\n",
          freqant[i], tlna0[i], tlna1[i], tlna2[i],
          sqrt(tlna1[i] * tlna1[i] + tlna2[i] * tlna2[i]),
          atan2(tlna2[i], tlna1[i]) * 180.0 / PI, dataout[i], md[i], (1.0 - La) * tant);
    //        printf("loss %f dB La %f freq %f dataout %f md %f beamcr
    //        %f\n",loss,La,freqant[i],dataout[i],md[i],beamcr); beamcorr assumes a
    //        spectral index of -2.5
    if (test & 16 && skymod2 <= -128)
      dataout[i] =
          beamcorr(freqant[i] * (1.0 + low), bfit2, antaz, secs, mcalc, fitf, aarr, bbrr,
                   0, 0, &lst, bb2, &bspac, cmb, &freqref, 0, 0, site, map) *
          pow(1.0 / (1.0 + low), -2.5);  // substitute  beamcorrection
    if (skymode >= 0) {
      skymodel[i] =
          beamcorr(freqant[i] * (1.0 + low), bfit, antaz, secs, mcalc, fitf, aarr, bbrr,
                   0, 0, &lst, bb, &bspac, cmb, &freqref, 0, 0, site, map) *
          pow(1.0 / (1.0 + low), -2.5);
      ...
      dataout[i] = dataout[i] *
                   beamcorr(freqref, bfit, antaz, secs, mcalc, fitf, aarr, bbrr, 0, 0,
                            &lst, bb, &bspac, cmb, &freqref, 0, 0, site, map) *
                   pow(freqant[i] / freqref, -2.5) / skymodel[i];
    }
    if (skymod2 >= 0) {
      skymodel[i] =
          beamcorr2(freqant[i] * (1.0 + low), bfit, antaz, secs, mcalc, fitf, aarr, bbrr,
                    0, 0, &lst, bb, &bspac, cmb, &freqref, 0, 0, site) *
          pow(1.0 / (1.0 + low), -2.5);
      dataout[i] = dataout[i] *
                   beamcorr2(freqref, bfit, antaz, secs, mcalc, fitf, aarr, bbrr, 0, 0,
                             &lst, bb, &bspac, cmb, &freqref, 0, 0, site) *
                   pow(freqant[i] / freqref, -2.5) / skymodel[i];
    }

    if (test & 1 && (skymode >= 0 || skymod2 >= 0) && (skymode >= 256 || skymod2 >= 256))
      dataout[i] = skymodel[i];  // substitute  beamcorrection
    if (test == 2)
      dataout[i] =
          300 * pow(freqant[i] / 150.0, specin);  // test rms 14 mK at 2.55 1 mK at 2.51
    if (test == 4)
      dataout[i] =
          300 *
          pow(freqant[i] / 150.0,
              -2.55 +
                  0.01 * log(freqant[i] /
                             150.0));  // test rms 14 mK at 2.55 1 mK at 2.51 gamma 0.01
    if (test == 6) {
      dataout[i] = 300 * pow(freqant[i] / 150.0,
                             -2.55 + 0.0 * log(freqant[i] / 150.0));  // gamma 0.0
      dataout[i] += dataout[i] * (-1e-3 * pow(freqant[i] / 150.0, -2.0)) +
                    1e3 * 1e-3 * pow(freqant[i] / 150.0, -2.0);
    }  // 0.1% ion at 1000K
    if (test & 8) {
      if (tau > 0)
        dataout[i] += -eoramp *
                      (1 - exp(-tau * exp(-(freqant[i] - eorcen) * (freqant[i] - eorcen) *
                                          (-log(-log((1 + exp(-tau)) / 2) / tau)) /
                                          (eorwid * eorwid * 0.25)))) /
                      (1 - exp(-tau));
      else
        dataout[i] +=
            -eoramp * exp(-0.69 * (freqant[i] - eorcen) * (freqant[i] - eorcen) /
                          ((eorwid * eorwid + 1e-6) * 0.25));
      dataout[i] += dataout[i] * ion * (-1e-3 * pow(freqant[i] / 150.0, -2.0)) +
                    ion * 1e3 * 1e-3 * pow(freqant[i] / 150.0, -2.0);
    }  // 1% ion at 1000K
  }

  if (sim) {
    for (i = 0; i < nant; i++) {
      if (test & 1 && (skymode >= 0 || skymod2 >= 0) &&
          (skymode >= 256 || skymod2 >= 256)) {
        tant2 = skymodel[i] +
                skymodel[i] * ion * (-1e-3 * pow(freqant[i] / 150.0, -2.0)) +
                ion * 1e3 * 1e-3 * pow(freqant[i] / 150.0, -2.0);
        if (tau > 0)
          tant2 += -eoramp *
                   (1 - exp(-tau * exp(-(freqant[i] - eorcen) * (freqant[i] - eorcen) *
          ...
          tant2 += -eoramp * exp(-0.69 * (freqant[i] - eorcen) * (freqant[i] - eorcen) /
                                 ((eorwid * eorwid + 1e-6) * 0.25));
      } else
      ...
      data[i] =
          (w3p(ss11ant[i], ss11lna[i], tant2, tload, tamb, tlna0[i], tlna1[i], tlna2[i]) -
           tamb + tcal_ofssm[i]) /
              tcal_scasm[i] +
          tamb + noise * gauss();
    }
  ...
  if (ltemp[0])
    plotfspec(nant, freqant, ltemp, ltemp, wtant, dscale, 0, 5, title, datfile, info);
    ...
    for (i = 0; i < nfit; i++) {
      for (j = 0; j < nant; j++) {
        ...
        if (eorcen == 0) {
          ...
          if (i > 0 && (i % 2) == 1) {
            fitf[k] =
                w3pinv(ss11ant[j], ss11lna[j], sspant[j], tload, tamb, tlna0[j], tlna1[j],
                       tlna2[j])  // s11ant mag
                -
                w3pinv(ss11ant[j] * pow(10.0, -0.05 * 0.01 * pow(freq / fcen, (i / 2))),
                       ss11lna[j], sspant[j], tload, tamb, tlna0[j], tlna1[j], tlna2[j]);
          }
          ...
          if (i > 0 && (i % 2) == 0) {
            fitf[k] =
                ...
                -
                w3pinv(
                    ss11ant[j] * cexp(-(PI / 180.0) * pow(freq / fcen, (i / 2) - 1) * I),
                    ss11lna[j], sspant[j], tload, tamb, tlna0[j], tlna1[j], tlna2[j]);
          }
        } else {
          if (mdd == 1) {
          ...
          if (i == i1)
            fitf[k] = log(freq / freqr) *
                      pow(freq / freqr, specin);  // adjustment of spec. index
          if (i == i2)
            fitf[k] = log(freq / freqr) * log(freq / freqr) *
                      pow(freq / freqr, specin);  // slope of index "gamma"
          //       if(i==i2) fitf[k] =
          //       log(freq/freqr)*((freq-freqr)/freqr)*pow(freq/freqr,specin); linear
          //       case almost same
          if (i == i7)
            fitf[k] =
                pow(log(freq / 150.0), 3.0) *
                pow(freq / 150.0,
                    specin);  // next term
                              //       if(i==4) fitf[k] =
                              //       pow(log(freq/150.0),4.0)*pow(freq/150.0,specin);
                              //       // next term
          if (i == i3)
            fitf[k] = pow(
                freq / freqr,
                -2.0 + specin);  // ion absorption should be negative and multiplicative
          if (i == i4)
            fitf[k] =
                pow(freq / freqr, -2.0);  // ion emission should be positive and additive
          if (i == i5)
            fitf[k] = 1;  // constant
          ...
          if (i == i6)
            fitf[k] = pow(
                freq / freqr,
                sunind);  // Sun spe_ind 0 (quiet to -2(activ))   - see Kraus  0.5 best
          //       if(i==10) fitf[k] = 1.0/(1-ss11ant[j]*conj(ss11ant[j])); // some added
          //       instrumental functions will be needed for ant. s11 structure
          if (!mdd)
            fitf[k] = pow(freq / freqr,
                          specin + i);  // best according to tests 5 + 1 terms 20 MHz
                                        // full-width err=2 40 MHz err=6
          //       if(i==11) fitf[k] = 1.0 -
          //       loss(ss11ant[j],0,0,0,0,freqant[i],dlyrob,lmode); // some added
          //       instrumental functions will be needed for ant. s11 structure
          //       if((i==nfit-1 && nfit > 1 && eorwid > 0.0) || (i==1 && kk==0 && eorwid
          //       > 0.0)) fitf[k] =
          //       exp(-0.69*(freq-eorcen)*(freq-eorcen)/(eorwid*eorwid));  // eor
        }
      }
    }
  ...
  if (eorwid > 0.0 && cmb & 32) {
    for (i = 0; i < nant; i++)
      wtemp[i] = wtant[i] * exp(-0.69 * (freqant[i] - eorcen) * (freqant[i] - eorcen) /
                                (eorwid * eorwid));
    rms2 = rmscalc(nant, mcalc, dataout2, wtemp);
    if (smooth)
      rms2 = rms2 * sqrt(fabs(smooth));  // to correct correlated noise since smooth does
                                         // not reduce number of freqs
  }
  t150 = 0;
  for (i = 0; i < nfit; i++)
  ...
  i = nfit - 1;
  if (eorcen > 100.0)
    days = aarr[i + i * nfit] * 1e2 * 300.0 * 300.0 /
           (20e-3 * 20e-3 * (freqant[1] - freqant[0]) * 1e6 * 3600.0 * 8.0 * 0.3 * 0.4 *
            0.5);
  else
    days = aarr[i + i * nfit] * 1700.0 * 1700.0 * 1e2 /
           (100e-3 * 100e-3 * (freqant[1] - freqant[0]) * 1e6 * 3600.0 * 8.0 * 0.3 * 0.4 *
            0.5);  // 8hrs,3pos,40%eff,loadnoise
  if (eorwid > 0.0)
    printf(
        "Summary mfit %d mdd %d rms %4.0f K rms1 %6.1f mK value %8.1f err %5.1f snr "
        "%3.0f days "
        "%3.0f gha %6.2f T%d %5.1f sqrtcov %5.1f\n",
        mfit, mdd, rms, 1e3 * rms1, 1e3 * bbrr[i], sqrt(aarr[i + i * nfit]) / err1,
        bbrr[i] / (sqrt(aarr[i + i * nfit]) * rms2), days, gha, (int)freqr, t150,
        sqrt(aarr[i + i * nfit]));
  else
    printf("Summary mfit %d mdd %d rms %4.0f K rms1 %6.1f mK gha %6.2f T%d %5.1f\n", mfit,
           mdd, rms, 1e3 * rms1, gha, (int)freqr, t150);
  ...
  if ((i1 > 0 && i1 < nfit) || (i3 > 0 && i3 < nfit))
    printf(
        "Spectral_index %6.3f +/- %5.3f gamma %6.3f +/- %5.3f T%d %5.1f ion_opac %5.3f "
        "ion_em "
        "%5.0f K rms %6.3f K mfit %d mdd %d date %04d:%03d:%02d:%02d:%02d gha %8.3f\n",
        spe_ind, spe_inderr, gamma, gammaerr, (int)freqr, t150, ion_abs, ion_em, rms1,
        mfit, mdd, yr, dy, hr, mn, sc, gha);
...
void wavefit(int nopen, int nshort, int wfit, double spopen[], double wtopen[],
             double freqopen[], complex double s11cab[], complex double s11lna[],
             double tcab, double tload, double tamb, double tlna0[], double tlna1[],
             double tlna2[], double dataout[], double mcalc[], double fitf[],
    ...
    for (dl = 0; dl <= dd; dl += 1e-9) {  // not clear a delay search is needed
      ...
      for (j = 0; j < nfit; j++) {
        for (i = 0; i < mopen; i++) {
          ...
          cc = tlnacc * cos(2.0 * PI * freqopen[m] * dly) +
               tlnacs * sin(2.0 * PI * freqopen[m] * dly);
          cs = tlnacs * cos(2.0 * PI * freqopen[m] * dly) -
               tlnacc * sin(2.0 * PI * freqopen[m] * dly);
          fitf[i + j * mopen] =
              w3p(s11cab[i], s11lna[m], tcab, tload, tamb, tlnau, cc, cs) -
              w3p(s11cab[i], s11lna[m], tcab, tload, tamb, 0, 0,
...
void fittp(int np, double freqq2[], complex double s11[], double wtt[], int npout,
           double freqq1[], complex double Taa[], int nfit, double fitf[], double mcalc[],
           long double aarr[], double bbrr[], int mode, char *fname) {
  int i, j, k, i1, i2;
  double data[NDATA], dataout[NDATA], dbdiff[NDATA], pdiff[NDATA], sum, a, dfreq, rmsdb,
      rmsphase, f0, fcen, max, del, delay;
  ...
  for (j = 0; j < nfit; j++) {
    for (i = 0; i < np; i++) {
      if (nfit > 16) {  // changed 13dec17 to 14 7aug18 to 16 20aug18
        ...
        if (j % 2)
          fitf[i + j * np] = cos((freqq2[i] - f0) * 2.0 * PI * (j / 2 + 1) / dfreq);
        if (j % 2 == 0 && j > 0)
          fitf[i + j * np] = sin((freqq2[i] - f0) * 2.0 * PI * (j / 2) / dfreq);
      } else
        fitf[i + j * np] = pow(log10(freqq2[i] / fcen),
                               j);  // log  helps a little  pow(log10(freqq2[i]/fcen),j);
    }
  }
  for (k = 0; k < 2; k++) {
    for (i = 0; i < np; i++) {
    ...
    // if(mode==2) for(i=0;i<np;i++) printf("k %d i %d data %f s11 %f %f delay %f freqq2
  ...
  for (i = 0; i < np; i++)
    Taaa[i] = Taaa[i] * cexp(-2.0 * PI * freqq2[i] * 1e6 * delay * I);
  for (i = 0; i < npout; i++)
    Taa[i] = Taa[i] * cexp(-2.0 * PI * freqq1[i] * 1e6 * delay * I);
  sum = 0;
  rmsdb = rmsphase = 0;
  for (i = 0; i < np; i++) {
    dbdiff[i] = a =
        20.0 * log10(cabs(s11[i]) + 1e-6) - 20.0 * log10(cabs(Taaa[i]) + 1e-6);
    rmsdb += wtt[i] * a * a;
    a = (atan2(cimag(s11[i]), creal(s11[i])) - atan2(cimag(Taaa[i]), creal(Taaa[i]))) *
        180.0 / PI;
    if (a > 180.0) a += -360.0;
    if (a < -180.0) a += 360.0;
    pdiff[i] = a;
  ...
  if (mode == 0)
    printf("ant  S11 rms %f dB rms %f deg nfit4 %d %5.1f ns np %d npout %d\n", rmsdb,
           rmsphase, nfit, delay * 1e9, np, npout);
  if (mode == 1)
    printf("LNA  S11 rms %f dB rms %f deg nfit3 %d %5.1f ns np %d npout %d\n", rmsdb,
           rmsphase, nfit, delay * 1e9, np, npout);
  if (mode == 2)
    printf("hot  S11 rms %f dB rms %f deg nfit2 %d %5.1f ns np %d npout %d\n", rmsdb,
           rmsphase, nfit, delay * 1e9, np, npout);
  if (mode == 3)
    printf("amb  S11 rms %f dB rms %f deg nfit2 %d %5.1f ns np %d npout %d\n", rmsdb,
           rmsphase, nfit, delay * 1e9, np, npout);
  if (mode == 4)
    printf("cab1 S11 rms %f dB rms %f deg nfit2 %d %5.1f ns np %d npout %d\n", rmsdb,
           rmsphase, nfit, delay * 1e9, np, npout);
  if (mode == 5)
    printf("cab2 S11 rms %f dB rms %f deg nfit2 %d %5.1f ns np %d npout %d\n", rmsdb,
           rmsphase, nfit, delay * 1e9, np, npout);
  if (mode == 6)
    printf("tmp rms %f dB rms %f deg nfit1 %d np %d npout %d\n", rmsdb, rmsphase, nfit,
           np, npout);
  if (mode == 7)
    printf("tmp rms %f dB rms %f deg nfit2 %d np %d npout %d\n", rmsdb, rmsphase, nfit,
           np, npout);
  if (mode == 8)
    printf("tmp rms %f dB rms %f deg nfit2 %d np %d npout %d\n", rmsdb, rmsphase, nfit,
           np, npout);
  if (mode == 9)
    printf("tmp rms %f dB rms %f deg nfit2 %d np %d npout %d\n", rmsdb, rmsphase, nfit,
           np, npout);
  if (mode <= 5)
    plotvna(np, freqq2, s11, wtt, npout, freqq1, Taa, dbdiff, pdiff, mode, rmsdb,
            rmsphase, nfit, fname);
}
...
double wavemodel(complex double Ta, complex double Tl, double tsky, double tlnau,
  ...
  complex double F;
  double t, ph;
  F = sqrt(1.0 - Tl * conj(Tl)) /
      (1.0 - Ta * Tl);  // F = 1.0/(1.0 - Ta*Tl) gets same results after cal
...
double w3p(complex double Ta, complex double Tl, double tsky, double tload, double tamb,
           double tlnau, double tlnacc, double tlnacs) {
  ...
  return 100.0 * (pant - pload) / (pcal - pload) +
         tamb;  // assumes first stage processing uses same tamb and tcal for all spectra
}

double w3pinv(complex double Ta, complex double Tl, double tin, double tload, double tamb,
              double tlnau, double tlnacc, double tlnacs) {
  complex double F;
  double tout;
  F = 1.0 / (1.0 - Ta * Tl);
...
double antloss(double freq, double mcalc[], double fitf[], long double aarr[],
               double bbrr[], double aloss, int mode) {
  double sum, sum2;
  int i, j, nfit, neff;
  ...
  double eff[] = {99.9364, 99.9485, 99.9576, 99.9644, 99.9694, 99.9729, 99.9751,
                  99.9764, 99.9769, 99.9766, 99.9758, 99.9745, 99.9727, 99.9706,
                  99.9681, 99.9654, 99.9625, 99.9594, 99.9561, 99.9528, 99.9493};
  ...
  double losh[] = {3.470102e+01, -1.335697e+02, 1.824230e+02, -1.000643e+02,
                   1.874884e+01};  // highband GFhigh
  //    double los[] = {1.564797e+02,  -5.130209e+02,  6.599304e+02,
  //    -3.749992e+02,  7.931530e+01};
  //    // temploss9 - 2.5x2.5 0.1 genblade9e GF 13 2e-3 double los[] = {7.580215e+01,
  //    -2.684529e+02,  3.635722e+02,  -2.154928e+02,  4.726889e+01}; // GF 3.5 1e-3
  double los[] = {
      5.744029e+01, -1.726088e+02, 2.142321e+02, -1.225013e+02,
      2.702699e+01};  // GF 3.5 180deg
                      //    double losk[] = {6.410025e+00, -1.549803e+01,  1.544351e+01,
                      //    -7.041051e+00,  1.300656e+00};
  //    double losk[] = {6.321415e+01,  -1.886167e+02,  2.293411e+02,
  //    -1.206021e+02,  2.315841e+01};
  //    // for low3 memo 290 double losk[] = {1.041479e+01, -6.031950e+00,  2.307908e+00};
  //    // for low3 3-terms double losk[] =
  //    {-1.179120134559797e+01,  8.816080087652101e+01,
  //    -2.263158238997412e+02,  2.823787777684456e+02,
  //    -1.647342754171209e+02,  3.580581311748211e+01};
  double losk[] = {
      -1.345035060985746e+00, 2.240940678524730e+00, 7.902278433276813e+00,
      -5.318319232436456e+00};  // loss_low3a_2e-2_rock.txt -nfit 4 -fmin 52 -fmax 110
  double losg5[] = {4.370180753656857e+02, -7.610893859849077e+02, 7.252089244174796e+02,
                    -3.487898288054475e+02, 6.949783330081493e+01};  // ground loss
  double losg7[] = {4.846670825246866e+02, -1.023102678080684e+03,
                    1.303906332843779e+03, -1.000317701465261e+03,
                    4.580417451841646e+02, -1.129872060290666e+02,
                    1.163765940857081e+01};  // ground loss
  double losg8[] = {3.926509849561377e+00, -1.838394495633041e+01,
                    3.929777791337121e+01, -4.597706096084568e+01,
                    3.037985609919453e+01, -1.062848140609678e+01,
                    1.531802534710291e+00};  // mesh loss
  ...
  if (mode == 1) {  // only use antloss for lmode = 1
    ...
    sum2 = 0;
    for (j = 0; j < nfit; j++)
      sum2 += losh[j] * pow(freq / 150.0, j);  // when using GFhigh
  ...
  if (mode == 5) {  // for lmode = 5 original ground plane
    ...
    sum = 0;
    for (j = 0; j < nfit; j++)
      sum += los[j] * pow(freq * 2.0 / 150.0,
                          j);  // when using GF
                               //     for(j=0;j<nfit;j++) sum += (los[j]/4.0)*pow(freq
                               //     * 2.0 / 150.0,j);  // divide by 4 to get 5x5m
  ...
  if (mode == 100) {
    return (
        pow(10.0, -0.1 * aloss * sqrt(freq / 100.0)));  // loss in dB independent of S11
  }
  return 1;
}
...
double loss(complex double s11a, double atten)  // simple attenuation
{
  ...
  L = ss21 * conj(ss21) * (1 - T * conj(T)) /
      ((1 - s11a * conj(s11a)) * (1 - ss22 * T) * (1 - conj(ss22 * T)));
  //   if(L < 0.1) printf("error check loss L=%f\n",L);
  return L;
}

double lossmodel(double freq, complex double s11a, int mode) {
  complex double s11, s22, s12, s21, ta11, ta12, ta21, ta22, tb11, tb12, tb21, tb22, t11,
      t12, t21, t22, T;
  ...
  double ss11r[] = {
      0.000622, 0.004519, 0.000199,
      -0.000442};  // lossfit2m2a 40 - 200 using semi_rigid_s_parameters_WITH_HEADER.txt
  ...
  if (mode == 5 || mode == 6 || mode == 7 || mode == 8 || mode == 9)
    del = 43.6 * 2.54e-2 / 3e08;  // balun lowband 43"

  if (mode == 10) del = 35 * 2.54e-2 / 3e08;  // balun midband 35"
  if (mode == 11)
    del = 33.5 * 2.54e-2 /
          3e08;  // balun midband 33.5"
                 //   if(mode==4) {del4=2.5*2.54e-2/3e8; del=0.51/3e08; ttest=0.2;}
                 //   // balun + bends for active coldload test if(mode==0) {del=0;
                 //   del4=3.03e-10;}  // for hotload  - fit to semi from raul
  if (mode == 5 || mode == 6 || mode == 7 || mode == 8 || mode == 9 || mode == 10 ||
      mode == 11) {
  ...
  if (mode ==
      2) {  // fit to /home/aeer/px14/mro/data2/semi_rigid_S_parameters.txt - as example
    s11 = 0;
    for (m = 0; m < 4; m++)
      s11 += s11r[m] * pow(freq / 150.0, m) + s11i[m] * pow(freq / 150.0, m) * I;
    s12 = 0;
    for (m = 0; m < 4; m++)
      s12 += s12r[m] * pow(freq / 150.0, m) + s12i[m] * pow(freq / 150.0, m) * I;
    s22 = 0;
    for (m = 0; m < 4; m++)
      s22 += s22r[m] * pow(freq / 150.0, m) + s22i[m] * pow(freq / 150.0, m) * I;
    s21 = s12 = csqrt(s12);
    T = (s11a - s11) /
        (s12 * s21 - s11 * s22 +
         s22 * s11a);  // from memo 132 - need to check Raul's convention s11 vs s22
    return cabs(s12 * s21) * (1 - T * conj(T)) /
           ((1 - s11a * conj(s11a)) * (1 - s22 * T) * (1 - conj(s22 * T)));
  }
  if (mode == 0) {  // fit to /home/aeer/px14/mro/data4/semi_rigid_s_parameters.txt - as
                    // example lossfit2m2.c
    s11 = 0;
    for (m = 0; m < 4; m++)
      s11 += ss11r[m] * pow(freq / 150.0, m) + ss11i[m] * pow(freq / 150.0, m) * I;
    s12 = 0;
    for (m = 0; m < 4; m++)
      s12 += ss12r[m] * pow(freq / 150.0, m) + ss12i[m] * pow(freq / 150.0, m) * I;
    s22 = 0;
    for (m = 0; m < 4; m++)
      s22 += ss22r[m] * pow(freq / 150.0, m) + ss22i[m] * pow(freq / 150.0, m) * I;
    s21 = s12 = csqrt(s12);
    T = (s11a - s11) /
        (s12 * s21 - s11 * s22 +
         s22 * s11a);  // from memo 132 - need to check Raul's convention s11 vs s22
  ...
  //   t11=tb11*ta11+tb12*ta21; t12=tb11*ta12+tb12*ta22; t21=tb21*ta11+tb22*ta21;
  //   t22=tb21*ta12+tb22*ta22; //B*A
  s11 = t12 / t22;
  s12 = (t11 * t22 - t12 * t21) / t22;
  s21 = 1 / t22;
  s22 = -t21 / t22;
  T = (s11a - s11) / (s12 * s21 - s11 * s22 + s22 * s11a);  // from memo 132
  L2 = cabs(s12 * s21) * (1 - T * conj(T)) /
       ((1 - s11a * conj(s11a)) * (1 - s22 * T) * (1 - conj(s22 * T)));
...
double rigloss(complex double s11a, complex double s11, complex double s12,
               complex double s22) {
  complex double T;
  T = (s11a - s11) / (s12 - s11 * s22 + s22 * s11a);  // from memo 132  s12 = s12*s21
  return cabs(s12) * (1 - T * conj(T)) /
         ((1 - s11a * conj(s11a)) * (1 - s22 * T) * (1 - conj(s22 * T)));
}
...
complex double cabl2(double freq, double delay, complex double Tin, int mode,
                     complex double *ss11, complex double *ss12, double ttest) {
  ...
  // ttest = 0.5;  // best fit from day 2015_023
  {
    ...
    d2 = sqrt(1.0 /
              (PI * 4.0 * PI * 1e-7 * 5.96e07 * 0.29));  // skin depth at 1 Hz for brass
  ...
  if (mode == 5) {
    ...
    d2 = sqrt(1.0 /
              (PI * 4.0 * PI * 1e-7 * 5.96e07 * 0.29));  // skin depth at 1 Hz for brass
    d = sqrt(1.0 /
             (PI * 4.0 * PI * 1e-7 * 5.96e07 * ttest));  // skin depth at 1 Hz for copper
  }
  if (mode == 8) {
    b = 0.161 * 2.54e-2 * 0.5;
    a = 0.05 * 2.54e-2 * 0.5;
    ...
    d = sqrt(1.0 / (PI * 4.0 * PI * 1e-7 * 5.96e07 * 0.24 *
                    ttest));  // skin depth at 1 Hz for brass - might be less 0.24
  }
  if (mode == 9) {
    b = 0.16 * 2.54e-2 * 0.5;
    a = 0.05 * 2.54e-2 * 0.5;
    ...
    d2 = sqrt(1.0 / (PI * 4.0 * PI * 1e-7 * 5.96e07 * 0.024 * ttest));  // for Stainless
    d = sqrt(1.0 / (PI * 4.0 * PI * 1e-7 * 5.96e07 * 0.20 *
                    ttest));  // skin depth at 1 Hz note change
  }
  if (mode == 12) {
    b = 0.1515 * 2.54e-2 * 0.5;
    a = 0.0453 * 2.54e-2 * 0.5;
    diel = 2.1;  // Molex WM10479-nd FEP molex 100054008 could only find inner = 0.0453
                 // inch twicked outer to get 50 ohms
...
complex double cabl2(double freq, double delay, complex double Tin, int mode,complex
double *ss11, complex double *ss12, double ttest) { complex double
T,Zcab,g,s11,s12,s21,s22,Vin,Iin,Vout,Iout,VVin,Z; double a,b,d,diel,R,C,L,G; double
q,Rs,R0,Ra,Xa,s,s1,s2;
  // ttest = 0.5;  // best fit from day 2015_023
  ...
  if(mode==5) {  b=0.75*2.54e-2*0.5; a=(5.0/16.0)*2.54e-2*0.5; diel = 1.07;  // balun tube
lowband s2 = 5.96e07*0.29;  // skin depth at 1 Hz for brass s1 = 5.96e07;  // skin depth
at 1 Hz for copper
              }
  if(mode==8) { b=0.304*2.54e-2*0.5; a=0.086*2.54e-2*0.5; diel = 2.05;  // SC3792
connector s2 = 5.96e07*0.024*ttest;   // for Stainless s1 = 5.96e07*0.24*ttest;  // skin
depth at 1 Hz for brass - might be less 0.24
              }
  if(mode==9) { b=0.16*2.54e-2*0.5; a=0.05*2.54e-2*0.5; diel = 2.05;  // SMA connector
                s2 = 5.96e07*0.024*ttest;   // for Stainless
                s1 = 5.96e07*0.05*ttest;  // skin depth at 1 Hz for brass - might be less
0.24
              }
  L = (4.0*PI*1e-7/(2.0*PI))*log(b/a);
  C=2.0*PI*8.854e-12*diel/log(b/a);
...
double beamcorr(double freq, int bfit, double ang, double secs, double mcalc[],
                double fitf[], long double aarr[], double bbrr[], int skymode, int integ,
                double *lst, double bb[], double *bspac, int cmb, double *freqref,
                double fbstart, double fbstop, int site, int map) {
  char name[255], buf[32768], *p;
  FILE *file3;
  static double azel[360 * 91 * NBEAM], data[NBEAM], data2[NBEAM], wt[NBEAM], fsum[NBEAM],
      fsum1[NBEAM], fsum2[NBEAM], poww[NBEAM], mapgal[512][1024];
  static double rraa[512][1024], sindec[512][1024], cosdec[512][1024];
  static double map45[512][1024], spind[512][1024];
  double k, sum, amp, amp150, raa, dec, lat, lon, gstt, sunra, sundec, sunaz, sunel, glat,
      glon, wsum, azz, el, sang, mp, dp, gp, opac, telec, rms, wb, aind, wsum2;
  double ssecs, fr, cmbb, av45, av408, nav;
  int i, j, n, nn, az, frq, frq150, iaz, iel, m, frqst, frqspac, nbeam, yr, dy, hr, mn,
      sc, ii, jj, iii, jjj;
  ...
  if (integ > 0) {
        ...
        while (j < 91 && p && *p != '\n') {
          ...
          if (skymode >= 512) {
            ...
            if (fabs(sint) > 1e-6 && el >= 0.0) {
              amp = PI * ((frqst + i * frqspac) / 150.0) /
                    2.0;  // half-wavelength at 150 MHz dipole
            ...
            if (zb > 0.0)
              ...
              amp = 1e-6;
            // if((az==0 || az==90) && el==60)     printf("az %d el %d fr %d amp1 %f amp2
            // %f ratio
    ...
    while (fgets(buf, 32768, file3) != 0) {
      ...
      while (j < 1024 && p && sscanf(p, "%lf", &k) == 1) {
        while (*p == ' ') p++;
        if (*p && p) p = strchr(p, ' ');
        if (cmb & 4 && (map == 0 || map == 2))
          k += -5;  // offset correction for Haslam 408 map
        if (cmb & 4 && (map == 1 || map == 3))
          k = k * 1.076 - 160.0;  // Guzman map correction
        if (cmb & 8) {
          if (i == 256 && j == 512)
            k = 2.3e6;
          else
        ...
        if (cmb & 64)
          k = exp(-4.0 * log(2.0) * ((i - 256) * (i - 256) + (j - 512) * (j - 512)) /
                  810.0) *
                  2.6e3 +
    ...
    if (map >= 2) {
      ...
      for (i = 0; i < 512; i++) {
        for (j = 0; j < 1024; j++) {
          ...
          spind[i][j] =
              log((av45 / nav - cmbb) / (av408 / nav - cmbb)) / log(408.0 / 45.0);
        }
      }
    }
    telec = 1e3;
    ...
    for (n = 0; n < nn; n++) {
      ...
      *lst = (gstt + lon) * 12.0 / PI;
      ...
      printf(
          "Spsecs %10.0f %4d:%03d:%02d:%02d:%02d sunaz %6.2f sunel %6.2f lst %6.2f gha "
          "%6.2f\n",
          ssecs, yr, dy, hr, mn, sc, sunaz, sunel, *lst, *lst - (17.0 + 45.67 / 60.0));
      for (i = 0; i < 512; i += 1) {
        glat = (i - 256.0) * 90.0 / 256.0;
        sang = (180.0 / 512.0) * cos(glat * PI / 180.0) * (360.0 / 1024.0) * PI * PI /
               (180.0 * 180.0);
        for (frq = 0; frq < nbeam; frq++) {
        ...
        for (j = 0; j < 1024; j += 1) {
          radec_azel2(gstt - rraa[i][j] + lon, sindec[i][j], cosdec[i][j], sinlat, coslat,
                      &azz, &el);
          azz = azz * 180.0 / PI;
          el = el * 180.0 / PI;
          //  phi=el*PI/180.0; R=6356.0; h=300.0;  opac =
          //  (1.8/500.0)*(R+h)/sqrt(R*R*sin(phi)*sin(phi)+2*R*h+h*h); // from derivative
          if (azz < 0) azz += 360.0;
          if (el < 0) el = 0;
          iaz = azz + 0.5;
          iel = el + 0.5;
          ...
          //  Haslam 384 -site 0 Guz 384 site 10 Haslam+spind 256 site 100 Guz+spind 256
          //  site 1000
          aind = 2.5;
          if (el > 0 && !(skymode & 128)) {
          ...
          for (frq = 0; frq < nbeam; frq++) {
            ...
            if (el > 0) {
              ...
              if (skymode & 128) {
                wsum =
                    (mp - cmbb) * poww[frq] +
                    cmbb;  // speeds up execution
                           //                   if(fabs(glat)<10) wsum *=
                           //                   pow(fr*0.5/408.0,0.1);  // correct
                           //                   galactic plane spectral index difference
              } else {
                if (map >= 2) {
                  ...
                  wsum = (mp - 3.0) * ((1 - wb) * pow(fr / 408.0, -2.52 + 0.016 * gp) +
                                       wb * pow(fr / 408.0, -2.8 - 0.13 * gp)) +
                         3.0;  // best values from 2015:024
                  if (fabs(glat - 45) < 15 && fabs(glon - 27) < 15)
                    wsum =
                        mp * pow(150.0 / 408.0, -2.5) *
                        pow(fr / 150.0, -3.0 - 0.5 * gp);  // test north polar spur region
                }
              }
              if (skymode & 4)
                wsum += telec * opac * pow(fr / 150.0, -2.0);  // ion emission
              if (skymode & 8)
                wsum += (telec - wsum) * opac *
                        pow(fr / 150.0, -2.0);  // ion emission and absorption
              if (skymode & 16) wsum = 500.0 * pow(fr / 150.0, -2.5);
              if (cmb & 16)
                wsum2 = 3e-3 * wsum *
                        cos(2 * PI * fr * 1e6 * 50.0 *
                            (1.0 - cos(el * PI / 180.0) * cos((azz - 90) * PI / 180.0)) /
                            3e8);  // part in 1e3 scatter object east of antenna
              if (cmb & 256)
                wsum2 = -0.5 *
                        (1 - exp(-7 * exp(-(fr - 78) * (fr - 78) *
                                          (-log(-log((1 + exp(-7.0)) / 2.0) / 7.0)) /
                                          (19 * 19 * 0.25)))) /
    ...
    if (bfit >= 0)
      bfit = nbeam - bfit;  // input bfit = 1 useful to avoid ripple at ends of fit
    if (bfit < 0) bfit = -bfit;
    if (bfit > 50) bfit = 50;
    *bspac = (nbeam * 1.2) * frqspac;  // need +20% to avoid ripple
    for (i = 0; i < bfit; i++) {
      for (j = 0; j < nbeam; j++) {
        //      if(j*frqspac+frqst >= 100 && j*frqspac+frqst < 200)  wt[j] = 1; else wt[j]
        //      = 0;
        wt[j] = 1;
        if (bfit >= 7 && bfit <= 14)
          ...
          if ((i % 2) == 0)
            fitf[j + i * nbeam] =
                cos((j * frqspac + frqst - (*freqref)) * 2 * PI * (i / 2) / (*bspac));
          if ((i % 2))
            fitf[j + i * nbeam] = sin((j * frqspac + frqst - (*freqref)) * 2 * PI *
                                      ((i + 1) / 2) / (*bspac));
        }
      }
    }
    ...
    for (frq = 0; frq < nbeam; frq++)
      data[frq] = (fsum[frq] / fsum1[frq]) /
                  (fsum2[frq] / fsum2[frq150]);  // normalize to 150 21 Jan 16
    if (cmb & 512)
      for (frq = 0; frq < nbeam; frq++)
        data[frq] = (fsum[frq] / fsum1[frq]) /
                    pow((frqst + frq * frqspac) / ((double)(frqst + frq150 * frqspac)),
                        -2.5);  // previous
    ...
    //    for(i=0;i<nbeam;i++) printf("Spbeam data %d %f %f diff %e bbrr %e rms %f lst
  ...
  for (i = 0; i < bfit; i++) {
    if (bfit >= 7 && bfit <= 14)
      ...
      if ((i % 2) == 0)
        sum += bb[i] * cos((freq - (*freqref)) * 2 * PI * (i / 2) / (*bspac));
      if ((i % 2))
        sum += bb[i] * sin((freq - (*freqref)) * 2 * PI * ((i + 1) / 2) / (*bspac));
    }
  }
...
double beamcorr2(double freq, int bfit, double ang, double secs, double mcalc[],
                 double fitf[], long double aarr[], double bbrr[], int skymode, int integ,
                 double *lst, double bb[], double *bspac, int cmb, double *freqref,
                 double fbstart, double fbstop, int site) {
  char name[255], buf[32768], *p;
  FILE *file3;
  static double azel[360 * 91 * NBEAM], data[NBEAM], data2[NBEAM], wt[NBEAM], fsum[NBEAM],
      fsum1[NBEAM], fsum2[NBEAM];
  static float gmap[3145728];
  double k, sum, amp, amp150, raa, dec, lat, lon, gstt, glat, glon, wsum, azz, el, sang,
      mp, dp, gp, opac, telec, rms, wb;
  ...
  if (integ > 0) {
    ...
    if (!fread(gmap, sizeof(gmap), 1, file3))
      printf("read error here %f %f\n", gmap[0], gmap[1]);
    fclose(file3);
    ...
    for (n = 0; n < nn; n++) {
      ...
      *lst = (gstt + lon) * 12.0 / PI;
      ...
      for (i = 0; i < 3145728; i++) {
        ...
        h = 300.0;
        opac = (1.8 / 500.0) * (R + h) /
               sqrt(R * R * sin(phi) * sin(phi) + 2 * R * h + h * h);  // from derivative
        m = 0;
        if (azz < 0) azz += 360.0;
        if (el < 0) el = 0;
        iaz = azz + 0.5;
        ...
        for (frq = 0; frq < nbeam; frq++) {
          ...
          if (el > 0) {
            ...
            if (skymode & 2)
              gp = -0.12 *
                   log((frqst + frq * frqspac) / 150.0);  // derivative Angelica's gamma
            else
              gp = 0.0;
            if (skymode & 128)
              wsum = (mp - cmbb) * pow((frqst + frq * frqspac) / 408.0, -2.5 + dp + gp) +
                     cmbb;
            else {
              // correct 408 MHz map to 150 MHz - makes slight improvement at most GHAs -
              // see memo 7
              if (fabs(glat) < 10.0)
                wb = (cos(glon * PI / 180.0) + 1.0) / 2.0;
              else
                wb = 0;
              wsum = (mp - 3.0) * (wb * pow(150.0 / 408.0, -2.75 + 2.5) + (1 - wb)) *
                         pow((frqst + frq * frqspac) / 408.0, -2.5 + dp + gp) +
                     3.0;
            }
            if (skymode & 4)
              wsum += telec * opac *
                      pow((frqst + frq * frqspac) / 150.0, -2.0);  // ion emission
            if (skymode & 8)
              wsum += (telec - wsum) * opac *
                      pow((frqst + frq * frqspac) / 150.0,
...
void dsmooth(int n, double data[], double data2[], double wt[], double mcalc[],
             int smooth) {
  double a, b, c;
  int i, j, k;
...
void plotfspec(int np, double freqq[], double data[], double data2[], double wtt[],
               double ddscale, int mode, int num, char title[], char datfile[],
               char info[])
// plot the spectrum
{
  char txt[256];
  ...
  if (num == 0)
    sprintf(txt,
            "raw sky spectrum (thin line) vs calibrated spectrum of sky (thick line)");
  if (num == 1)
    sprintf(txt,
            "hot temperature (thin line) vs calibrated spectrum of hot load(thick line)");
  if (num == 2)
    sprintf(txt,
            "ambient temperature (thin line) vs calibrated spectrum of ambient "
            "load(thick line)");
  if (num == 3)
    sprintf(txt, "cable spectrum (thin line) vs fit to cable spectrum (thick line)");
  if (num == 4)
    sprintf(txt,
            "calibrated sky spectrum (thin line) vs sky model spectrum (thick line)");
  if (num == 5)
    sprintf(txt, "spectrum of loss (thin line) vs spectrum of loss (thick line)");
  if (num == 6)
    sprintf(txt, "spectrum of beam (thin line) vs spectrum of fit (thick line)");
  if (num == 99)
    sprintf(txt,
            "calibrated sky spectrum (thin line) vs fit to sky spectrum (thick line) %s",
            info);
  fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n", xoffset, 55.0, txt);
...
void outfspec(int np, double freqq[], double data[], double skymodel[], double sunn[],
              double lst, int mode, int nline, char title[])
// writeout calibrated spectrum
{
  ...
  for (k = 0; k < np; k++) {
    if (mode == 1) {
      fprintf(file,
              "freq %10.6f tantenna %10.6f K skymodel %10.6f K ant_area %8.6f m^2 "
              "Tsun_per_SFU "
              "%5.3f K lst %5.2f %s\n",
              freqq[k], data[k], skymodel[k], sunn[k], sunn[k] * 1e-22 / (2.0 * 1.38e-23),
              lst, title);
    }
    if (mode == 0) {
...
void outcal(int np, double freqq[], complex double s11lna[], double sca[], double ofs[],
            double tlnau[], double tlnacc[], double tlnacs[], double wtcal[],
            char title[])
// writeout calibrated spectrum
{
  char txt[256];
  ...
  for (k = 0; k < np; k++) {
    fprintf(file,
            "freq %10.6f s11lna %10.6f %10.6f sca %10.6f ofs %10.6f tlnau %10.6f tlnac "
            "%10.6f "
            "tlnas %10.6f wtcal %1.0f %s\n",
            freqq[k], creal(s11lna[k]), cimag(s11lna[k]), sca[k], ofs[k], tlnau[k],
            tlnacc[k], tlnacs[k], wtcal[k], title);
  }
  fclose(file);
}

void outsim(int np, double freqq[], double spant[], int yr, int dy, int hr, int mn,
            int sc)
// simulated spectrum
{
  char txt[256];
...
void readcal(int *np, double freqq[], complex double s11lna[], double sca[], double ofs[],
             double tlnau[], double tlnacc[], double tlnacs[], double wtcal[]) {
  ...
  *np = 0;
  ...
  while (fgets(buf, 32768, file) != 0) {
    ...
    T = re + im * I;
    //                    T = T*cexp(-2.0*PI*freq*1e6*(0.0e-12)*I)*pow(10.0,0.05*0.2); //
  ...
  *np = k;
  for (k = 0; k < *np; k++) {
    //    printf("freq %10.6f s11lna %10.6f %10.6f sca %10.6f ofs %10.6f tlnau %10.6f
    //    tlnac %10.6f tlnas %10.6f wtcal %1.0f\n",
    //       freqq[k],creal(s11lna[k]),cimag(s11lna[k]),sca[k],ofs[k],tlnau[k],tlnacc[k],tlnacs[k],wtcal[k]);
  }
  fclose(file);
}

void plotvna(int np, double freqq[], complex double data[], double wtt[], int npout,
             double freqq2[], complex double data2[], double dbdiff[], double pdiff[],
             int mode, double rmsdb, double rmsphase, int nfit, char *fname)
// plot the spectrum
{
  char txt[256];
  ...
  dmin = 1e99;
  for (k = 0; k < npout; k++)
    if (10.0 * log10(creal(data2[k] * conj(data2[k]))) > dmax)
      dmax = 10.0 * log10(creal(data2[k] * conj(data2[k])));
  for (k = 0; k < npout; k++)
    if (10.0 * log10(creal(data2[k] * conj(data2[k]))) < dmin)
      dmin = 10.0 * log10(creal(data2[k] * conj(data2[k])));
  dscale = pscale = 1e-99;
  for (k = 0; k < np; k++)
  ...
  for (iter = 0; iter < 6; iter++) {
    ...
    for (k = 0; k < nn; k++) {
      ...
      if (iter == 0)
        totpp = (10.0 * log10(creal(data[k] * conj(data[k]))) - dmin) / scale;
      if (iter == 2)
        totpp = (10.0 * log10(creal(data2[k] * conj(data2[k]))) - dmin) / scale;
  ...
  }
  {
    ...
    fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n", xoffset + 50.0, yoffset + 465.0,
            txt);
  }
  {
    sprintf(txt, "fit %5.4f p-p deg", 2.0 * pscale);
    fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n", xoffset + 50.0, yoffset + 405.0,
            txt);
  }
  {
    sprintf(txt, "magnitude(thin line)");
    fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n", xoffset + 280.0, yoffset + 25.0,
            txt);
  }
  {
    sprintf(txt, "phase(thick line)");
    fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n", xoffset + 280.0, yoffset + 10.0,
            txt);
  }
...
void plotnwave(int np, double freqq[], double data[], double data2[], double data3[],
               double delaycab, int wfit)
// plot the spectrum
{
  char txt[256];
  int k, iter, nter;
  ...
  for (iter = 0; iter < nter; iter++) {
    ...
    for (k = 0; k < np; k++) {
      ...
      totpp = (data[k] - dmin) / scale;
      if (iter == 1)
        totpp = (sqrt(data2[k] * data2[k] + data3[k] * data3[k]) - dmin) / scale;
      if (iter == 2) {
        dly = delaycab * 1e6;
        cc = data2[k] * cos(2.0 * PI * freqq[k] * dly) +
             data3[k] * sin(2.0 * PI * freqq[k] * dly);
        cs = data3[k] * cos(2.0 * PI * freqq[k] * dly) -
             data2[k] * sin(2.0 * PI * freqq[k] * dly);
  ...
  sprintf(txt, "%5.5f ns delay taken out %d term Fit to noise waves", delaycab * 1e9,
          wfit);
...
void radec_azel2(double ha, double sindec, double cosdec, double sinlat, double coslat,
                 double *azs, double *elevs) {
