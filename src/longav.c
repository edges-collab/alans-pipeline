...
double fition(int,int,double*,double*,double*,double*,double*,double*,double*,double*,double*,double*,double*,double,double,int,int,double,double);
...
double chifit(int,int,double*,double*,double*,double*,int,double,double*,double*,double*,double*,double*,int,double,double,unsigned char*,double,double,double,double);
...
FILE *file3;
static double spec[NSPEC*NDATA],spdiff[NSPEC*NDATA],freq[NSPEC*NDATA],wtt[NSPEC*NDATA],specout[NSPEC*NDATA],wtav[NDATA],freqpl[NSPEC*NDATA];
...
double rms,maxrms,t150,f150,spind,spcurv,ionabs,ionemm,speor,cov,lim,limm,fstart,fstop,wid,eora,aeora,seor,feor,rfi,mchk,tchk,schk,test;
double snr,adnoise,gha,t150dn,diff,avrms,tau,fsmax,fsmin;
int i,j,m,yr,day,hr,min,sc,nplot,j3,nfit,nn[NSPEC],nnpl[NSPEC],out,wtmode,sig,ndn,g10,alt,half,sim,imode,tmode,pmode,rr,resid,sm,md;
    dmax = 10; lim=1e6; fstart=0; fstop=200; nfit=4; seor=0; adnoise = 0; out=0; sig=0; g10=1; diff=alt=half=rfi=mchk=schk=imode=tmode=pmode=rr=test=resid=0; 
    eora=aeora=0; feor = 150; wid=10; wtmode=0; sim=0; tchk = -1e99; tau= 0; sm = 0; md = 0; fsmax = 200; fsmin = 40;
    sprintf(ti,"GHA");
   ...
   if (strstr(buf, "-ti")) {sscanf(argv[m+1], "%s",ti); tmode=1;}
...
// freq 100.0045 tantenna 746.8551 K skymodel 746.9250 K wt 1 2015:227:20:43:03
          ...
          if(frq < frqp) {i=0; j++; // printf("j %d freq %f %f\n",j,frq,frqp);
          ...
          if(sim) {spe = sim * 300.0*pow(frq/150,-2.6+0.1*log(frq/50)+0.0*pow(log(frq/75),2.0))*exp(-2e-2*150.0*150.0/(frq*frq))+2.0e-2*3e3*pow(frq/150,-2.0);
//          if(sim) {spe = sim * 300.0*pow(frq/150,-2.6+0.1*log(frq/50)+0.0*pow(log(frq/75),2.0))*exp(-2e-3*150.0*150.0/(frq*frq))+2.0e-3*3e3*pow(frq/150,-2.0);
//                   spe += sim * 300.0*pow(frq/150,-2.6+0.3*log(frq/50)+0.1*pow(log(frq/75),2.0))*exp(-2e-2*150.0*150.0/(frq*frq))+2.0e-2*3e3*pow(frq/150,-2.0);
                  }
...
//          if(test) spe += -test*(1-exp(-flt*exp(-0.693*(frq-feor)*(frq-feor)/(wid*wid*0.25))));  // test positive for absorption
          if(test) spe += -test*(1-exp(-tau*exp(-(frq-feor)*(frq-feor)*(-log(-log((1+exp(-tau))/2)/tau))/(wid*wid*0.25))))/(1-exp(-tau));  // true width
          if(eora && (sig%10)==1) spe += eora*0.5*(tanh((1420/frq-1420/feor)/(wid*1420/(feor*feor)))+1.0);
          if(adnoise) {spe += adnoise*1e-3*gauss(); if(frq>=fstart && frq<=fstop) wt=1;}
          if(resid) spec[i+j*NDATA] = spe - mde; else spec[i+j*NDATA] = spe;
          wtt[i+j*NDATA] = wt;
      ...
      fclose(file3);
      nplot=j+1;
      if(half && !alt) for(j=0;j<nplot;j++) if(j < nplot/2) typ[j] = 0; else typ[j] = 1; 
      if(half && alt) for(j=0;j<nplot;j++) if(j < nplot/2) typ[j] = 1; else typ[j] = 0; 
      maxrms=-1e99; j3=0; avrms=0;
      for(j=0;j<nplot;j++) {
      for(i=0;i<nn[j];i++) {spdiff[i] = spec[i+j*NDATA];
                            if(j<nplot-1) m = j+1; else m = j;
      ...
      for(i=0;i<nn[0];i++) if(rfi && (spdiff[i]-specout[i]) > rms*rfi) wtav[i] = 0;
      if(rms>maxrms) maxrms=rms;
      limm = lim;
      if(typ[j] == 0) limm = 0;
      m = 0;
      if(rms < limm && moon(timm[j],mchk) && sun(timm[j],schk) && t150 > tchk){
      for(i=0;i<nn[j];i++) {sppl[i+j3*NDATA]=spdiff[i]-specout[i]; spplm[i+j3*NDATA] = specout[i]; 
// sppl is a residual - tests show it can be a total and it doesn't effect difference 
                            wtpl[i+j3*NDATA]=wtav[i]; freqpl[i+j3*NDATA]=freq[i]; nnpl[j3]=nn[j]; t150p[j3]=t150;}
        ...
        if(title3[j*100] && tmode != 2 && tmode != 3)  sprintf(&title2[j3*100]," %s",&title3[j*100]);  // needs condx to avoid getting nline instead of date
      ...
      printf("t%d %8.3f rms %8.5f %s %d spind %10.3f ionabspercent %7.2f ionemmK %7.0f spcurv %10.3f cov %10.3f m %d nline %s\n",
             (int)f150,t150,rms,&title[j*100],typ[j],spind,ionabs,ionemm,spcurv,cov,m,&title3[j*100]);
      }
      if(!j3) {printf("no data accepted\n"); return 0;}
      for(i=0;i<nn[0];i++) dnav[i]=dnavm[i]=wtdn[i]=0;
      ndn =t150dn=0;
      for(j=0;j<j3;j++) {
        if(typp[j]==1) {t150dn += t150p[j]; ndn++;}
       for(i=0;i<nn[j];i++) {
        if(typp[j]==1) {dnav[i] += wtpl[i+j*NDATA]*sppl[i+j*NDATA]; wtdn[i] += wtpl[i+j*NDATA]; dnavm[i] += spplm[i+j*NDATA]; } 
      }}
      for(i=0;i<nn[0];i++) {if(wtdn[i])dnav[i] = dnav[i]/wtdn[i];}
//      for(i=0;i<nn[0];i++) spdiff[i]= dnav[i];  
      for(i=0;i<nn[0];i++) spdiff[i]= dnav[i] + dnavm[i]/ndn;  
      for(i=0;i<nn[0];i++) {if(wtdn[i] < ndn/3) wtav[i]=0; else wtav[i]=1;}
//      for(i=0;i<nn[0];i++) wtav[i]=1;
      for(i=0;i<nn[0];i++) {if(wtdn[i]==0) wtav[i]=0;}
      if(diff > 0) for(i=0;i<nn[0];i++) {spdiff[i] = (sppl[i+NDATA] - diff*sppl[i])/(1.0-diff);
// note correction for amplitude as per memo 247
                                         if(diff == 1) spdiff[i] = (sppl[i+NDATA] - sppl[i]);
//      if(diff > 0) for(i=0;i<nn[0];i++) {spdiff[i] = (sppl[i+NDATA] + spplm[i+NDATA] - diff*(sppl[i]+spplm[i]))/(1.0-diff); // tested made no diff
//      if(diff > 0) for(i=0;i<nn[0];i++) {spdiff[i] = 4000.0*(sppl[i+NDATA] + spplm[i+NDATA])/(sppl[i]+spplm[i]); // ratio test
//            if(i==nn[0]/2) printf("rmsin sp %f %f\n",spdiff[i],spplm[i+NDATA]);
                                          if(wtpl[i+NDATA] && wtpl[i]) wtdn[i] = 1; else wtdn[i] = 1;
                                        }
        if(rfi) {
          fition(nn[0],7,freq,spdiff,wtav,specout,&t150,&spind,&spcurv,&ionabs,&ionemm,&speor,&cov,feor,wid,10,0,0,tau);  // rfi filt with 7 term poly
          rms = rmscalc(nn[0],spdiff,specout,wtav);
          for(i=0;i<nn[0];i++) if((spdiff[i]-specout[i]) > rms*rfi) wtav[i] = 0;
        }
        if(nfit > 0 ) fition(nn[0],nfit,freq,spdiff,wtav,specout,&t150,&spind,&spcurv,&ionabs,&ionemm,&speor,&cov,feor,wid,sig,imode,t150dn/(ndn+1e-6),0);
        else fition(nn[0],-abs(fabs(nfit)+1),freq,spdiff,wtav,specout,&t150,&spind,&spcurv,&ionabs,&ionemm,&speor,&cov,feor,wid,sig,0,0,tau);
        rms = rmscalc(nn[0],spdiff,specout,wtav);
...
//        sprintf(&info[j3*100],"rms %4.0f %4.0f/div mK",rms*1e3,2e3*dmax/sqrt(j3));
      if(sm) {for(i=0;i<nn[0];i++) spdiff[i] = spdiff[i]-specout[i];
              fition(nn[0],sm,freq,spdiff,wtav,specout,&t150,&spind,&spcurv,&ionabs,&ionemm,&speor,&cov,feor,wid,sig,imode,t150dn/(ndn+1e-6),0); 
              for(i=0;i<nn[0];i++) {spdiff[i] = specout[i]; specout[i]=0;}
              rms = rmscalc(nn[0],spdiff,specout,wtav);
              sprintf(&info[j3*100],"rms %3.1e scale x %d",rms,g10);
             }
      for(i=0;i<nn[0];i++) {sppl[i+j3*NDATA]=spdiff[i]-specout[i]; wtpl[i+j3*NDATA]=wtav[i]; freqpl[i+j3*NDATA]=freq[i];}
//      if(imode) for(i=0;i<nn[0];i++) sppl[i+j3*NDATA]=spdiff[i]-specout[i];  // for plot
      ...
      printf("t%d %7.3f maxrms %5.1f avrms %6.4f rmsofav %6.4f K j3 %d %s t150dn %6.4f spind %6.4f ionabs %6.4f ionemm %6.4f spcurv %6.4f eor_K %6.4f snr %f\n",
              (int)feor,t150,maxrms,avrms/(j3-1.0),rms,j3,&title4[(j3-2)*100],t150dn/(ndn+1e-6),spind,ionabs,ionemm,spcurv,speor,snr);
...
void plotfspec(int nn[],double freqq[], double data[], double wt[], int nplot,double ddmax,char title[],char info[],int g10,double avrms, int pmode, char txxt[])
  // plot the spectrum 
{
    char txt[256];
    ...
    if(pmode == 1) {nnplot = nplot - 1; dd = 480.0;} else {nnplot = nplot; dd = 440.0;}
    ...
    for (k = 0; k < np; k++) {
        ...
        if(k==0 || x < xp) {xp=x;yp=y;}
          ...
          totpp = -dmin / scale;
          if(iter<nplot-1) ypp = 480.0 - dd/nnplot - iter*dd/nnplot + totpp * dd/nnplot;
          else ypp = 20;
...
//        sprintf(txt, "%5.1f", f);
    ...
    for (f = dmin; f <=dmax; f += (dmax - dmin)) {
        ...
        x = 0;
        y = (f-dmin) / (dmax-dmin);
        if(iter<nplot-1){ y = 480.0 -dd/nnplot - iter*dd/nnplot + y*dd/nnplot;
//        else y = 20;
        fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n 0 0 0 sethsbcolor stroke\n",
...
//           {sprintf(txt, "%4.0f K", f); fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",x + xoffset - 45.0,y - 1.0+yoffset, txt);}
    ...
    for (f = fstart; f <= fstop+step*0.1; f += step) {
        ...
        y = 0;
        fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n 0 0 0 sethsbcolor stroke\n",
         x + xoffset,y+ z -0.5*dd/nplot + yoffset,x + xoffset,y + z +0.5*dd/nplot+yoffset);
        sprintf(txt, "%4.0f", f);
        fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",x + xoffset - 15.0,y - 15.0+yoffset, txt);
    }
    ...
    for(iter=0;iter<5;iter++){
        ...
        fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n 0 0 0 sethsbcolor stroke\n",
           x + xoffset,y+yoffset,x + xoffset + 400.0,y+yoffset);
    }
    fprintf(file, "1 setlinewidth\n");
    }
/*
  ...
  // plot the spectrum 
{
    ...
    for (f = fstart; f <= fstop+step*0.1; f += step) {
        ...
        y = 0;
        fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n 0 0 0 sethsbcolor stroke\n",
         x + xoffset,y+yoffset,x + xoffset,y + 480.0+yoffset);
        sprintf(txt, "%4.0f", f);
        fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",x + xoffset - 15.0,y - 15.0+yoffset, txt);
    }
    ...
    for (f = dmin; f <=dmax; f += (dmax - dmin)*0.05) {
        ...
        fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n 0 0 0 sethsbcolor stroke\n",
           x + xoffset,y+yoffset,x + xoffset + 400.0,y+yoffset);
         if(dmax< 10) sprintf(txt, "%3.1f", f); else sprintf(txt, "%3.0f", f);
...
double fition(int np,int mode,double freqq[], double data[], double wt[],double dataout[],double *t150,double *spind,double *spcurv, double *ionabs, double *ionemm, double *speor, double *cov, double feor, double wid, int sig, int imode, double tt, double tau)
{
    int k, i, j, nfit;
    double freq,spi,f150,f;
    static double wtt[32768];

    for(j=0;j<np;j++) if(wt[j]) wtt[j] = 1; else wtt[j] = 0;
     nfit = abs(mode);
     spi = 2.55;
     if((freqq[0] + freqq[np-1])/2.0 > 100.0) f150 = 150; else f150 = 75;
...
//    if((nfit>7 && mode>0) || (nfit>8 && mode<0))  fitf[k] = pow(freq,-spi-2 + i);  // not quite as good for updn  best for dn only
    ...
    if(mode < 0 && i && i==nfit-1 && (sig%10) == 0){ if(tau==0) fitf[k] = -exp(-0.693*(freqq[j]-feor)*(freqq[j]-feor)/(wid*wid*0.25));  // negative for absorption
                                                     else fitf[k] = -(1-exp(-tau*exp(-(freqq[j]-feor)*(freqq[j]-feor)*(-log(-log((1+exp(-tau))/2)/tau))/(wid*wid*0.25))))/(1-exp(-tau));  // true width
                                                   }
    if(mode < 0 && i && i==nfit-1 && (sig%10) == 1) fitf[k] = 0.5*(tanh((1420/freqq[j]-1420/feor)/(wid*1420/(feor*feor)))+1.0);
...
//    if(mode < 0 && i && i==nfit-1 && sig == 1) fitf[k] = sqrt((1420/freqq[j])/10.0)*0.5*(tanh((1420/freqq[j]-1420/feor)/(wid*1420/(feor*feor)))+1.0);
    }}
     if(sig>=30) for(j=0;j<np;j++) data[j] = log(data[j]);
     if(sig>=30) for(j=0;j<np;j++) if(wtt[j] > 0) wtt[j] = data[j]*data[j];  // to make equivalent to fitting without using non linear log(u+a)~log(u)+a/u
     polyfitr(nfit,np,data,mcalc,wtt,dataout,fitf,aarr,bbrr);
     if(sig>=30) for(j=0;j<np;j++) if(wtt[j] > 0) wtt[j] = 1;
     if(sig>=30) for(j=0;j<np;j++) {data[j] = exp(data[j]);  dataout[j] = exp(dataout[j]);}
    *t150=0;
    for(i=0;i<nfit;i++){
    freq = 1;
  if(sig<10){
   ...
   if(sig>=30 && sig<40 && mode < 1) {f = (exp(bbrr[0]+bbrr[1]*log(feor/f150)))/pow(feor/f150,-spi); bbrr[nfit-1] = f*bbrr[nfit-1]; aarr[(nfit-1)+(nfit-1)*nfit]=sqrt(f)*aarr[(nfit-1)+(nfit-1)*nfit];} // approx. 
//     *t150=bbrr[0]; 
    *spind= -spi + bbrr[1]/bbrr[0]; *ionabs=-1e2*bbrr[2]/bbrr[0]; *ionemm=-bbrr[3]*bbrr[0]/bbrr[2]; *spcurv=bbrr[4]/bbrr[0]; *speor=bbrr[nfit-1]; *cov=aarr[(nfit-1)+(nfit-1)*nfit];
    if(nfit < 3 || (mode < 0 && nfit < 5)) *ionemm=0;
    if(imode) {*spind = *spcurv = *speor = 0; *cov = 1; *ionabs=-1e2*bbrr[0]/tt; *ionemm=-tt*bbrr[1]/bbrr[0];}  // tt comes from t150dn 
...
void eorsnr(int np,double freqq[], double data[], double wt[],double dataout[],double fstart,double fstop,double wid, int sig, int nfit,double seor,double tau,int mode,double fsmax,double fsmin,char info4[])
{ double feor,t150,spind,spcurv,ionabs,ionemm,speor,cov,rms,snr,widd,widmax,widmaxx,snrmax,snrmaxx,maxfreq,sigmax,sigmaxx,rmsin,rmss,rmsss,wwid;
 ...
 for(i=0;i<np;i++) dataout1[i] = data[i] - dataout[i];
  if(fstop > 150) wwid = 60; else wwid = 30;
 ...
 for(i=0;i<np;i++){
   ...
   if(seor == -2) {if(speor > 0) snr = 1.0/rms; else snr = 0;}  // best fit with correct sign
    if(wtt[i] && snr > snrmax) {snrmax = snr; widmax = widd; sigmax = speor; rmss = rms;}
    } 
    }
   if(snrmax > snrmaxx) {snrmaxx = snrmax; widmaxx = widmax; sigmaxx = sigmax; maxfreq = feor; rmsss = rmss;}  
   snrr[i] = snrmax;
...
// for(j=0;j<np;j++) dataout2[j] = data[j] - dataout2[j];
 ...
 if(mode) {
  for(j=0;j<np;j++){ if(tau) datasig[j] = -sigmaxx*(1-exp(-tau*exp(-(freqq[j]-maxfreq)*(freqq[j]-maxfreq)*(-log(-log((1+exp(-tau))/2)/tau))/(widmaxx*widmaxx*0.25))))/(1-exp(-tau));
   ...
   else zplott(np,freqq,dataout1,dataout2,snrr,wt,nfit,info);
}
...
int chisolve(int np, double freqq[], double data[], double wt[], double dataout[], int sig, int nfit,double fstart, double fstop, double fsmax,double fsmin)
{ double tau,aasig,ffsig,wwidsig,ttau,rmsfit,dfreq,damp,dwid,dtau,snr;
  ...
  char rslts[256];
   ...
   tau = 7; dfreq=0.1; damp=0.02; dwid = 0.2; dtau = 0.5; 
   if(sig==0) phy=2; else phy=0;
   ...
   for(n=1;n<=4;n++) chifit(n,np,freqq,data,wt,dataout,nfit,tau,&aasig,&ffsig,&wwidsig,&ttau,&rmsfit,phy,fsmax,fsmin,map,dfreq,damp,dwid,dtau); 

   printf("amp down\n");
   for(j=0;j<16;j++)
                   { for(i=0;i<16;i++) {
                min = 99; {for(k=0;k<16;k++) for(l=0;l<16;l++) {m = i+j*16+k*16*16+l*16*16*16; if(map[m]<min) min=map[m];}} printf("%d",min/10);
                                       }
               printf("\n");
                   }
   ...
   for(k=0;k<16;k++)
                   { for(i=0;i<16;i++) {
                min = 99; {for(j=0;j<16;j++) for(l=0;l<16;l++) {m = i+j*16+k*16*16+l*16*16*16; if(map[m]<min) min=map[m];}} printf("%d",min/10);
                                       }
               printf("\n");
                   }
   printf("frequency across\n");
   ...
   for(i=0;l<16;l++)
                   { for(i=0;i<16;i++) {
                min = 99; {for(j=0;j<16;j++) for(k=0;k<16;k++) {m = i+j*16+k*16*16+l*16*16*16; if(map[m]<min) min=map[m];}} printf("%d",min/10);
                                       }
               printf("\n");
                   }
   printf("frequency across\n");
  ...
  printf("3\n");
   ...
   for(k=0;k<16;k++)
                   { for(j=0;j<16;j++) {
                min = 99; {for(i=0;i<16;i++) for(l=0;l<16;l++) {m = i+j*16+k*16*16+l*16*16*16; if(map[m]<min) min=map[m];}} printf("%d",min/10);
                                       }
               printf("\n");
                   }
   ...
   for(l=0;l<16;l++)
                   { for(j=0;j<16;j++) {
                min = 99; {for(i=0;i<16;i++) for(k=0;k<16;k++) {m = i+j*16+k*16*16+l*16*16*16; if(map[m]<min) min=map[m];}} printf("%d",min/10);
                                       }
               printf("\n");
                   }
   printf("amplitude across\n");
  ...
  printf("5\n");
   ...
   for(l=0;l<16;l++)
                   { for(k=0;k<16;k++) {
                min = 99; {for(i=0;i<16;i++) for(j=0;j<16;j++) {m = i+j*16+k*16*16+l*16*16*16; if(map[m]<min) min=map[m];}} printf("%d",min/10);
                                       }
               printf("\n");
                   }
   printf("width across\n");
   ...
   printf("6\n");
  sprintf(rslts,"fcen %5.2f amp %5.2f wid %5.2f tau %5.2f nterms %d phy %d fstart %5.2f fstop %5.2f fsmin %5.2f fsmax %5.2f snr %5.2f",
     ffsig,aasig,wwidsig,ttau,nfit,phy,fstart,fstop,fsmin,fsmax,snr);
  plotq(map,ffsig,dfreq,aasig,damp,wwidsig,dwid,ttau,dtau,rslts);
   return 0;
}

double chifit(int mode,int np,double freqq[],double data[],double wt[],double dataout[],int nfit,double tau, double *aasig, double *ffsig,double *wwidsig,double *ttau,double *rmsfit,int phy,double fsmax,double fsmin,unsigned char map[],double dfreq, double damp, double dwid, double dtau)
{ double feor,speor,cov,rms,snr,widd,widmax,widmaxx,snrmax,snrmaxx,maxfreq,sigmax,sigmaxx,rmsin,rmss,rmsss,tttau,tau1,tau2,taumax,taumaxx;
  double wtt[NDATA],data1[NDATA],sig,chi,chi0,nn;
  int i,j,k;
 rmss = 0;  tau1=1; tau2=tau+5;
 nn = 0; for(i=0;i<np;i++) nn += wt[i];
 if(mode==0){
...
//   snr = speor/(sqrt(cov)*rms);
   snr = 1.0/rms;
    if(snr > snrmax) {snrmax = snr; widmax = widd; sigmax = speor; taumax = tttau; rmss = rms;}
    } 
    }
   printf("freq %f wtt %f snrmax %f sigmax %f widmax %f taumax %f\n",feor,wtt[i],snrmax,sigmax,widmax,taumax);
   if(snrmax > snrmaxx) {snrmaxx = snrmax; widmaxx = widmax; sigmaxx = sigmax; maxfreq = feor; taumaxx = taumax; rmsss = rmss;}
  }
  fit(np,phy+1,nfit,freqq,data,wtt,dataout,&speor,&cov,maxfreq,widmaxx,taumaxx);
   rms = rmscalc(np,data,dataout,wtt);
   snr = speor/(sqrt(cov)*rms);
   printf("maxfreq %5.1f snrmax %5.1f sigmax %5.2f widmax %5.1f taumax %5.1f rmsin %6.4f rms %6.4f snr %5.1f\n",maxfreq,snrmaxx,sigmaxx,widmaxx,taumaxx,rmsin,rmsss,snr);
   *aasig = sigmaxx; *ffsig = maxfreq; *wwidsig = widmaxx; *ttau = taumaxx; *rmsfit = rmsss;
 ...
 snrmax=0;
  for(feor=*ffsig-1;feor<=*ffsig+1;feor+=dfreq){
   ...
   for(k=0;k<np;k++)data1[k] = data[k] +sig*(1-exp(-tttau*exp(-(freqq[k]-feor)*(freqq[k]-feor)*(-log(-log((1+exp(-tttau))/2)/tttau))/(widd*widd*0.25))))/(1-exp(-tttau));
   fit(np,phy,nfit,freqq,data1,wtt,dataout,&speor,&cov,feor,widd,tttau);
   rms = rmscalc(np,data1,dataout,wtt);
   mapp(map,feor,*ffsig,dfreq,sig,*aasig,damp,widd,*wwidsig,dwid,tttau,*ttau,dtau,nn*((rms/(*rmsfit))*(rms/(*rmsfit))-1));
   snr = 1.0/rms;
   if(snr > snrmax) {snrmax = snr; rmss = rms;}
    }
  }
  }
   chi0 = 1;
 ...
 for(feor=*ffsig-1;feor<=*ffsig+1;feor+=dfreq){
   ...
   fit(np,phy+1,nfit,freqq,data,wtt,dataout,&speor,&cov,feor,widd,tttau);
   rms = rmscalc(np,data,dataout,wtt);
   mapp(map,feor,*ffsig,dfreq,speor,*aasig,damp,widd,*wwidsig,dwid,tttau,*ttau,dtau,nn*((rms/(*rmsfit))*(rms/(*rmsfit))-1));
   snr = 1.0/rms;
      if(snr > snrmax) {snrmax = snr; rmss = rms;}
  }
  }
...
void mapp(unsigned char map[],double feor,double feor0,double dfeor,double sig,double sig0, double dsig,double wid,double wid0,double dwid,double tau,double tau0,double dtau,double chi)
{int i,j,k,l,m,n;
 i = (feor-feor0)/dfeor+8.5;     if(i<0) i=0; if(i>15) i=15;
 j = (sig-sig0)/dsig+8.5;        if(j<0) j=0; if(j>15) j=15;
 k = (wid-wid0)/dwid+8.5;        if(k<0) k=0; if(k>15) k=15;
 l = (tau-tau0)/dtau+8.5;        if(l<0) l=0; if(l>15) l=15;
 m = i+j*16+k*16*16+l*16*16*16;
 n = 10.0*chi; if(n<0) n = 0; if(n>255) n = 255;
 map[m] = n;
}

void fit(int np,int mode,int nfit, double freqq[], double data[], double wt[], double dataout[], double*speor, double*cov,double feor,double wid,double tau)
{
    int k, i, j, mfit;
    double freq;
    static double wtt[32768];
    ...
    if((mode&1) && i==mfit-1)fitf[k] = -(1-exp(-tau*exp(-(freqq[j]-feor)*(freqq[j]-feor)*(-log(-log((1+exp(-tau))/2)/tau))/(wid*wid*0.25))))/(1-exp(-tau));  // true width

    }
   }
     polyfitr(mfit,np,data,mcalc,wtt,dataout,fitf,aarr,bbrr);
    *speor=bbrr[mfit-1]; *cov=aarr[(mfit-1)+(mfit-1)*mfit];
}
...
//   writeout spectrum  
{
    ...
    np=nn[nplot-1];
    for (k = 0; k < np; k++) {
    if(out == 1) fprintf(file,"freq %8.4f resid %8.4f K wt %8.4f\n",freqq[k+(nplot-1)*NDATA],data[k+(nplot-1)*NDATA],wt[k+(nplot-1)*NDATA]); 
    if(out == 2) fprintf(file,"freq %10.6f tantenna %10.6f K skymodel %10.6f K wt %1.0f %s\n", freqq[k+(nplot-1)*NDATA],data[k+(nplot-1)*NDATA],0.0,wt[k+(nplot-1)*NDATA],title);
    if(out == 3) fprintf(file,"freq %10.6f tantenna %10.6f K skymodel %10.6f K resid %10.6f K wt %1.0f %s\n", 
                  freqq[k+(nplot-1)*NDATA],data2[k],data3[k],data[k+(nplot-1)*NDATA],wt[k+(nplot-1)*NDATA],title);
    if(out == 4) fprintf(file,"freq %10.6f tantenna %10.6f K skymodel %10.6f K wt %1.0f %s\n", freqq[k+(nplot-1)*NDATA],data2[k],0.0,wt[k+(nplot-1)*NDATA],title);
    }
    fclose(file);
}
...
void zplott(int np,double freqq[], double datain[], double data[], double snr[], double wt[],int nfit,char info[])
{
    ...
    rms=m=0;
    for(k=0;k<np;k++) {rms += wt[k]*data[k]*data[k]; m += wt[k];}
    rms = sqrt(rms/m);
    rmsin=m=0;
    for(k=0;k<np;k++) {rmsin += wt[k]*datain[k]*datain[k]; m += wt[k];}
    rmsin = sqrt(rmsin/m);
    ...
    xoffset = 60.0;
    yoffset = 100.0;
    if(freqq[np/2] > 100) {dmin = -100e-3; dmax = 100e-3;}
    else {dmax = 200e-3;  dmin = -200e-3;}
    scale = dmax-dmin;
    for (y = 0; y < 2; y++) 
    fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n 0 0 0 sethsbcolor stroke\n",
             xoffset, y * 480 + yoffset, xoffset + 400.0, y * 480 + yoffset);
...
//        sprintf(txt, "%5.1f", f);
    ...
    for (z = 7; z <= 50; z += 1) {
        ...
        y = 0;
        fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n 0 0 0 sethsbcolor stroke\n",
         x + xoffset,y+yoffset + 480,x + xoffset,y - 5.0+yoffset + 480);
                sprintf(txt, "%4.0f", z);
...
//           x + xoffset,y-240.0/2+yoffset,x + xoffset + 400,y-240.0/2+yoffset);
           ...
           {sprintf(txt,"%4.0f",f*1e3); fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",x + xoffset - 28.0,y - 1.0+yoffset, txt);}
    } 
    }
      fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n 0 0 0 sethsbcolor stroke\n",
                 x + xoffset,240.0+yoffset,x + xoffset + 400,240.0+yoffset);
...
void zplott2(int np,double freqq[], double datain[], double data[], double datasig[], double snr[], double wt[],int nfit,char info[],char info4[])
{
    char txt[256];
    int k, m,iter;
    double x, y, xp, yp, dmax, dmin, f, totpp, scale, step, h, s, b, z;
    ...
    xoffset = 60.0;
    yoffset = 100.0;
    if(freqq[np/2] > 100 && freqq[0] > 80) {dmin = -100e-3; dmax = 100e-3;}
    else {dmax = 400e-3;  dmin = -400e-3;}
    scale = dmax-dmin;
    for (y = 0; y < 2; y++) 
    fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n 0 0 0 sethsbcolor stroke\n",
             xoffset, y * 480 + yoffset, xoffset + 400.0, y * 480 + yoffset);
...
//        sprintf(txt, "%5.1f", f);
    ...
    for (z = 7; z <= 50; z += 1) {
        ...
        fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n 0 0 0 sethsbcolor stroke\n",
         x + xoffset,y+yoffset + 480,x + xoffset,y - 5.0+yoffset + 480);
              if(z <= 22 || ((int)z)%2==0) { sprintf(txt, "%4.0f", z);
                        fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",x + xoffset - 15.0,y - 15.0+yoffset + 480 + 20, txt);}
...
//           x + xoffset,y-240.0/2+yoffset,x + xoffset + 400,y-240.0/2+yoffset);
         ...
         if(iter<2)  {sprintf(txt,"%4.0f",f*1e3); fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",x + xoffset - 28.0,y - 1.0+yoffset, txt);}
         else  {sprintf(txt,"%4.0f",f*1e3-0*dmax*1e3); fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",x + xoffset - 28.0,y - 1.0+yoffset, txt);}
    } 
    }
      fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n 0 0 0 sethsbcolor stroke\n",
                 x + xoffset,240.0+yoffset,x + xoffset + 400,240.0+yoffset);
...
//    time_t now;
   ...
   for(j=0;j<16;j++)
                   { for(i=0;i<16;i++) {
                min = 99; {for(k=0;k<16;k++) for(l=0;l<16;l++) {m = i+j*16+k*16*16+l*16*16*16; if(map[m]<min) min=map[m];}} 
   mapp[i][j] = min/10.0;
                                       }
                   }
    printf("frequency across\n");
   ...
   for(k=0;k<16;k++)
                   { for(i=0;i<16;i++) {
                min = 99; {for(j=0;j<16;j++) for(l=0;l<16;l++) {m = i+j*16+k*16*16+l*16*16*16; if(map[m]<min) min=map[m];}}
   mapp[i][k] = min/10.0;
                                       }
                   }
   printf("frequency across\n");
   ...
   for(l=0;l<16;l++)
                   { for(i=0;i<16;i++) {
                min = 99; {for(j=0;j<16;j++) for(k=0;k<16;k++) {m = i+j*16+k*16*16+l*16*16*16; if(map[m]<min) min=map[m];}}
   mapp[i][l] = min/10.0;
                                       }
                   }
   printf("frequency across\n");
   ...
   for(k=0;k<16;k++)
                   { for(j=0;j<16;j++) {
                min = 99; {for(i=0;i<16;i++) for(l=0;l<16;l++) {m = i+j*16+k*16*16+l*16*16*16; if(map[m]<min) min=map[m];}}
   mapp[j][k] = min/10.0;
                                       }
                   }
   printf("frequency across\n");
   ...
   for(l=0;l<16;l++)
                   { for(j=0;j<16;j++) {
                min = 99; {for(i=0;i<16;i++) for(k=0;k<16;k++) {m = i+j*16+k*16*16+l*16*16*16; if(map[m]<min) min=map[m];}}
   mapp[j][l] = min/10.0;
                                       }
                   }
   printf("frequency across\n");
   ...
   for(l=0;l<16;l++)
                   { for(k=0;k<16;k++) {
                min = 99; {for(i=0;i<16;i++) for(j=0;j<16;j++) {m = i+j*16+k*16*16+l*16*16*16; if(map[m]<min) min=map[m];}}
   mapp[k][l] = min/10.0;
                                       }
                   }
   printf("frequency across\n");
...
//    now = time(NULL);
    ...
    max=0; minn=1e99; sm=16;  mm = 0;
for(xx=0;xx<128;xx+=1){
for(yy=0;yy<128;yy+=1){
 w = ww = 0;
 for(i=0;i<16;i++){
 for(j=0;j<16;j++){
  dx = (xx+mm-i*8); dy = (yy+mm-j*8);
...
//  ww += 1.0/dd; 
  ...
  if(ampp<minn) minn=ampp;
    ...
    printf("max %f min %f\n",max,minn);
    rlev=3;
if(mode==1) { fprintf(file2,"%d %d  moveto\n (%4.1f) show\n",xoff+60,yoff-14,fcen);
...
if(mode==2) { fprintf(file2,"%d %d  moveto\n (%4.1f) show\n",xoff+60,yoff-14,fcen);
              fprintf(file2,"%d %d  moveto\n (%4.1f) show\n",xoff+60-64,yoff-14,fcen-8*dfcen);
              fprintf(file2,"%d %d  moveto\n (%4.1f) show\n",xoff+60+64,yoff-14,fcen+8*dfcen);
              fprintf(file2,"%d %d  moveto\n (freq(MHz)) show\n",xoff+50,yoff-24);
...
if(mode==3) { fprintf(file2,"%d %d  moveto\n (%4.1f) show\n",xoff+60,yoff-14,fcen);
              fprintf(file2,"%d %d  moveto\n (%4.1f) show\n",xoff+60-64,yoff-14,fcen-8*dfcen);
              fprintf(file2,"%d %d  moveto\n (%4.1f) show\n",xoff+60+64,yoff-14,fcen+8*dfcen);
              fprintf(file2,"%d %d  moveto\n (freq(MHz)) show\n",xoff+50,yoff-24);
...
if(mode==4) { fprintf(file2,"%d %d  moveto\n (%4.2f) show\n",xoff+60,yoff-14,amp);
...
if(mode==5) { fprintf(file2,"%d %d  moveto\n (%4.2f) show\n",xoff+60,yoff-14,amp);
              fprintf(file2,"%d %d  moveto\n (%4.2f) show\n",xoff+60-64,yoff-14,amp-8*damp);
              fprintf(file2,"%d %d  moveto\n (%4.2f) show\n",xoff+60+64,yoff-14,amp+8*damp);
              fprintf(file2,"%d %d  moveto\n (amp) show\n",xoff+60,yoff-24);
              fprintf(file2,"%d %d  moveto\n (%4.1f) show\n",xoff-25,yoff+64+64,tau+8*dtau);
...
if(mode==6) { fprintf(file2,"%d %d  moveto\n (%4.1f) show\n",xoff+60,yoff-14,wid);
...
for(yy=0;yy<128;yy+=1){
 ...
 for(j=0;j<16;j++){
  ...
  ampp = (w/ww)/max;
     ...
     h=ampp; s=b=1.0; 
     gray=ampp;
     if(ampp>0.8){ h=0; s=0; b=1;}
     h=0.67*floor(h*rlev)/rlev;
     if(h<0.2) h=0;
     gray=floor(gray*rlev)/rlev;
     if (bmode == 0 && ampp > 0.8*0) fprintf(file2,
...
for(side=0;side<4;side++){
  if(side==0){xx1=xx;yy1=yy;xx2=xx+1;yy2=yy;}
  if(side==1){xx1=xx+1;yy1=yy;xx2=xx+1;yy2=yy+1;}
  if(side==2){xx1=xx;yy1=yy+1;xx2=xx+1;yy2=yy+1;}
  if(side==3){xx1=xx;yy1=yy;xx2=xx;yy2=yy+1;}
   w = ww = 0;
 for(i=0;i<16;i++){
 for(j=0;j<16;j++){
  dx = (xx2+mm-i*8); dy = (yy2+mm-j*8);
  dd = dx*dx+dy*dy;
  dd = exp((-dx*dx-dy*dy)/sm);
  w += mapp[i][j]*dd;
  ww += dd;
 ...
 for(j=0;j<16;j++){
  ...
  ampp = (w/ww)/max;
     ...
     k1=a1;k2=a2;
     if(k2!=k1){ n++;
      if(a2>a1) d=(k2-a1)/(a2-a1);
       else d=(a1-k1)/(a1-a2);
     if(side==0){ x1=xx1+d; y1=yy1;}
     if(side==1 && n==1){ x1=xx1; y1=yy1+d;}
     if(side==1 && n==2){ x2=xx1; y2=yy1+d;}
     if(side==2 && n==1){ x1=xx1+d; y1=yy1;}
     if(side==2 && n==2){ x2=xx1+d; y2=yy1;}
     if(side==3){ x2=xx1; y2=yy1+d;}
   }
   }
