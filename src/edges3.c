...
#include <fcntl.h>
#include <complex.h>
// SGM: changed precision of PI to match numpy better
//#ifdef PIFIX
...
void plotvna(int,double*,complex double*,double*,int,double*,complex double*,double*,double*, int,double,double,double,int,char*);
...
double antloss(double,int);
double lossinv(double,double,double);
double beamcorr(double,int,double, double, double*,double*,long double*,double*,int,int,double*,double*,double*,int,double*,double,double,int);
double gst(double);
...
int main(int argc, char *argv[])
{
    ...
    double spe_ind,spe_inderr,gamma,gammaerr,ion_abs,ion_em,ssun,sunind,ghav,ghamax,ghamin,ion,tau,noise,opn,fbstart,fbstop;
    ...
    int i,j,k,kk,nant,nopen,nshort,nhot,namb,mode,ns11ant,ns11lna,ns11hot,ns11amb,ns11cab1,ns11cab2,ns11rig,nfit,nfit1,imp,db,nfit2,nfit3,nfit4,iter,mfit,cfit,wfit,skymode,yr,dy,hr,mn,sc,smooth,cal,wtmode,lmode,cmb,nline,site;
    ...
    mode = 0; nant=nopen=nshort=nhot=namb=ns11ant=ns11lna=ns11hot=ns11amb=ns11cab1=ns11cab2=ns11rig=0; wtmode=0; cons=0; nocal=0; low=0; sim=ion=tau=noise=rr=0;
    ...
    wfstart = 50; wfstop = 200; imp=0; db=0; secs=0; yr=yrh=yrc=yro=yrs=dy=hr=mn=sc=cmb=nline=0; fstart = 50; fstop = 200; fbstart = 0; site = 0;
    ...
    if (strstr(buf, "-nter")) {sscanf(argv[i+1], "%d",&nter);} // cal fitting default=8
...
//      for(j=0;j<ns11lna;j++) if(wtmode==0 && (freqs11lna[j] < wfstart || freqs11lna[j] > wfstop)) wttlna[j]=0;
      for(j=0;j<ns11lna;j++) s11lna[j] = s11lna[j]*cexp(-2.0*PI*freqs11lna[j]*1e6*((delaylna+delaycorr)*I))*pow(10.0,0.05*(dbcorr+ldb));  // corrections for adapters
      for(j=0;j<ns11ant;j++) {if(opn) s11ant[j] = opn; 
                              else s11ant[j] = s11ant[j]*cexp(-2.0*PI*freqs11ant[j]*1e6*(delaycorr+delayant)*I)*pow(10.0,0.05*(dbcorr+adb));
                             }
      ...
      if(skymode >= 0) bfit = beamcorr(0,bfit,antaz,secs,mcalc,fitf,aarr,bbrr,skymode,binteg,&lst,bb,&bspac,cmb,&freqref,fbstart,fbstop,site); // fit polynomial
...
// 27 or higher for antenna
    ...
    fprintf(s11file, "# freq, amb_real amb_imag hot_real hot_imag open_real open_imag short_real short_imag lna_real lna_imag\n");
    for(i=0;i<namb;i++){
      fprintf(s11file, "%1.12e %1.12e %1.12e %1.12e %1.12e %1.12e %1.12e %1.12e %1.12e %1.12e %1.12e\n", 
        freqhot[i],  
...
//    for(i=0;i<namb;i++) Tll[i]=Tll2[i]=s11lna[i]; // test of fitting
      wavefit(
        nopen,nshort,wfit,sspopen,wtcal,freqopen,ss11cab,ss11lna,tcab,tload,tamb,
        tlna0,tlna1,tlna2,dataout,mcalc,fitf,aarr,bbrr,&delayln
      );
      printf("NW DELAY: %1.3e", delayln);
   }
...
//   for(i=0;i<namb;i++) printf("freq %f tcal_ofs %f tcal_ofssm %f diff %f wtcal %f\n",freqamb[i],tcal_ofs[i],tcal_ofssm[i],tcal_ofs[i]-tcal_ofssm[i],wtcal[i]);
    ...
    // SGM ADDITION: write out calibrated temperatures.
    FILE *calspec_file;
    calspec_file = fopen("calibrated_open.txt","w");
    fprintf(calspec_file, "# freq, uncal T[mK], calib T[mK]\n");
    for(i=0;i < namb; i++){
        sspopen[i] = (spopen[i]-tamb)*tcal_scasm[i] + tamb - tcal_ofssm[i];
        fprintf(calspec_file, "%1.16e %1.16e %1.16e\n", freqopen[i], spopen[i], w3pinv(ss11cab[i],ss11lna[i],sspopen[i],tload,tamb,tlna0[i],tlna1[i],tlna2[i]));
    }
    fclose(calspec_file);

    calspec_file = fopen("calibrated_short.txt","w");
    fprintf(calspec_file, "# freq, uncal T[mK], calib T[mK]\n");
    for(i=0;i < namb; i++){
        sspopen[namb + i] = (spshort[i]-tamb)*tcal_scasm[i] + tamb - tcal_ofssm[i];
        fprintf(calspec_file, "%1.16e %1.16e %1.16e\n", freqopen[i], spshort[i], w3pinv(ss11cab[namb+i],ss11lna[i],sspopen[namb+i],tload,tamb,tlna0[i],tlna1[i],tlna2[i]));
    }
    fclose(calspec_file);

    calspec_file = fopen("calibrated_hot.txt","w");
    fprintf(calspec_file, "# freq, uncal T[mK], calib T[mK]\n");
    for(i=0;i < namb; i++){
        fprintf(calspec_file, "%1.16e %1.16e %1.16e\n", freqhot[i], sphot[i], dataout[i]);
    }
    fclose(calspec_file);
    // SGM END ADDITION 
    ...
    // SGM ADDITION: write out calibrated temperatures.
    calspec_file = fopen("calibrated_ambient.txt","w");
    fprintf(calspec_file, "# freq, uncal T[mK], calib T[mK]\n");
    for(i=0;i < namb; i++){
        fprintf(calspec_file, "%1.16e %1.16e %1.16e\n", freqamb[i], spamb[i], dataout[i]);
    }
    fclose(calspec_file);
    // SGM END ADDITION 
 ...
 if(cal==0 || nant != namb) { readcal(&ncal,freqcal,ss11lna,tcal_scasm,tcal_ofssm,tlna0,tlna1,tlna2,wtcal); 
              if(ncal==0) return 0;
                for(i=0;i<ncal;i++) tmp[i] = ss11lna[i]*cexp(-2.0*PI*freqcal[i]*1e6*(delaylna*I))*pow(10.0,0.05*ldb);
   ...
   // SGM: added for cross-checks
    FILE *ants11file;
    printf("FREQANT[0] = %.3f\n", freqant[0]);
    ants11file = fopen("ant_s11_modelled.txt", "w");
    fprintf(ants11file, "# freq real imag\n");
    for(i=0;i<nant;i++){
      fprintf(ants11file, "%1.12e %1.12e %1.12e\n", 
...
//   if(freqant[i] < wfstart || freqant[i] > wfstop || wtcal[i] == 0) wtant[i] = 0;
   ...
   if(lmode < 0) La = loss(ss11ant[i],atten+aloss*sqrt(freqant[i]/100.0));  // aloss > 0 for simple atten or cross-check
                  ...
                  else La = lossmodel(freqant[i],ss11ant[i],lmode)*antloss(freqant[i],lmode); // lmode 0=hot load 1=balun+bend 5or6=balun+bend lowband
              }
...
//        beamcorr assumes a spectral index of -2.5
    if(skymode >=0) {skymodel[i] = beamcorr(freqant[i]*(1.0+low),bfit,antaz,secs,mcalc,fitf,aarr,bbrr,0,0,&lst,bb,&bspac,cmb,&freqref,0,0,site)*pow(1.0/(1.0+low),-2.5);
//                    dataout[i] = dataout[i] - (skymodel[i] - beamcorr(150.0,bfit,antaz,secs,mcalc,fitf,aarr,bbrr,0,0,&sun,&lst,bb,bbsun)*pow(freqant[i]/150.0,-2.5));
                      dataout[i] = dataout[i]*beamcorr(freqref,bfit,antaz,secs,mcalc,fitf,aarr,bbrr,0,0,&lst,bb,&bspac,cmb,&freqref,0,0,site)*pow(freqant[i]/freqref,-2.5)/skymodel[i];
      }
    if(test&1 && skymode >=0  && skymode>=256)  dataout[i] = skymodel[i];  // substitute  beamcorrection 
   ...
   if(sim) {
   for(i=0;i<nant;i++) {
                  if(test&1 && skymode >=0 && skymode>=256) {
...
//       if(i==11) fitf[k] = 1.0 - loss(ss11ant[j],0,0,0,0,freqant[i],dlyrob,lmode); // some added instrumental functions will be needed for ant. s11 structure
    ...
    if(smooth) {
                 dsmooth(nant,dataout,dataout2,wtant,mcalc,abs(smooth));
                 if(smooth < 0) {
                   j = 0; kk = abs(smooth);
                   for(i=0; i<nant; i+=kk) {
                       wtemp[j] = 0;
    ...
    if(smooth) rms3 = rms3 * sqrt(abs(smooth));
    if(eorwid > 0.0) {
        ...
        if(smooth) rms2 = rms2 * sqrt(abs(smooth)); // to correct correlated noise since smooth does not reduce number of freqs
    }
    ...
    if(i2>0 && i2<nfit) {gamma=bbrr[i2]/bbrr[0]; gammaerr=rms3*sqrt(fabsl(aarr[i2+i2*nfit]))/bbrr[0];}
...
{ int i,j,k,i1,i2;
  double data[NDATA],dataout[NDATA],dbdiff[NDATA],pdiff[NDATA],sum,a,dfreq,rmsdb,rmsphase,rms11,f0,fcen,max,del,delay;
...
//      printf("Sp delay %e\n",delay);
    ...
    printf("First and last freq: %.3f %.3f\n", freqq2[i1], freqq2[i2]);
...
//    for(i=0;i<np;i++) printf("k %d i %d data %f dataout %f diff %f\n",k,i,data[i],dataout[i],data[i]-dataout[i]);
    ...
    for(i=0;i<np;i++){
     ...
     rmsdb = sqrt(rmsdb/sum); rmsphase = sqrt(rmsphase/sum); rms11 = sqrt(rms11/sum);
     ...
     if(mode<=5) plotvna(np,freqq2,s11,wtt,npout,freqq1,Taa,dbdiff,pdiff,mode,rmsdb,rmsphase,rms11,nfit,fname);
}
...
double antloss(double freq, int mode)
{
    double sum;
    int j,nfit;
//    double losh[] = {3.470102e+01,  -1.335697e+02,  1.824230e+02,  -1.000643e+02,  1.874884e+01}; // highband GFhigh
    ...
    double losg7[] = {4.846670825246866e+02,  -1.023102678080684e+03,  1.303906332843779e+03,  -1.000317701465261e+03,  4.580417451841646e+02,  -1.129872060290666e+02,  1.163765940857081e+01}; // ground loss
     ...
     if(mode == 1) return 1;  // no antenna loss
...
// mode 0 = hot load  5 = low band 5 or 6 or 7 = lowband
   if(mode==0) { // 4" cable for hot load 
   cabl2(freq,4*2.54e-2/3e8,0,10,&s11,&s12,1.0); s21=s12; s22=s11;
   T = (s11a-s11)/(s12*s21-s11*s22+s22*s11a);   // from memo 132 
   return cabs(s12*s21)*(1-T*conj(T))/((1-s11a*conj(s11a))*(1-s22*T)*(1-conj(s22*T)));
             }
  if(mode==1) { // 2.5" cable for antenna
   cabl2(freq,2.5*2.54e-2/3e8,0,10,&s11,&s12,1.0); s21=s12; s22=s11;
   T = (s11a-s11)/(s12*s21-s11*s22+s22*s11a);   // from memo 132 
   return cabs(s12*s21)*(1-T*conj(T))/((1-s11a*conj(s11a))*(1-s22*T)*(1-conj(s22*T)));
             }
    if(mode==26) { // 10ft molex cablebox for antenna
   cabl2(freq,120.0*2.54e-2/3e8,0,12,&s11,&s12,1.0); s21=s12; s22=s11;
   T = (s11a-s11)/(s12*s21-s11*s22+s22*s11a);   // from memo 132
   return cabs(s12*s21)*(1-T*conj(T))/((1-s11a*conj(s11a))*(1-s22*T)*(1-conj(s22*T)));
             }
   ...
   if(mode==22) del4=0.9*2.54e-2/3e08; // EDGES3
   ...
   if(mode==22) del=3*2.54e-2/3e08; // balun midband 3"
...
//   if(mode==0) {del=0; del4=3.03e-10;}  // for hotload  - fit to semi from raul
   if(mode==5 || mode==6 || mode==7 || mode==8 || mode ==9 || mode==10 || mode==11 || mode==22) { cabl2(freq,del4,0,8,&s11,&s12,ttest); s21=s12; s22=s11;} // SC3792   
   ...
   if(mode == 5 || mode==6 || mode==7 || mode==8 || mode == 9) { cabl2(freq,del,0,5,&s11,&s12,ttest2); s21=s12; s22=s11;}  // low band balun tube
   if(mode == 10 || mode==11) { cabl2(freq,del,0,21,&s11,&s12,ttest2); s21=s12; s22=s11;}  // mid band balun tube
   if(mode == 22) { cabl2(freq,del,0,10,&s11,&s12,ttest2); s21=s12; s22=s11;}  // EDGES-3 cable
...
{ double L;
  complex double T,s11,s12,s21,s22;
     s11=0.01; s22=-0.01; s12=s21=0.99*cexp(-2*PI*freq*1e6*1e-9*I);
...
{ complex double T,Zcab,g,s11,s12,s21,s22,Vin,Iin,Vout,VVin,Z;
  ...
  if(mode==10){ b=0.1175*2.54e-2*0.5; a=0.0362*2.54e-2*0.5; diel = 2.05; // UT-141C-SP 
                d2 = sqrt(1.0/(PI*4.0*PI*1e-7*5.96e07*0.8));   // for tinned copper
                d = sqrt(1.0/(PI*4.0*PI*1e-7*5.96e07));  // skin depth at 1 Hz for copper
              }

  if(mode==11){ b=1.57e-3*0.5; a=0.51e-3*0.5; diel = 2.05; // UT-086C-SP for 0.086 2S-MRC hot load
                d2 = sqrt(1.0/(PI*4.0*PI*1e-7*5.96e07*0.8));   // for tinned copper
                d = sqrt(1.0/(PI*4.0*PI*1e-7*5.96e07));  // skin depth at 1 Hz for copper
              }

  if(mode==12){ b=0.1175*2.54e-2*0.5; a=0.0453*2.54e-2*0.5; diel = 1.32;  // Molex WM10479-nd FEP molex 0897621372 temp-flex 141-1701 center 0.0453 outer not known 87% VOP
...
double beamcorr(double freq,int bfit, double ang, double secs, double mcalc[],double fitf[],long double aarr[],double bbrr[],int skymode, int integ,double* lst,double bb[],double* bspac,int cmb,double *freqref,double fbstart,double fbstop,int site)
{
...
double k,sum,amp,amp150,raa,dec,lat,lon,gstt,sunra,sundec,sunaz,sunel,glat,glon,wsum,azz,el,sang,mp,dp,gp,opac,telec,rms,wb;
double ssecs,fr,cmbb;
int i,j,n,nn,az,frq,frq150,iaz,iel,m,frqst,frqspac,nbeam,yr,dy,hr,mn,sc;
...
// printf("i %d j %d k %6.0f\n",i,j,k);
    ...
    if(site == 3) {lon = -119.05 * PI / 180.0; lat = 75.432 * PI / 180.0;} // Haughton crater for simulations
    if(site == 4) {lon = -89.811 * PI / 180.0; lat = 75.432 * PI / 180.0;} // Haughton crater 
...
//    ssecs += -2*3600; //  test only
    ...
    if(skymode&128) poww[frq] = pow(fr/408.0,-2.5+dp+gp);
    }
    for(j=0;j<1024;j+=1){
//    GalactictoRadec(glat,glon,&raa,&dec);
...
//                   if(fabs(glat)<10) wsum *= pow(fr*0.5/408.0,0.1);  // correct galactic plane spectral index difference
    ...
    else wsum=0.0;  // was 300 - needs change if horizon limit 
    fsum[frq] += amp * wsum;
    fsum1[frq] += amp;
...
//    plotfspec(nbeam,poww,data,data2,wt,0,0,6,name,name,name);
    printf("beamfit nbeam %d bfit %d rms %e\n",nbeam,bfit,sqrt(rms/sum));
...
void plotvna(int np, double freqq[],complex double data[],double wtt[], int npout,double freqq2[],complex double data2[],double dbdiff[],double pdiff[],int mode,double rmsdb,double rmsphase,double rms11,int nfit,char *fname)
  // plot the spectrum 
{
    char txt[256];
...
//             if(fabs(k-0.7*nn) < 1.0) ypos[iter]=y+10.0;
    ...
    fprintf(file, "/Times-Roman findfont\n 18 scalefont\n setfont\n");
    fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",xoffset + 0.0, 45.0, txt);
    fprintf(file, "/Times-Roman findfont\n 14 scalefont\n setfont\n");
    sprintf(txt,"rms diff %6.3f dB %6.3f deg %4.1e\n",rmsdb,rmsphase,rms11);
    fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",xoffset + 220.0, 45.0, txt);
