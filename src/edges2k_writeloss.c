...
void   fittp(int,double*,complex double*,double*,int,double*,complex double*,int,double*,double*,long double*,double*,int,char*);
void plotvna(int,double*,complex double*,double*,int,double*,complex double*,double*,double*, int,double,double,int,char*);
void  wavefit(int,int,int,double*,double*,double*,complex double*,complex double*,double,double,double,
              double*,double*,double*,double*,double*,double*,long double*,double*,double*);
...
int main(int argc, char *argv[])
{
    ...
    static double sspant[NDATA],sspopen[NDATA],ssphot[NDATA],sspamb[NDATA],wtemp[NDATA]; // temporary - to preserve original
    ...
    int i,j,k,kk,nant,nopen,nshort,nhot,namb,mode,ns11ant,ns11lna,ns11hot,ns11amb,ns11cab1,ns11cab2,ns11rig,nfit,nfit1,imp,db,nfit2,nfit3,nfit4,iter,mfit,cfit,wfit,skymode,yr,dy,hr,mn,sc,smooth,cal,wtmode,lmode,cmb,nline,site,map;
    int ncal,cons,binteg,nter,nocal,bfit,bfit2,mdd,skymod2,test,i1,i2,i3,i4,i5,i6,i7,yrh,dyh,hrh,mnh,sch,yrc,dyc,hrc,mnc,scc,yro,dyo,hro,mno,sco,yrs,dys,hrs,mns,scs,low,sim,rr;
    ...
    mode = 0; nant=nopen=nshort=nhot=namb=ns11ant=ns11lna=ns11hot=ns11amb=ns11cab1=ns11cab2=ns11rig=0; wtmode=0; cons=0; nocal=0; low=0; sim=ion=tau=noise=rr=0; lr=li=ar=ai=0;
    datfile[0] = ambfile[0]=hotfile[0]=openfile[0]=shortfile[0]=0;
    title[0]=hottitle[0]=ambtitle[0]=opentitle[0]=shorttitle[0]=0;
    antaz = 90.0;
    tamb = 300.0;  // should agree with what is in acqplot2 - but doesn't really matter 
    tload = 300.0; // internal comparison load - value doesn't matter as long as it have same value as used in calibration
    cal=0; tcab=0; atten=0; Lhh = 1;
    wfstart = 50; wfstop = 200; imp=0; db=0; secs=0; yr=yrh=yrc=yro=yrs=dy=hr=mn=sc=cmb=nline=0; fstart = 50; fstop = 200; fbstart = 0; site = 0; map = 0;
    lst = 0; skymode=skymod2= -1; tcold = tant = 273.0+25.0; aloss = 0; bfit = bfit2 = 0;
    mfit=1; thot = 273.0+95.0; delaylna = 0;  wfit = 7; cfit = 7; nfit1 = 27; nfit2 = 27; nfit3 = 27; nfit4 = 37; dlyrob = 0.5/3e8; smooth=1; 
    Lh = 0.05;  // estimated loss of 6 inches 0.141 stainless steel dB at 100 MHz   thot  and tcold have to be correct or both scaled 1K error ~ 10mK
    eorcen=eorwid=eoramp=0; delaycorr=dbcorr=0; lmode= -1; adb=ldb=opn=0; binteg = 0; dscale = 0; nter=8; delayant = 0; mdd = 0; test = 0; specin = -2.5; sunind = 0.5;
    for(i=0;i<argc-1;i++){
    ...
    if (strstr(buf, "-hotsparms")) mode=31; // from /data5/edges/data/CalHotLoadCableData/ but has to be fit if freq spacing different
    ...
    if (strstr(buf, "-skymode")) {sscanf(argv[i+1],"%d",&skymode);}  // -1 = no beamcorrection 0 = beamcorrection 256 = use beamcorr as data + 1,2,4,8,16,32,64,128 opts 
    if (strstr(buf, "-skymod2")) {sscanf(argv[i+1],"%d",&skymod2);}  // use for new slower beamcorrection using new Haslam map 
    if (strstr(buf, "-lmode")) {sscanf(argv[i+1],"%d",&lmode);} // -1 = to use aloss 0=for hot load 2=hot+rigloss 1= balun + bean using interpolated antloss 5or6=lowband balun +conn
    if (strstr(buf, "-wtmode")) {sscanf(argv[i+1],"%d",&wtmode);} // sets weighting for fit and > 10 for  calibration 1 = normal zero wt outside wfstart & wfstop 
    ...
    if (strstr(buf, "-tcab")) {sscanf(argv[i+1], "%lf",&tcab);} // temperature of cable used for noise wave calibration get set to tcold unless entered
    if (strstr(buf, "-tant")) {sscanf(argv[i+1], "%lf",&tant);}  // temperature of antenna used in loss calculation needed even when specal.txt is used
    ...
    if (strstr(buf, "-Lh")) {sscanf(argv[i+1], "%lf",&Lh);}   // loss of hot load at 100 MHz uses tamb for loss calc Lh = -1 for hot load model Lh = -2 to use s12,s22 
...
//                   printf("j %d freq %f tt %f\n",j,freq,tt);
                   if(mode==1 && j==0) { sscanf(buf, "%*s %*s %*s %d %*s %s %d:%d:%d:%d:%d %lf %lf %lf %lf %lf",&nline,datfile,&yr,&dy,&hr,&mn,&sc,&gha,&dgha,&ghav,&ghamax,&ghamin);}
                   if(mode==2 && j==0) { sscanf(buf, "%*s %*s %*s %*s %*s %s %d:%d:%d:%d:%d %lf %lf %lf %lf %lf",openfile,&yro,&dyo,&hro,&mno,&sco,&gha,&dgha,&ghav,&ghamax,&ghamin);}
                   if(mode==5 && j==0) { sscanf(buf, "%*s %*s %*s %*s %*s %s %d:%d:%d:%d:%d %lf %lf %lf %lf %lf",shortfile,&yrs,&dys,&hrs,&mns,&scs,&gha,&dgha,&ghav,&ghamax,&ghamin);}
                   if(mode==3 && j==0) { sscanf(buf, "%*s %*s %*s %*s %*s %s %d:%d:%d:%d:%d %lf %lf %lf %lf %lf",hotfile,&yrh,&dyh,&hrh,&mnh,&sch,&gha,&dgha,&ghav,&ghamax,&ghamin);}
                   if(mode==4 && j==0) { sscanf(buf, "%*s %*s %*s %*s %*s %s %d:%d:%d:%d:%d %lf %lf %lf %lf %lf",ambfile,&yrc,&dyc,&hrc,&mnc,&scc,&gha,&dgha,&ghav,&ghamax,&ghamin);}
                   if(freq >= fstart && freq <= fstop){
                   if(mode==1) { freqant[j] = freq; md[j]=spant[j]=tt; wtant[j]=wt; nant = j+1;}
                   if(mode==2) { freqopen[j] = freq; spopen[j]=tt; wtopen[j]=wt; nopen = j + 1;}
                   if(mode==5) { freqshort[j] = freq; spshort[j]=tt; wtshort[j]=wt; nshort = j +1;}
                   if(mode==3) { freqhot[j] = freq; sphot[j]=tt; wthot[j]=wt; nhot = j +1;}
                   if(mode==4) { freqamb[j] = freq; spamb[j]=tt; wtamb[j]=wt; namb = j + 1;}
                 j++;
                 }}
                if(mode>10 && mode < 17){
                  ...
                  if(k>=2){
                         ...
                         if(freq >= fstart*1e6 && freq <= fstop*1e6){
                          if(mode==11){s11ant[j] = T; freqs11ant[j] = freq / 1e6; wttant[j]=1; ns11ant = j +1; strcpy(antfname,fname);}  
                          if(mode==12){s11cab1[j] = T; freqs11cab1[j] = freq / 1e6; wttcab1[j]=1; ns11cab1 = j + 1; strcpy(openfname,fname);} 
                          if(mode==16){s11cab2[j] = T; freqs11cab2[j] = freq / 1e6; wttcab2[j]=1; ns11cab2 = j + 1; strcpy(shortfname,fname);} 
                          if(mode==13){s11lna[j] = T; freqs11lna[j] = freq / 1e6; wttlna[j]=1; ns11lna = j + 1; strcpy(lnafname,fname);} 
                          if(mode==14){s11hot[j] = T; freqs11hot[j] = freq / 1e6; wtthot[j]=1; ns11hot = j + 1; strcpy(hotfname,fname);}
                          if(mode==15){s11amb[j] = T; freqs11amb[j] = freq / 1e6; wttamb[j]=1; ns11amb = j + 1; strcpy(ambfname,fname);}
                          if(buf[0] != 'B') j++;}
                          }
                          if(k) k++;
                  }
                  if(mode >=17 && mode < 40){ // Raul's format for all 
// s11 cal file
//freq [MHz],real(LNA),imag(LNA),real(ambient),imag(ambient),real(hot),imag(hot),real(open_cable),imag(open_cable),real(shorted_cable),imag(shorted_cable),real(s11_semirigid),imag(s11_semirigid),real(s12s21_semirigid),imag(s12s21_semirigid),real(s22_semirigid),imag(s22_semirigid),real(ant_sim1),imag(ant_sim1),real(ant_sim2),imag(ant_sim2)
//
...
//
                  ...
                  if(buf[0] != '#'){
                  sscanf(buf, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf"
                              ,&freq,&re,&im,&re1,&im1,&re2,&im2,&re3,&im3,&re4,&im4,&re5,&im5,&re6,&im6,&re7,&im7,&re8,&im8,&re9,&im9,
                               &re10,&im10,&re11,&im11,&re12,&im12,&re13,&im13,&re14,&im14);
                    if(freq >= fstart && freq <= fstop){
                          if(mode == 17) {  // don't overwrite other cals
                          T = re  + im*I;  s11lna[j] = T; freqs11lna[j] = freq; wttlna[j]=1; ns11lna = j + 1; strcpy(lnafname,fname);
                          T = re1 + im1*I; s11amb[j] = T; freqs11amb[j] = freq; wttamb[j]=1; ns11amb = j + 1; strcpy(ambfname,fname);
                          T = re2 + im2*I; s11hot[j] = T; freqs11hot[j] = freq; wtthot[j]=1; ns11hot = j + 1; strcpy(hotfname,fname);
                          T = re3 + im3*I; s11cab1[j] = T; freqs11cab1[j] = freq; wttcab1[j]=1; ns11cab1 = j + 1; strcpy(openfname,fname);
                          T = re4 + im4*I; s11cab2[j] = T; freqs11cab2[j] = freq; wttcab2[j]=1; ns11cab2 = j + 1; strcpy(shortfname,fname);
                                         }
                          if(mode == 17 || mode == 30) {  // all calfile
                          T = re5 + im5*I; s11rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re6 + im6*I; s12rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re7 + im7*I; s22rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                                         }
                          if(mode > 17 && mode < 26) {
                          ...
                          if(mode==25) T = re11 + im11*I;
                                       s11ant[j] = T; freqs11ant[j] = freq; wttant[j]=1; ns11ant = j + 1; strcpy(antfname,fname);
                          }
                          if(mode == 31) { // from /data5/edges/data/CalHotLoadCableData
                          T = re  + im*I;  s11rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re1 + im1*I; s12rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re2 + im2*I; s22rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          // printf("j %d freq %f T %f %s\n",j,freq,20*log10(cabs(s11rig[j])),fname);
                                         }
                          if(mode == 33) { // simulator 3
                          s11ant[j] = re5 + im5*I; freqs11ant[j] = freq; wttant[j]=1; ns11ant = j + 1; strcpy(antfname,fname);
                          }
                          if(mode == 26) {  // cboxnew
                          T = re2 + im2*I; s11rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re3 + im3*I; s12rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re4 + im4*I; s22rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re1 + im1*I;
                          s11ant[j] = T; freqs11ant[j] = freq; wttant[j]=1; ns11ant = j + 1; strcpy(antfname,fname);
                          }
                          if(mode == 27) { // 3 dB for lowband2
                          T = re3 + im3*I; s11rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re4 + im4*I; s12rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re5 + im5*I; s22rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re1 + im1*I;
                          s11ant[j] = T; freqs11ant[j] = freq; wttant[j]=1; ns11ant = j + 1; strcpy(antfname,fname);
                          }
                          if(mode == 28) { // cbx for lowband2
                          T = re10 + im10*I; s11rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re11 + im11*I; s12rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re12 + im12*I; s22rig[j] = T; freqs11rig[j] = freq; wttrig[j]=1; ns11rig = j + 1; strcpy(rigfname,fname);
                          T = re3 + im3*I;
                          s11ant[j] = T; freqs11ant[j] = freq; wttant[j]=1; ns11ant = j + 1; strcpy(antfname,fname);
                          }
                          if(mode == 29) { // noise
           ...
           fclose(file1);
//    printf("mode %d j %d nant %d nopen %d nhot %d namb %d ns11ant %d ns11lna %d ns11hot %d ns11amb %d ns11cab1 %d ns11cab2 %d\n",
...
//      for(j=0;j<ns11lna;j++) if(wtmode==0 && (freqs11lna[j] < wfstart || freqs11lna[j] > wfstop)) wttlna[j]=0;
      for(j=0;j<ns11lna;j++) s11lna[j] = s11lna[j]*cexp(-2.0*PI*freqs11lna[j]*1e6*((delaylna+delaycorr)*I))*pow(10.0,0.05*(dbcorr+ldb)) +lr+li*I;  // corrections for adapters
      for(j=0;j<ns11ant;j++) {if(opn) s11ant[j] = opn; 
                              else s11ant[j] = s11ant[j]*cexp(-2.0*PI*freqs11ant[j]*1e6*(delaycorr+delayant)*I)*pow(10.0,0.05*(dbcorr+adb)) +ar+ai*I;
                             }
      ...
      if(yr) secs=tosecs(yr,dy,hr,mn,sc); else secs = 0;
      ...
      fstart=freqamb[0]; fstop=freqamb[namb-1]; 
      ...
      if(skymod2 >= 0) bfit = beamcorr2(0,bfit,antaz,secs,mcalc,fitf,aarr,bbrr,skymod2,binteg,&lst,bb,&bspac,cmb,&freqref,fbstart,fbstop,site); // fit polynomial
      ...
      if(skymode >= 0) sprintf(info,"lst %5.2f",lst); else sprintf(info," "); 

 if(nant && (nopen || nshort) && nhot && namb && ns11ant && ns11lna && ns11hot && ns11amb && ns11cab1 && !nocal) { // continue as all data is present
   //      L = pow(10.0,-0.1*atten);  // L not used if atten needed for cable use R factor method of memo 98 

   // fit s11 with series

    printf("nopen %d nshort %d nhot %d namb %d ns11hot %d ns11amb %d ns11cab1 %d ns11cab2 %d freq %f %f %f %f\n",
   ...
   // 27 or higher for antenna
    printf("nant %d ns11ant %d ns11lna %d\n",nant,ns11ant,ns11lna); // period under 100 MHz
    fittp(ns11lna,freqs11lna,s11lna,wttlna,namb,freqamb,ss11lna,nfit3,fitf,mcalc,aarr,bbrr,1,lnafname);
    fittp(ns11hot,freqs11hot,s11hot,wtthot,namb,freqhot,ss11hot,nfit2,fitf,mcalc,aarr,bbrr,2,hotfname);
    fittp(ns11amb,freqs11amb,s11amb,wttamb,namb,freqamb,ss11amb,nfit2,fitf,mcalc,aarr,bbrr,3,ambfname);
    if(nopen) fittp(ns11cab1,freqs11cab1,s11cab1,wttcab1,namb,freqamb,ss11cab,nfit2,fitf,mcalc,aarr,bbrr,4,openfname);
    if(nshort) fittp(ns11cab2,freqs11cab2,s11cab2,wttcab2,namb,freqamb,&ss11cab[namb],nfit2,fitf,mcalc,aarr,bbrr,5,shortfname);
    if(ns11rig) { 
              fittp(ns11rig,freqs11rig,s11rig,wttrig,namb,freqamb,ss11rig,nfit2,fitf,mcalc,aarr,bbrr,7,rigfname);
              fittp(ns11rig,freqs11rig,s12rig,wttrig,namb,freqamb,ss12rig,nfit2,fitf,mcalc,aarr,bbrr,8,rigfname);
              fittp(ns11rig,freqs11rig,s22rig,wttrig,namb,freqamb,ss22rig,nfit2,fitf,mcalc,aarr,bbrr,9,rigfname);
                }
    if(!nopen){ for(i=0;i<namb;i++) {ss11cab[i]=ss11cab[i+namb]; spopen[i]=spshort[i]; freqopen[i] = freqshort[i]; wtopen[i]=wtshort[i]; }
                nopen = nshort; nshort = 0;   // used if only shorted cable available  
              }    
    // Do calibration in lab
        for(iter=0;iter<nter;iter++){   // needs 4 for accurate convergence
        for(i=0;i<namb;i++) {
          if(iter==0) {tcal_sca[i]=1; tcal_ofs[i]=tlna0[i]=tlna1[i]=tlna2[i]=0; sspopen[i] = spopen[i]; sspopen[namb+i] = spshort[i]; 
                       wtopen[namb+i] = wtshort[i]; sspamb[i] = spamb[i]; ssphot[i] = sphot[i]; freqopen[namb+i] = freqopen[i];}
    // invert with scale error and offset and changed S11 (i.e. with changes it will analyzed with wrong S11) 
          ...
          if(Lh == -1) Lhh = lossmodel(freqhot[i],ss11hot[i],0);  // mode = 0 for cable + adapter
          if(Lh == -2) Lhh = rigloss(ss11hot[i],ss11rig[i],ss12rig[i],ss22rig[i]);  // rigid cable loss
          ...
          if(wtopen[i] == 0 || wthot[i] == 0 || wtamb[i] == 0 || freqamb[i] < wfstart || freqamb[i] > wfstop) wtcal[i] = 0; else wtcal[i] = 1;
          if(wtshort[i] == 0 || wtcal[i] == 0) wtcal[namb+i] = 0; else wtcal[namb+i] = 1;
          ...
          if((wtmode%10)==3){
            if(wtopen[i] == 0 || wthot[i] == 0 || wtamb[i] == 0 || freqamb[i] < 80.0 || freqamb[i] > 195.0) wtcal[i] = 0; else wtcal[i] = 1;
            if(wtshort[i] == 0 || wthot[i] == 0 || wtamb[i] == 0 || freqamb[i] < 80.0 || freqamb[i] > 195.0) wtcal[namb+i] = 0; else wtcal[namb+i] = 1;
            }
          if((wtmode%10)==4){
            if(wtopen[i] == 0 || wthot[i] == 0 || wtamb[i] == 0 || freqamb[i] < 50.0 || freqamb[i] > 100.0) wtcal[i] = 0; else wtcal[i] = 1;
...
//    for(i=0;i<namb;i++) Tll[i]=Tll2[i]=s11lna[i]; // test of fitting
   ...
   for(i=0;i<namb;i++) {data[i]=thot; s2 =  w3pinv(ss11hot[i],ss11lna[i],ssphot[i],tload,tamb,tlna0[i],tlna1[i],tlna2[i]);
                      if(Lh < 0) Lhh = lossmodel(freqhot[i],ss11hot[i],0);
                      else Lhh = loss(ss11hot[i],Lh*sqrt(freqhot[i]/100.0));
                      dataout[i] = lossinv(s2,tcold,Lhh);}    // note tcold 
   printf("rms %f K wfit %d nhot %d\n",rmscalc(namb,data,dataout,wtcal),wfit,nhot); 
//   plotfspec(namb,freqhot, sphot, dataout, wtcal,dscale,0,1,title,datfile,info);
   for(i=0;i<namb;i++) {data[i]=tcold; dataout[i] =  w3pinv(ss11amb[i],ss11lna[i],sspamb[i],tload,tamb,tlna0[i],tlna1[i],tlna2[i]);}
   printf("rms %f K wfit %d namb %d\n",rmscalc(namb,data,dataout,wtcal),wfit,namb);
//   plotfspec(namb,freqamb, data, dataout, wtcal,dscale,0,2,title,datfile,info);
   
// 7 terms to smooth cal corrections
   for(j=0;j<cfit;j++){
   for(i=0;i<namb;i++){
    fitf[i+j*namb] = pow((freqamb[i]/fcen),0.5*j);    // poly is best cfit 6 - removed Fourier series - add 0.5 may be better 
    }}

 if(cfit>0){
   polyfitr(cfit,namb,tcal_sca,mcalc,wtcal,tcal_scasm,fitf,aarr,bbrr);
   printf("rms_tcal_sca %f dB cfit %d\n",4.0*rmscalc(namb,tcal_sca,tcal_scasm,wtcal),cfit);  //scale to get in dB approx
...
//   for(i=0;i<namb;i++) printf("freq %f tcal_ofs %f tcal_ofssm %f diff %f wtcal %f\n",freqamb[i],tcal_ofs[i],tcal_ofssm[i],tcal_ofs[i]-tcal_ofssm[i],wtcal[i]);
   }
  else for(i=0;i<namb;i++){tcal_scasm[i]=tcal_sca[i]; tcal_ofssm[i]=tcal_ofs[i];} // no fitting
 ...
 else if(!nant) {printf("missing data file\n"); return 0;}
 if(cal==0 || nant != namb) { readcal(&ncal,freqcal,ss11lna,tcal_scasm,tcal_ofssm,tlna0,tlna1,tlna2,wtcal); 
              if(ncal==0) return 0;
                for(i=0;i<ncal;i++) tmp[i] = ss11lna[i]*cexp(-2.0*PI*freqcal[i]*1e6*(delaylna*I))*pow(10.0,0.05*ldb)+lr+li*I;
                fittp(ncal,freqcal,tmp,wtcal,nant,freqant,tmpsm,nfit1,fitf,mcalc,aarr,bbrr,6,fname);
                for(i=0;i<nant;i++) ss11lna[i] = tmpsm[i];
                for(i=0;i<ncal;i++) tmp[i] = tcal_scasm[i] + tcal_ofssm[i]*I;
                fittp(ncal,freqcal,tmp,wtcal,nant,freqant,tmpsm,nfit1,fitf,mcalc,aarr,bbrr,6,fname);
                for(i=0;i<nant;i++) {tcal_scasm[i]=creal(tmpsm[i]); tcal_ofssm[i]=cimag(tmpsm[i]);}
                for(i=0;i<ncal;i++) tmp[i] = tlna1[i] + tlna2[i]*I;
                fittp(ncal,freqcal,tmp,wtcal,nant,freqant,tmpsm,nfit1,fitf,mcalc,aarr,bbrr,6,fname);
                for(i=0;i<nant;i++) {tlna1[i]=creal(tmpsm[i]); tlna2[i]=cimag(tmpsm[i]);}
                for(i=0;i<ncal;i++) tmp[i] = tlna0[i];
                fittp(ncal,freqcal,tmp,wtcal,nant,freqant,tmpsm,nfit1,fitf,mcalc,aarr,bbrr,6,fname);
                for(i=0;i<nant;i++) tlna0[i]=creal(tmpsm[i]);
                for(i=0;i<nant;i++) if(freqant[i] < freqcal[0] || freqant[i] > freqcal[ncal-1]) wtant[i] = 0;
              }
    if(wtmode > 10) for(i=0;i<nant;i++) if(freqs11ant[i] < wfstart - 1 || freqs11ant[i] > wfstop + 1) wttant[i] = 0; // useful for lower order fit 
    kk = 0;
    for(j=0;j<ns11ant;j++) if(wttant[j]) kk++;
    if(nfit4 >= kk) nfit4 = kk-1;
   fittp(ns11ant,freqs11ant,s11ant,wttant,nant,freqant,ss11ant,nfit4,fitf,mcalc,aarr,bbrr,0,antfname);
   if(lmode == 2){
              fittp(ns11rig,freqs11rig,s11rig,wttrig,nant,freqant,ss11rig,nfit2,fitf,mcalc,aarr,bbrr,7,rigfname);
              fittp(ns11rig,freqs11rig,s12rig,wttrig,nant,freqant,ss12rig,nfit2,fitf,mcalc,aarr,bbrr,8,rigfname);
              fittp(ns11rig,freqs11rig,s22rig,wttrig,nant,freqant,ss22rig,nfit2,fitf,mcalc,aarr,bbrr,9,rigfname);
   }
   if((freqant[nant-1] + freqant[0])/2.0 < 100.0) freqr = 75.0; else freqr = 150.0;
   if(freqref > freqr*1.5) low = 1;

   file1 = fopen("loss_file.txt", "w");
   fprintf(file1, "#freq  loss  loss_temp bmcorr[t_amb=%g]\n", tant);

   for(i=0;i<nant;i++){
   sspant[i] = (spant[i]-tamb)*tcal_scasm[i] + tamb - tcal_ofssm[i];
//   if(freqant[i] < wfstart || freqant[i] > wfstop || wtcal[i] == 0) wtant[i] = 0;
   ...
   if(lmode < 0) La = loss(ss11ant[i],atten+aloss*sqrt(freqant[i]/100.0));  // aloss > 0 for simple atten or cross-check
              else { // 6 = no antenna or ground plane loss
                  if(lmode == 2) La = rigloss(ss11ant[i],ss11rig[i],ss12rig[i],ss22rig[i]);  // hot load - can only use if calibration being done otherwise ss11rig=0
                  else La = lossmodel(freqant[i],ss11ant[i],lmode)*antloss(freqant[i],mcalc,fitf,aarr,bbrr,aloss,lmode); // lmode 0=hot load 1=balun+bend 5or6=balun+bend lowband
              }
   if(wtant[i] && (La < 0.1 || La > 1.0)) printf("check loss freq %f La %f\n",freqant[i],La);

   ltemp[i] = (1.0-La)*tant; 
   dataout[i] = lossinv(dataout[i],tant,La);
    fprintf(file1, "%g  %g  %g", freqant[i], La, ltemp[i]);

   if(!(i%(nant/10))) printf("freqant %7.2f tlnau %7.2f tlnacc %7.2f tlnacs %7.2f tlnac %7.2f phase %7.2f tsky %7.2f md %7.2f loss %5.2f\n",
...
//        printf("loss %f dB La %f freq %f dataout %f md %f beamcr %f\n",loss,La,freqant[i],dataout[i],md[i],beamcr);
//        beamcorr assumes a spectral index of -2.5
    if(test&16 && skymod2 <= -128) dataout[i] = beamcorr(freqant[i]*(1.0+low),bfit2,antaz,secs,mcalc,fitf,aarr,bbrr,0,0,&lst,bb2,&bspac,cmb,&freqref,0,0,site,map)*pow(1.0/(1.0+low),-2.5); // substitute  beamcorrection
    if(skymode >=0) {
      //printf("low=%d bfit=%d antaz=%g, secs=%g mcalc=%g, fitf=%g cmb=%d\n", low, bfit, antaz, secs, mcalc, fitf, cmb);
      ...
      fprintf(file1, "  %g\n", beamcorr(freqref,bfit,antaz,secs,mcalc,fitf,aarr,bbrr,0,0,&lst,bb,&bspac,cmb,&freqref,0,0,site,map)*pow(freqant[i]/freqref,-2.5)/skymodel[i]);
    ...
    if(skymod2 >=0) {skymodel[i] = beamcorr2(freqant[i]*(1.0+low),bfit,antaz,secs,mcalc,fitf,aarr,bbrr,0,0,&lst,bb,&bspac,cmb,&freqref,0,0,site)*pow(1.0/(1.0+low),-2.5);
    ...
    if(test==2)   dataout[i]=300*pow(freqant[i]/150.0,specin);  // test rms 14 mK at 2.55 1 mK at 2.51
    if(test==4)   dataout[i]=300*pow(freqant[i]/150.0,-2.55 + 0.01*log(freqant[i]/150.0));  // test rms 14 mK at 2.55 1 mK at 2.51 gamma 0.01
    if(test==6) {  dataout[i]=300*pow(freqant[i]/150.0,-2.55 + 0.0*log(freqant[i]/150.0));  // gamma 0.0
                     dataout[i] += dataout[i] * (-1e-3*pow(freqant[i]/150.0,-2.0)) + 1e3 * 1e-3*pow(freqant[i]/150.0,-2.0);} // 0.1% ion at 1000K 
    if(test&8) {  if(tau > 0) dataout[i] += -eoramp*(1-exp(-tau*exp(-(freqant[i]-eorcen)*(freqant[i]-eorcen)*(-log(-log((1+exp(-tau))/2)/tau))/(eorwid*eorwid*0.25))))/(1-exp(-tau));
                  else dataout[i] += -eoramp*exp(-0.69*(freqant[i]-eorcen)*(freqant[i]-eorcen)/((eorwid*eorwid+1e-6)*0.25));
                  dataout[i] += dataout[i]*ion*(-1e-3*pow(freqant[i]/150.0,-2.0))+ion*1e3*1e-3*pow(freqant[i]/150.0,-2.0);} // 1% ion at 1000K
   }
   ...
   for(i=0;i<nant;i++) {
                  if(test&1 && (skymode >=0 || skymod2>=0) && (skymode>=256 || skymod2>=256)) {
                      ...
                      if(tau > 0) tant2 += -eoramp*(1-exp(-tau*exp(-(freqant[i]-eorcen)*(freqant[i]-eorcen)*(-log(-log((1+exp(-tau))/2)/tau))/(eorwid*eorwid*0.25))))/(1-exp(-tau));
                      else tant2 += - eoramp*exp(-0.69*(freqant[i]-eorcen)*(freqant[i]-eorcen)/((eorwid*eorwid+1e-6)*0.25));
                  }
                  else tant2 = sim*300.0*pow(freqant[i]/150.0,-2.5);
    ...
    err1=0; i1=i2=i3=i4=i5=i6=i7=-1; // none selected  
    for(kk=0;kk<2;kk++){
    if(kk==0) nfit=2; else nfit=abs(mfit);
    for(i=0;i<nfit;i++){
    for(j=0;j<nant;j++) {
    k = j+i*nant; freq = freqant[j]; fcen = (freqant[nant-1] + freqant[0])/2.0;
    if(eorcen == 0) {
       if(i==0) fitf[k] =  1;  // constant
       if(i>0 && (i%2)==1) {fitf[k] =  w3pinv(ss11ant[j],ss11lna[j],sspant[j],tload,tamb,tlna0[j],tlna1[j],tlna2[j])    // s11ant mag
                           -w3pinv(ss11ant[j]*pow(10.0,-0.05*0.01*pow(freq/fcen,(i/2))),ss11lna[j],sspant[j],tload,tamb,tlna0[j],tlna1[j],tlna2[j]);}

       if(i>0 && (i%2)==0) {fitf[k] =  w3pinv(ss11ant[j],ss11lna[j],sspant[j],tload,tamb,tlna0[j],tlna1[j],tlna2[j])    // s11ant phase
                           -w3pinv(ss11ant[j]*cexp(-(PI/180.0)*pow(freq/fcen,(i/2)-1)*I),ss11lna[j],sspant[j],tload,tamb,tlna0[j],tlna1[j],tlna2[j]);}
       }
      else {
       if(mdd==1){i1=1; i3=2; i4=3;} // 4 terms sca,sp_ind,ion_abs,ion_em
       if(mdd==2){i3=1; i4=2;} // 3 terms sca,ion_abs,ion_em
       if(mdd==4){i1=1; i2=2; i3=3; i4=4; i5=5;} // 5 (6) terms sca,sp_ind,gamma,ion_abs,ion_em,(cons)
       if(mdd==5){i3=1; i6=2;} // 3 terms sca,ion_abs,sun
       if(mdd==6){i6=1;} // 2 terms sca,sun
       if(mdd==7){i3=1; i4=2; i6=3;} // 4 terms sca,ion_abs,ion_em,sun
       if(mdd==8){i1=1; i2=2; i7=3; i3=4; i4=5; i5=6; } // 5(6) terms sca,sp_ind,gamma,curve,sca,ion_abs,ion_em
       if(i==0) {if(mfit > 0 )fitf[k] =  pow(freq/freqr,specin); else fitf[k] = 1;}    // scale mfit=-1 useful for test
       if(i==i1) fitf[k] =   log(freq/freqr)*pow(freq/freqr,specin);   // adjustment of spec. index
       if(i==i2) fitf[k] =   log(freq/freqr)*log(freq/freqr)*pow(freq/freqr,specin);   // slope of index "gamma"  
//       if(i==i2) fitf[k] =   log(freq/freqr)*((freq-freqr)/freqr)*pow(freq/freqr,specin); linear case almost same
       if(i==i7) fitf[k] =   pow(log(freq/150.0),3.0)*pow(freq/150.0,specin);   // next term
//       if(i==4) fitf[k] =   pow(log(freq/150.0),4.0)*pow(freq/150.0,specin);   // next term       
       if(i==i3) fitf[k] =   pow(freq/freqr,-2.0+specin); // ion absorption should be negative and multiplicative
       if(i==i4) fitf[k] =   pow(freq/freqr,-2.0);  // ion emission should be positive and additive
       if(i==i5) fitf[k] =  1;  // constant
//       ss0=2.8; ss1=2.15;  var0=0.1; var1=0.01;
       if(i==i6) fitf[k] = pow(freq/freqr,sunind);  // Sun spe_ind 0 (quiet to -2(activ))   - see Kraus  0.5 best  
//       if(i==10) fitf[k] = 1.0/(1-ss11ant[j]*conj(ss11ant[j])); // some added instrumental functions will be needed for ant. s11 structure
       if(!mdd) fitf[k] = pow(freq/freqr,specin+i);  // best according to tests 5 + 1 terms 20 MHz full-width err=2 40 MHz err=6
//       if(i==11) fitf[k] = 1.0 - loss(ss11ant[j],0,0,0,0,freqant[i],dlyrob,lmode); // some added instrumental functions will be needed for ant. s11 structure
//       if((i==nfit-1 && nfit > 1 && eorwid > 0.0) || (i==1 && kk==0 && eorwid > 0.0)) fitf[k] =  exp(-0.69*(freq-eorcen)*(freq-eorcen)/(eorwid*eorwid));  // eor
      }
     }
     }
    for(i=0;i<40;i++) bbrr[i] = 0;  // zero previous soln. 
    if(cmb&1) for(i=0;i<nant;i++) dataout[i] += -2.725; // subtract cmb for fit
    polyfitrc(nfit,nant,dataout,mcalc,wtant,dataout2,fitf,aarr,bbrr,cons);
    if(cmb&1) for(i=0;i<nant;i++) {dataout[i] += 2.725; dataout2[i] += 2.725;} // add back cmb
    ...
    if(smooth) {
                 ...
                 if(smooth < 0) {
                   ...
                   for(i=0; i<nant; i+=kk) {
                       wtemp[j] = 0;
                       for(k=j*kk-kk/2; k<=j*kk+kk/2; k++) if(k >= 0 && k < nant && wtant[k]) wtemp[j] = 1;
                       dataout[j] = dataout[i]; freqant[j] = freqant[i]; mcalc[j] = mcalc[i]; dataout2[j] = dataout2[i]; wtant[j] = wtemp[j]; 
                       j++;}  
                       nant = j;
                 }
               }
    else for(i=0;i<nant;i++) mcalc[i]=dataout[i];
    ...
    if(eorwid > 0.0 && cmb&32) {
        ...
        if(smooth) rms2 = rms2 * sqrt(fabs(smooth)); // to correct correlated noise since smooth does not reduce number of freqs
    }
    t150=0; for(i=0;i<nfit;i++) if(!mdd || (i != i1 && i != i2)) t150 += bbrr[i];
    i=nfit-1;
    if(eorcen>100.0) days=aarr[i+i*nfit]*1e2*300.0*300.0/(20e-3*20e-3*(freqant[1]-freqant[0])*1e6*3600.0*8.0*0.3*0.4*0.5);
    else days=aarr[i+i*nfit]*1700.0*1700.0*1e2/(100e-3*100e-3*(freqant[1]-freqant[0])*1e6*3600.0*8.0*0.3*0.4*0.5);  // 8hrs,3pos,40%eff,loadnoise
    if(eorwid > 0.0)
      printf("Summary mfit %d mdd %d rms %4.0f K rms1 %6.1f mK value %8.1f err %5.1f snr %3.0f days %3.0f gha %6.2f T%d %5.1f sqrtcov %5.1f\n",
    ...
    else printf("Summary mfit %d mdd %d rms %4.0f K rms1 %6.1f mK gha %6.2f T%d %5.1f\n",mfit,mdd,rms,1e3*rms1,gha,(int)freqr,t150); 
    gamma=ion_abs=ion_em=ssun=spe_inderr=gammaerr=0;
    spe_ind = specin;
    if(i1>0 && i1<nfit) {spe_ind=specin+bbrr[i1]/bbrr[0]; spe_inderr=rms3*sqrt(aarr[i1+i1*nfit])/bbrr[0];}
    if(i3>0 && i3<nfit) ion_abs= -bbrr[i3]/bbrr[0];
    if(i4>0 && i4<nfit) if(ion_abs)ion_em=bbrr[i4]/ion_abs;
    if(i2>0 && i2<nfit) {gamma=bbrr[i2]/bbrr[0]; gammaerr=rms3*sqrt(fabs(aarr[i2+i2*nfit]))/bbrr[0];}
// note errors are not realistic unless random noise
    if(i6>0 && i6<nfit) ssun=bbrr[i6];
    if((i1>0 && i1<nfit) || (i3>0 && i3<nfit)) printf("Spectral_index %6.3f +/- %5.3f gamma %6.3f +/- %5.3f T%d %5.1f ion_opac %5.3f ion_em %5.0f K rms %6.3f K mfit %d mdd %d date %04d:%03d:%02d:%02d:%02d gha %8.3f\n",spe_ind,spe_inderr,gamma,gammaerr,(int)freqr,t150,ion_abs,ion_em,rms1,mfit,mdd,yr,dy,hr,mn,sc,gha);
...
void  wavefit(int nopen,int nshort, int wfit,double spopen[],double wtopen[],double freqopen[],complex double s11cab[],complex double s11lna[],
 ...
 { int i,j,k,nfit,m,mopen,iter;
   ...
   static double dataout2[NDATA];
    ...
    mopen = nopen+nshort;
    rms=0; min=1e99; bdly=0;
    for(iter=0;iter<2;iter++){
     if(iter==0) dd = 10e-9; else dd = 0;
     for(dl=0;dl<=dd;dl+=1e-9){  // not clear a delay search is needed
     if(iter==0) dly = -dl*1e6; else {dly = -bdly*1e6; *delay = bdly;}
     for(j=0;j<nfit;j++){
     for(i=0;i<mopen;i++){
     m = i%nopen;
     freq = freqopen[m]/freqopen[nopen/2];
     if(j%3==0) {tlnau = pow(freq,j/3); tlnacc=0.0; tlnacs=0;}
     if(j%3==1) {tlnau = 0.0; tlnacc=pow(freq,j/3); tlnacs=0;}
     if(j%3==2) {tlnau = 0.0; tlnacc=0.0; tlnacs=pow(freq,j/3);}
     ...
     fitf[i+j*mopen] = w3p(s11cab[i],s11lna[m],tcab,tload,tamb,tlnau,cc,cs) - w3p(s11cab[i],s11lna[m],tcab,tload,tamb,0,0,0);  // need to subract value for tlnau,tlnacc,tlnacs all zero
     dataout[i] = spopen[i] - w3p(s11cab[i],s11lna[m],tcab,tload,tamb,0,0,0);   // need to subtract cable noise
     }
     }
    polyfitr(nfit,mopen,dataout,mcalc,wtopen,dataout2,fitf,aarr,bbrr);
    rms = rmscalc(mopen,dataout,dataout2,wtopen);
//  printf("Sp rms %f\n",rms);
    if(rms < min) {min=rms; bdly=dl;}
    }
   }
   for(i=0;i<mopen;i++){
   m = i%nopen;
   ...
   cc=tlnacc*cos(2.0*PI*freqopen[m]*dly)+tlnacs*sin(2.0*PI*freqopen[m]*dly);
   cs=tlnacs*cos(2.0*PI*freqopen[m]*dly)-tlnacc*sin(2.0*PI*freqopen[m]*dly);
   dataout[i] =  w3p(s11cab[i],s11lna[m],tcab,tload,tamb,tlnau,cc,cs);  // treat like an antenna
   tlna0[i] = tlnau; tlna1[i] = cc; tlna2[i] = cs;
//   if(!(i%10))printf("freq %f data %f S11 %f %f\n",freqopen[i],dataout[i],creal(s11cab[i]),cimag(s11cab[i]));
   }
}


void fittp(int np,double freqq2[],complex double s11[],double wtt[],int npout, double freqq1[],complex double Taa[],int nfit, double fitf[], double mcalc[], long double aarr[], double bbrr[], int mode,char*fname)
{ int i,j,k,i1,i2;
  ...
  for(del=-1e-9;del<=100e-9;del+=1e-10){
    ...
    if(cabs(sumc) > max) {max = cabs(sumc); delay = del;}
    }
//      printf("Sp delay %e\n",delay);
    }
    i1 = 0;
    ...
    i2 = 0;
    for(i=np-1;i>=0 && i2==0;i--) if(wtt[i]) i2=i;
    f0=freqq2[i1];
    dfreq = (freqq2[i2] - f0)*1.5;  // was 2 might be better also wt = 1/cabs(T)  
    if(dfreq==0.0) {printf("no data in fittp\n"); return;}
    fcen = (freqq2[i2] + f0)*0.5;    // fixed 25sep15 now O.K. to nfit4 = 10
    for(j=0;j<nfit;j++){
    for(i=0;i<np;i++){
    if(nfit > 16){   // changed 13dec17 to 14 7aug18 to 16 20aug18
    ...
    else fitf[i+j*np] = pow(log10(freqq2[i]/fcen),j);   // log  helps a little  pow(log10(freqq2[i]/fcen),j);
    }
    }
    for(k=0;k<2;k++){
    for(i=0;i<np;i++) {if(k==0) data[i] = creal(s11[i]*cexp(2.0*PI*freqq2[i]*1e6*delay*I)); else data[i] = cimag(s11[i]*cexp(2.0*PI*freqq2[i]*1e6*delay*I));}
// if(mode==2) for(i=0;i<np;i++) printf("k %d i %d data %f s11 %f %f delay %f freqq2 %f\n",k,i,data[i],creal(s11[i]),cimag(s11[i]),delay,freqq2[i]);
//    {for(i=0;i<np;i++) printf("k %d i %d data %f s11 %f %f\n",k,i,data[i],creal(s11[i]),cimag(s11[i]));}
    polyfitr(nfit,np,data,mcalc,wtt,dataout,fitf,aarr,bbrr);
//    for(i=0;i<np;i++) printf("k %d i %d data %f dataout %f diff %f\n",k,i,data[i],dataout[i],data[i]-dataout[i]);
    for(i=0;i<npout;i++){
     sum=0; a=0;
     for(j=0;j<nfit;j++){ 
     if(nfit > 16){
     ...
     else a = pow(log10(freqq1[i]/fcen),j);    // 
     sum += bbrr[j]*a;
      }
     if(k==0) Taa[i] = sum;
     else  Taa[i] += sum*I;
    ...
    for(i=0;i<np;i++){
     ...
     rmsdb = sqrt(rmsdb/sum); rmsphase = sqrt(rmsphase/sum);
...
double wavemodel(complex double Ta, complex double Tl, double tsky, double tlnau,
                 double tlnacc, double tlnacs, double tlna)
{ complex double F;
  double t,ph;
...
//  ph = atan2(cimag(Ta*F*Tl),creal(Ta*F*Tl));
  t = (tsky*(1.0-Ta*conj(Ta))+ tlnau*Ta*conj(Ta))*F*conj(F)
      + cabs(Ta)*cabs(F)*(tlnacc*cos(ph)+tlnacs*sin(ph)) + tlna;
  return t;
}

double w3p(complex double Ta, complex double Tl, double tsky, double tload, double tamb, double tlnau,
                 double tlnacc, double tlnacs)
{ double pant,pcal,pload;
  ...
  return 100.0*(pant-pload)/(pcal-pload)+tamb;   // assumes first stage processing uses same tamb and tcal for all spectra
}

double w3pinv(complex double Ta, complex double Tl, double tin, double tload, double tamb, double tlnau,
                 double tlnacc, double tlnacs)
{ complex double F;
  double tout;
  F = 1.0 / (1.0 - Ta*Tl); 
...
//    double eff[] = {99.9066,99.9254,99.9388,99.9479,99.9534,99.9559,99.9560,99.9538,99.9496,99.9437,99.9361,99.9269,99.9162,99.9040,99.8904,99.8755,99.8592,99.8417,99.8229,99.8032};
//  eff from FEKO 100 to 200 MHz Blade  90 to 95 99.8965 and 99.9185
    double eff[] = {99.9364,99.9485,99.9576,99.9644,99.9694,99.9729,99.9751,99.9764,99.9769,99.9766,99.9758,99.9745,99.9727,99.9706,99.9681,99.9654,99.9625,99.9594,99.9561,99.9528,99.9493};
//    double los[] = {3.708375e+01,  -1.174062e+02,  1.632379e+02,  -1.067407e+02,  2.703088e+01};
//    double los[] = {4.412460e+01,  -1.632381e+02,  2.202354e+02,  -1.245132e+02,  2.495915e+01};
    double losh[] = {3.470102e+01,  -1.335697e+02,  1.824230e+02,  -1.000643e+02,  1.874884e+01}; // highband GFhigh
//    double los[] = {1.564797e+02,  -5.130209e+02,  6.599304e+02,  -3.749992e+02,  7.931530e+01}; // temploss9 - 2.5x2.5 0.1 genblade9e GF 13 2e-3
//    double los[] = {7.580215e+01,  -2.684529e+02,  3.635722e+02,  -2.154928e+02,  4.726889e+01}; // GF 3.5 1e-3
    double los[] = {5.744029e+01,  -1.726088e+02,  2.142321e+02,  -1.225013e+02,  2.702699e+01}; // GF 3.5 180deg
//    double losk[] = {6.410025e+00,  -1.549803e+01,  1.544351e+01,  -7.041051e+00,  1.300656e+00};
//    double losk[] = {6.321415e+01,  -1.886167e+02,  2.293411e+02,  -1.206021e+02,  2.315841e+01}; // for low3 memo 290
//    double losk[] = {1.041479e+01,  -6.031950e+00,  2.307908e+00}; // for low3 3-terms
//    double losk[] = {-1.179120134559797e+01,  8.816080087652101e+01,  -2.263158238997412e+02,  2.823787777684456e+02,  -1.647342754171209e+02,  3.580581311748211e+01};
    double losk[] = {-1.345035060985746e+00,  2.240940678524730e+00,  7.902278433276813e+00,  -5.318319232436456e+00}; // loss_low3a_2e-2_rock.txt -nfit 4 -fmin 52 -fmax 110 
...
//     for(j=0;j<nfit;j++) sum += (los[j]/4.0)*pow(freq * 2.0 / 150.0,j);  // divide by 4 to get 5x5m
...
//     printf("freq %f loss %f\n",freq,sum/300.0);
    ...
    if(mode == 20) {
     sum = 0; nfit = 5; // EDGES-3 ground loss
     for(j=0;j<nfit;j++) sum += losg5[j] * pow(freq/75.0,j);
//     printf("freq %f loss %f\n",freq,sum/300.0);
     return (1.0 - sum/300.0);
         }
    if(mode == 21) {
     sum = 0; nfit = 7; // EDGES-3 ground loss
     for(j=0;j<nfit;j++) sum += losg7[j] * pow(freq/75.0,j);
          return (1.0 - sum/300.0);
         }
    if(mode == 22) {
     sum = 0; nfit = 7; // EDGES-3 mesh loss
     for(j=0;j<nfit;j++) sum += losg8[j] * pow(freq/75.0,j);
          return (1.0 - sum/300.0);
         }
    if(mode == 100) {
          return (pow(10.0,-0.1*aloss*sqrt(freq/100.0))); // loss in dB independent of S11  
         }
     return 1;
}
...
//  double ss22i[] = {0.000466, 0.002232, -0.006258, 0.001766};
  ...
  double ss11r[] = {0.000622, 0.004519, 0.000199, -0.000442}; // lossfit2m2a 40 - 200 using semi_rigid_s_parameters_WITH_HEADER.txt
...
//   if(mode==4) {del4=2.5*2.54e-2/3e8; del=0.51/3e08; ttest=0.2;} // balun + bends for active coldload test
//   if(mode==0) {del=0; del4=3.03e-10;}  // for hotload  - fit to semi from raul
   if(mode==5 || mode==6 || mode==7 || mode==8 || mode ==9 || mode==10 || mode==11) { cabl2(freq,del4,0,8,&s11,&s12,ttest); s21=s12; s22=s11;printf("1stcall %f %f",creal(s11), cimag(s11));} // SC3792   
   else { cabl2(freq,del4,0,9,&s11,&s12,ttest); s21=s12; s22=s11;}  // bend or stainless 0.141
   if(mode==2) { // fit to /home/aeer/px14/mro/data2/semi_rigid_S_parameters.txt - as example 
    s11 = 0; for(m=0;m<4;m++) s11 += s11r[m] * pow(freq/150.0,m) + s11i[m] * pow(freq/150.0,m)*I;
    s12 = 0; for(m=0;m<4;m++) s12 += s12r[m] * pow(freq/150.0,m) + s12i[m] * pow(freq/150.0,m)*I;
    s22 = 0; for(m=0;m<4;m++) s22 += s22r[m] * pow(freq/150.0,m) + s22i[m] * pow(freq/150.0,m)*I;
    s21 = s12 = csqrt(s12);
   T = (s11a-s11)/(s12*s21-s11*s22+s22*s11a);   // from memo 132 - need to check Raul's convention s11 vs s22
   return cabs(s12*s21)*(1-T*conj(T))/((1-s11a*conj(s11a))*(1-s22*T)*(1-conj(s22*T)));
             }
   if(mode==0) { // fit to /home/aeer/px14/mro/data4/semi_rigid_s_parameters.txt - as example lossfit2m2.c 
    s11 = 0; for(m=0;m<4;m++) s11 += ss11r[m] * pow(freq/150.0,m) + ss11i[m] * pow(freq/150.0,m)*I;
    s12 = 0; for(m=0;m<4;m++) s12 += ss12r[m] * pow(freq/150.0,m) + ss12i[m] * pow(freq/150.0,m)*I;
    s22 = 0; for(m=0;m<4;m++) s22 += ss22r[m] * pow(freq/150.0,m) + ss22i[m] * pow(freq/150.0,m)*I;
    s21 = s12 = csqrt(s12);
   T = (s11a-s11)/(s12*s21-s11*s22+s22*s11a);   // from memo 132 - need to check Raul's convention s11 vs s22
   return cabs(s12*s21)*(1-T*conj(T))/((1-s11a*conj(s11a))*(1-s22*T)*(1-conj(s22*T)));
             }
   if(mode==12) { // loss for Molex cable 
    ...
    cabl2(freq,del4,0,12,&s11,&s12,ttest); s21=s12; s22=s11; 
    T = (s11a-s11)/(s12*s21-s11*s22+s22*s11a);   // from memo 132 - need to check Raul's convention s11 vs s22
   return cabs(s12*s21)*(1-T*conj(T))/((1-s11a*conj(s11a))*(1-s22*T)*(1-conj(s22*T)));
             }
   ta11= -(s11*s22-s12*s21)/s21; ta12=s11/s21; ta21=-s22/s21; ta22=1/s21;  // close to ref. plane
   if(mode == 1) { cabl2(freq,del,0,0,&s11,&s12,1.0); s21=s12; s22=s11;}  // high band balun tube
   if(mode == 5 || mode==6 || mode==7 || mode==8 || mode == 9) { cabl2(freq,del,0,5,&s11,&s12,ttest2); s21=s12; s22=s11;printf("1stcall %f %f",creal(s11), cimag(s21));}  // low band balun tube
   if(mode == 10 || mode==11) { cabl2(freq,del,0,21,&s11,&s12,ttest2); s21=s12; s22=s11;}  // mid band balun tube
   tb11= -(s11*s22-s12*s21)/s21; tb12=s11/s21; tb21=-s22/s21; tb22=1/s21; // T matrix from s-parms closer to antenna
   t11=ta11*tb11+ta12*tb21; t12=ta11*tb12+ta12*tb22; t21=ta21*tb11+ta22*tb21; t22=ta21*tb12+ta22*tb22; //A*B
//   t11=tb11*ta11+tb12*ta21; t12=tb11*ta12+tb12*ta22; t21=tb21*ta11+tb22*ta21; t22=tb21*ta12+tb22*ta22; //B*A
   s11=t12/t22; s12=(t11*t22-t12*t21)/t22; s21=1/t22; s22=-t21/t22;
...
//   T=cabl2(freq,del,T,5,&s11,&s12,1.0); T = cabl2(freq,del4,T,8,&s11,&s12,ttest);
//   printf("check s11a %f %f  %f %f\n",creal(s11a),cimag(s11a),creal(T),cimag(T));
return L2;
}
...
{ complex double T,Zcab,g,s11,s12,s21,s22,Vin,Iin,Vout,VVin,Z;
  ...
  // ttest = 0.5;  // best fit from day 2015_023
              {  b=0.37*2.54e-2*0.5; a=(5.0/32.0)*2.54e-2*0.5; diel = 1.07;  // balun tube
  ...
  if(mode==5) {  b=0.75*2.54e-2*0.5; a=(5.0/16.0)*2.54e-2*0.5; diel = 1.07;  // balun tube lowband
                 d2 = sqrt(1.0/(PI*4.0*PI*1e-7*5.96e07*0.29));  // skin depth at 1 Hz for brass
                 d = sqrt(1.0/(PI*4.0*PI*1e-7*5.96e07*ttest));  // skin depth at 1 Hz for copper
              }
  if(mode==8) { b=0.161*2.54e-2*0.5; a=0.05*2.54e-2*0.5; diel = 2.05;  // SC3792 connector - new dimensions from Fairviwe 8 Dec 15
  ...
  if(mode==9) { b=0.16*2.54e-2*0.5; a=0.05*2.54e-2*0.5; diel = 2.05;  // SMA connector
                d2 = sqrt(1.0/(PI*4.0*PI*1e-7*5.96e07*0.024*ttest));   // for Stainless
                d = sqrt(1.0/(PI*4.0*PI*1e-7*5.96e07*0.20*ttest));  // skin depth at 1 Hz note change  
              }
  if(mode==12) { b=0.1515*2.54e-2*0.5; a=0.0453*2.54e-2*0.5; diel = 2.1;  // Molex WM10479-nd FEP molex 100054008 could only find inner = 0.0453 inch twicked outer to get 50 ohms
                d2 = sqrt(1.0/(PI*4.0*PI*1e-7*5.96e07));   // for silver plated copper
                d = sqrt(1.0/(PI*4.0*PI*1e-7*5.96e07));  // for silver plated copper 
              }
  if(mode==21){  b=1.25*2.54e-2*0.5; a=(16.0/32.0)*2.54e-2*0.5; diel = 1.2;  // balun tube midband  8859k332    7782T333
                 d2 = sqrt(1.0/(PI*4.0*PI*1e-7*5.96e07*0.29));  // skin depth at 1 Hz for brass
                 d = sqrt(1.0/(PI*4.0*PI*1e-7*5.96e07*ttest));  // skin depth at 1 Hz for copper
              }
...
complex double cabl2(double freq, double delay, complex double Tin, int mode,complex double *ss11, complex double *ss12, double ttest)
{ complex double T,Zcab,g,s11,s12,s21,s22,Vin,Iin,Vout,Iout,VVin,Z;
  double a,b,d,diel,R,C,L,G;
  double q,Rs,R0,Ra,Xa,s,s1,s2;
  // ttest = 0.5;  // best fit from day 2015_023
              {  b=0.37*2.54e-2*0.5; a=(5.0/32.0)*2.54e-2*0.5; diel = 1.07;  // balun tube
                 s2 = 5.96e07*0.29;  // skin depth at 1 Hz for brass
                 s1 = 5.96e07;  // skin depth at 1 Hz for copper
              }
  if(mode==5) {  b=0.75*2.54e-2*0.5; a=(5.0/16.0)*2.54e-2*0.5; diel = 1.07;  // balun tube lowband
                 s2 = 5.96e07*0.29;  // skin depth at 1 Hz for brass
                 s1 = 5.96e07;  // skin depth at 1 Hz for copper
              }
  if(mode==8) { b=0.304*2.54e-2*0.5; a=0.086*2.54e-2*0.5; diel = 2.05;  // SC3792 connector
                s2 = 5.96e07*0.024*ttest;   // for Stainless
                s1 = 5.96e07*0.24*ttest;  // skin depth at 1 Hz for brass - might be less 0.24 
              }
  if(mode==9) { b=0.16*2.54e-2*0.5; a=0.05*2.54e-2*0.5; diel = 2.05;  // SMA connector
                s2 = 5.96e07*0.024*ttest;   // for Stainless
                s1 = 5.96e07*0.05*ttest;  // skin depth at 1 Hz for brass - might be less 0.24 
              }
  L = (4.0*PI*1e-7/(2.0*PI))*log(b/a);
  C=2.0*PI*8.854e-12*diel/log(b/a);
...
double lossinv(double tant, double tamb, double L)
{
  return (tant-tamb*(1-L))/L;
}


double beamcorr(double freq,int bfit, double ang, double secs, double mcalc[],double fitf[],long double aarr[],double bbrr[],int skymode, int integ,double* lst,double bb[],double* bspac,int cmb,double *freqref,double fbstart,double fbstop,int site,int map)
{
...
double sinlat,coslat;
 frqst = 0; frqspac = 0; n = 0; max = 0;
 if(integ > 0){
   if(cmb&2) cmbb = 2.725; else cmbb = 0;
   i = -1;
...
//                           printf("az %d el %d fr %d amp %f\n",az,j,frqst+i*frqspac,azel[(az+j*360)*NBEAM+i]);

 if(frqst < 70.0) *freqref = 75.0; else *freqref = 150.0;
 if(skymode >= 512){
   el=j;
...
//  if(fabs(sint)>1e-6 && el>=0.0) amp=(cos(0.5*PI*cost))/sint;
  if(fabs(sint)>1e-6 && el>=0.0){
     ...
     if(zb>0.0) amp = amp * sin(PI*2.0*zb*antght*(frqst+i*frqspac)/150.0);
     else amp = 1e-6;
// if((az==0 || az==90) && el==60)     printf("az %d el %d fr %d amp1 %f amp2 %f ratio %f\n",az,j,frqst+i*frqspac,azel[(az+j*360)*NBEAM+i],amp*amp,azel[(az+j*360)*NBEAM+i]/(amp*amp));
 ...
 if(i<NBEAM+1) nbeam = i+1; else return 0;
 if(n) nbeam = n+1;
// for(i=0;i<nbeam;i++) printf("i %d beam %f %f %f %f\n",i,azel[(0+0*360)*NBEAM+i],azel[(1+0*360)*NBEAM+i],azel[(0+1*360)*NBEAM+i],azel[(359+0*360)*NBEAM+i]);
 for(i=0;i<512;i++)for(j=0;j<1024;j++)mapgal[i][j]=-1;
 if(map == 0 || map == 2){if((file3=fopen("408-all-noh","r"))==NULL){return 0; }}    // map 0= use Haslam 1= use Guzman 2=Haslam + Guzman 3=Guzman + Haslam  
 if(map == 1 || map == 3){if((file3=fopen("/home/aeer/fits/45mhz.txt","r"))==NULL){return 0; }}   // use Guzmin map
 i=j=0;
 while(fgets(buf,32768,file3) != 0){
            p=buf;
            j=0;
            ...
            if(cmb&8) {if(i == 256 && j == 512) k = 2.3e6; else k = 18;} // simulate point source
...
// printf("i %d j %d k %6.0f\n",i,j,k);
    ...
    i++;}
    fclose(file3);
 if(map >= 2) {if(map == 2) if((file3=fopen("/home/aeer/fits/45mhz.txt","r"))==NULL){return 0; }
               if(map == 3) if((file3=fopen("408-all-noh","r"))==NULL){return 0; }
 ...
 while(fgets(buf,32768,file3) != 0){
    ...
    if(site == 11) {lon = 117.5 * PI / 180.0; lat = -27.7 * PI / 180.0;}
    if(site == 12) {lon = 117.5 * PI / 180.0; lat = -26.7 * PI / 180.0;}
    if(site == 13) {lon = 117.5 * PI / 180.0; lat = -25.7 * PI / 180.0;}
    if(site == 14) {lon = 116.5 * PI / 180.0; lat = -27.7 * PI / 180.0;}
    if(site == 15) {lon = 116.5 * PI / 180.0; lat = -26.7 * PI / 180.0;}
    if(site == 16) {lon = 116.5 * PI / 180.0; lat = -25.7 * PI / 180.0;}
    if(site == 17) {lon = 115.5 * PI / 180.0; lat = -27.7 * PI / 180.0;}
    if(site == 18) {lon = 115.5 * PI / 180.0; lat = -26.7 * PI / 180.0;}
    if(site == 19) {lon = 115.5 * PI / 180.0; lat = -25.7 * PI / 180.0;}
    if(site == 1) {lon = 116.5 * PI / 180.0; lat = 42.417 * PI / 180.0;} // Oregon for simulations
    if(site == 2) {lon = -119.05 * PI / 180.0; lat = 42.417 * PI / 180.0;} // Oregon
    if(site == 3) {lon = -90.76 * PI / 180.0; lat = 79.433 * PI / 180.0;} // Baffin Island
    if(site == 6) {lon = 116.5 * PI / 180.0; lat = 79.433 * PI / 180.0;} // Baffin Island for simulation
    if(site == 4) {lon = 21.9889 * PI / 180.0; lat = -30.969 * PI / 180.0;} // South Africa
    if(site == 5) {lon = 116.5 * PI / 180.0; lat = 13.0 * PI / 180.0;} // India for simulation
    if(site == 100){lon = 118.5 * PI / 180.0; lat = -24.7 * PI /180.0;} // for test offset from MRO
    if(site >100) {lon += ((site%10)-5)*PI/180.0; lat += ((site/10)-15)*PI/180.0;} // 155 = no offset 165 = +1deg lat 156 = +1deg long
    sinlat = sin(lat); coslat = cos(lat);
    for(m=0;m<nbeam;m++) fsum[m]=fsum1[m]=fsum2[m]=0;
    frq150=0; k = 1e6;
    for(frq=0;frq<nbeam;frq++) if(fabs(frqst+frq*frqspac-(*freqref)) < k) {k = fabs(frqst+frq*frqspac-(*freqref)); frq150=frq;} // find closest
    for(i=0;i<512;i++) {
...
//    nn = integ/900 + 1; // made a small difference
  ...
  for(n=0;n<nn;n++){
    ...
    sunaz = sunaz*180.0/PI; sunel=sunel*180.0/PI; *lst = (gstt+lon)*12.0/PI;
    if(*lst < 0) *lst += 24.0;
    if(*lst > 24.0) *lst -= 24.0;
    toyrday(ssecs,&yr,&dy,&hr,&mn,&sc);
    printf("Spsecs %10.0f %4d:%03d:%02d:%02d:%02d sunaz %6.2f sunel %6.2f lst %6.2f gha %6.2f\n",ssecs,yr,dy,hr,mn,sc,sunaz,sunel,*lst,*lst -(17.0+45.67/60.0));
    for(i=0;i<512;i+=1){
    glat=(i-256.0)*90.0/256.0;
    sang = (180.0/512.0)*cos(glat*PI/180.0)*(360.0/1024.0)*PI*PI/(180.0*180.0);
    for(frq=0;frq<nbeam;frq++){
    fr = frqst+frq*frqspac;
    if(skymode&1) dp = 0.05; else dp=0;  // spectral index variation
    if(fabs(glat) > 20.0) dp = -dp;  // sign changed nov18
    if(skymode&2) gp = -0.12*log(fr/150.0); // derivative Angelica's gamma
    else gp=0.0;
    ...
    azz=azz*180.0/PI; el=el*180.0/PI;
//  phi=el*PI/180.0; R=6356.0; h=300.0;  opac = (1.8/500.0)*(R+h)/sqrt(R*R*sin(phi)*sin(phi)+2*R*h+h*h); // from derivative
    if(azz < 0) azz += 360.0;
    if(el < 0) el = 0;
    iaz = azz+0.5; iel = el+0.5;
    if(iaz < 0) iaz = 0;
    if(iaz > 359) iaz = 359;
    if(iel > 90 ) iel = 90;
...
//    if(site < 10) mp += -5;  // offset has a large effect -5 best low2
//  Haslam 384 -site 0 Guz 384 site 10 Haslam+spind 256 site 100 Guz+spind 256 site 1000  
    aind = 2.5;
    if(el>0 && !(skymode&128)) {
...
//   aind = 2.6; // constant no effect  
    ...
    wsum = wsum2 = 0;
    if(el>0) {fr = frqst+frq*frqspac;
              if(skymode&128) {
                   wsum= (mp-cmbb)*poww[frq] + cmbb;  // speeds up execution 
//                   if(fabs(glat)<10) wsum *= pow(fr*0.5/408.0,0.1);  // correct galactic plane spectral index difference
                   }
              else {
              if(map >= 2)   { if(map == 2) wsum = (mp-cmbb)*pow(fr/408.0,-aind) + cmbb;
                               if(map == 3) wsum = (mp-cmbb)*pow(fr/45.0,-aind) + cmbb;
                              }
              else {
                gp = log(fr/150.0);
                 if(fabs(glat) < 9.0) wb = (cos(glon*PI/180.0)+1)/2.0; else wb = 0;  // values of 10 deg and 2.4 not yet well determined
                   wsum = (mp-3.0)*((1-wb)*pow(fr/408.0,-2.52 + 0.016*gp)+wb*pow(fr/408.0,-2.8-0.13*gp)) + 3.0; // best values from 2015:024
                   if(fabs(glat-45)<15 && fabs(glon-27)<15) wsum = mp*pow(150.0/408.0,-2.5)*pow(fr/150.0,-3.0-0.5*gp);   // test north polar spur region
                  }
              }
...
//  printf("fr %f amp %e wsum %e wsum2 %e azz %lf el %lf\n",fr,amp,wsum,wsum2,azz,el);
              }
    else wsum=300.0;
...
//   if(nbeam > NFIT) bfit = NFIT;   // 30 needed when azelq spacing is 2 MHz
   ...
   *bspac = (nbeam*1.2)*frqspac;  // need +20% to avoid ripple
   for(i=0;i<bfit;i++){
   for(j=0;j<nbeam;j++) {
//      if(j*frqspac+frqst >= 100 && j*frqspac+frqst < 200)  wt[j] = 1; else wt[j] = 0;
       wt[j]=1;
       if(bfit >= 7 && bfit <= 14) fitf[j+i*nbeam] =  pow(((j*frqspac+frqst))/(*freqref),i); // increased to 12 but 9 normally used 12nov18
       else {
   ...
   if(cmb&512) for(frq=0;frq<nbeam;frq++) data[frq]=(fsum[frq]/fsum1[frq])/pow((frqst+frq*frqspac)/((double)(frqst+frq150*frqspac)),-2.5);   // previous 
     polyfitr(bfit,nbeam,data,mcalc,wt,data2,fitf,aarr,bbrr);
    rms = 0; sum=0;
    for(i=0;i<nbeam;i++) {rms += wt[i]*(data[i]-data2[i])*(data[i]-data2[i]); sum+=wt[i];}
//    for(i=0;i<nbeam;i++) printf("Spbeam data %d %f %f diff %e bbrr %e rms %f lst %5.2f\n",frqst+i*frqspac,data[i],data2[i],data[i]-data2[i],bbrr[i],sqrt(rms/sum),*lst);
...
//    plotfspec(nbeam,poww,data,data2,wt,0,0,6,name,name,name);
  ...
  return bfit;
    ...
    sum = 0;
      for(i=0;i<bfit;i++){ 
       if(bfit >= 7 && bfit <= 14) sum +=  bb[i]*pow(freq/(*freqref),i);  // best since spectral index taken out cannot fit constant perfectly with pow(freq/150.0,-2.5+i);
       else {
...
double beamcorr2(double freq,int bfit, double ang, double secs, double mcalc[],double fitf[],long double aarr[],double bbrr[],int skymode, int integ,double* lst,double bb[],double* bspac,int cmb,double *freqref,double fbstart,double fbstop,int site)
{
 ...
 if((file3 = fopen("gmap.bin","rb"))==NULL) {
  ...
  if(site == 0){
    ...
    for(m=0;m<nbeam;m++) fsum[m]=fsum1[m]=fsum2[m]=0;
        frq150=0; k=1e6;
    for(frq=0;frq<nbeam;frq++) if(fabs(frqst+frq*frqspac-(*freqref)) < k) {k = fabs(frqst+frq*frqspac-(*freqref)); frq150=frq;} // find closest
//    for(frq=0;frq<nbeam;frq++) if(fabs(frqst+frq*frqspac-150.0) < 0.1) frq150=frq;
    nn = integ/1800 + 1;  // averaging over 3600 sec makes little difference to result
    d = (double)integ/(double)nn;
  for(n=0;n<nn;n++){
    ssecs = secs + d/2.0 + n*d - ((double)integ)/2.0;
  ...
  sang = 1;  // this should the case for HEALpix 
    ...
    for(frq=0;frq<nbeam;frq++){
    amp = sang*azel[(iaz+iel*360)*NBEAM+frq];
    if(skymode&32) {if(iel > 5) amp=1; else amp=0;}  // isotropic antenna
    if(skymode&64) {if(iel > 80)  amp=1; else amp=0;}  // beam at Zenith
    if(el>0) {mp = gmap[i]; 
              if(skymode&1) dp = -0.05; else dp=0;  // spectral index variation
              if(fabs(glat) > 20.0) dp = -dp;
              if(skymode&2) gp = -0.12*log((frqst+frq*frqspac)/150.0); // derivative Angelica's gamma
              else gp=0.0;
              if(skymode&128) wsum=(mp-cmbb)*pow((frqst+frq*frqspac)/408.0,-2.5+dp+gp) + cmbb;
              else {
              // correct 408 MHz map to 150 MHz - makes slight improvement at most GHAs - see memo 7
              if(fabs(glat) < 10.0) wb = (cos(glon*PI/180.0)+1.0)/2.0; else wb = 0;
              ...
              if(skymode&8) wsum += (telec - wsum)*opac*pow((frqst+frq*frqspac)/150.0,-2.0); // ion emission and absorption
...
//      if(j*frqspac+frqst >= 100 && j*frqspac+frqst < 200)  wt[j] = 1; else wt[j] = 0;
       wt[j]=1;
       if(bfit >= 7 && bfit <= 12) fitf[j+i*nbeam] =  pow(((j*frqspac+frqst))/(*freqref),i); // increased to 12 but 9 normally used 12nov18
       else {
        if((i%2)==0) fitf[j+i*nbeam] =  cos((j*frqspac+frqst-(*freqref))*2*PI*(i/2)/(*bspac));
        if((i%2)) fitf[j+i*nbeam] =  sin((j*frqspac+frqst-(*freqref))*2*PI*((i+1)/2)/(*bspac));
            }
   ...
   for(frq=0;frq<nbeam;frq++) data[frq]=(fsum[frq]/fsum1[frq])/(fsum2[frq]/fsum2[frq150]);  // normalize to 150 21 Jan 16
//     for(frq=0;frq<nbeam;frq++) data[frq]=(fsum[frq]/fsum1[frq])/pow((frqst+frq*frqspac)/150.0,-2.5);   // previous
     polyfitr(bfit,nbeam,data,mcalc,wt,data2,fitf,aarr,bbrr);
    rms = 0; sum=0;
    for(i=0;i<nbeam;i++) {rms += wt[i]*(data[i]-data2[i])*(data[i]-data2[i]); sum+=wt[i];}
//    for(i=0;i<nbeam;i++) printf("Spbeam data %d %f %f diff %e bbrr %e rms %f lst %5.2f\n",frqst+i*frqspac,data[i],data2[i],data[i]-data2[i],bbrr[i],sqrt(rms/sum),*lst);
...
//    plotfspec(nbeam,poww,data,data2,wt,0,0,6,name,name,name);
  ...
  return bfit;
    ...
    sum = 0;
      for(i=0;i<bfit;i++){ 
       if(bfit >= 7 && bfit <= 12) sum +=  bb[i]*pow(freq/(*freqref),i);  // best since spectral index taken out cannot fit constant perfectly with pow(freq/150.0,-2.5+i);
       else {
       if((i%2)==0)  sum +=  bb[i]*cos((freq-(*freqref))*2*PI*(i/2)/(*bspac));
       if((i%2))  sum +=  bb[i]*sin((freq-(*freqref))*2*PI*((i+1)/2)/(*bspac));
            }
...
{ int i;
  double rms,m;
  rms=m=0;
  for(i=0;i<n;i++) {rms += wt[i]*(data[i]-md[i])*(data[i]-md[i]); m += wt[i];}
  return sqrt(rms/m);
}
...
//     if(fabs(k-i)<=smooth/2) c=1; else c=0;  // boxcar
    ...
    if(b > 0.0) mcalc[i+n] = a/b;
    else mcalc[i+n] = mcalc[i];
    }
   for(i=0;i<n;i++){
//    if(wt[i] > 0.0)
...
double polyfitrc(int npoly, int nfreq, double ddata[], double mcalc[], double wtt[],
                double dataout[], double fitfn[], long double aarr[], double bbrr[],int mode)
{ int i,iter;
  double cons[50];
    polyfitr(npoly,nfreq,ddata,mcalc,wtt,dataout,fitfn,aarr,bbrr); 
    if(mode == 0) return bbrr[0];
    else {
      for(iter=0;iter<10;iter++){
      for(i=0;i<npoly;i++) if(bbrr[i] < 0) cons[i] = mode*1e-3; else cons[i]=0; 
...
double polyfitr2(int npoly, int nfreq, double ddata[], double mcalc[], double wtt[],
                double dataout[], double fitfn[], long double aarr[], double bbrr[], double cons[])
{
    int i, j, k, kk, m1, m2;
    double re;
    for (i = 0; i < nfreq; i++) {
...
double polyfitr(int npoly, int nfreq, double ddata[], double mcalc[], double wtt[], 
                double dataout[], double fitfn[], long double aarr[], double bbrr[])
{
    int i, j, k, kk, m1, m2;
    double re, dd;
    for (i = 0; i < nfreq; i++) {
...
//  for(i=0;i<n;i++)for(j=0;j<n;j++) aa[i][j]=a[j+i*n]; // save for check
  ...
  for (k=0;k<n-1;k++) {
   scale=0.0;
   for (i=k;i<n;i++) if(fabsl(a[k+i*n]))  scale=fabsl(a[k+i*n]);
   if (scale == 0.0) {
   printf("singular1\n");
   c[k]=d[k]=0.0;
   } else {
   ...
   if(a[k+k*n] > 0)sigma = sqrtl(sum); else  sigma = -sqrtl(sum);
   a[k+k*n] += sigma;
   c[k]=sigma*a[k+k*n];
   d[k] = -scale*sigma;
   for (j=k+1;j<n;j++) {
...
// int Upper_Triangular_Inverse(double *U, int n)
   ...
   for(i=0;i<n;i++){
   for(j=0;j<n;j++){
       if(j>i) u[i][j]=a[j+i*n]; else u[i][j]=0;
       if(i==j) u[i][i]= d[i];
   }} 

     for(k=0;k<n;k++) {
          if(u[k][k]==0) return;
          else u[k][k]=1.0/u[k][k];
          }
  ...
  // plot the spectrum 
{
    ...
    fstart = freqq[0];
    fstop = freqq[np-1];
        dmax = -1e99; dmin = 1e99; dscale = -1e99;
        for(k = 0; k < np; k++) if(wtt[k] && data2[k] > dmax) dmax = data2[k];
        for(k = 0; k < np; k++) if(wtt[k] && data2[k] < dmin) dmin = data2[k];
        for(k = 0; k < np; k++) if(wtt[k] && fabs(data2[k]-data[k]) > dscale) dscale = fabs(data2[k]-data[k]);
        if(dscale < 1e-6) dscale = 1;
        if(ddscale) dscale = ddscale;
        rms = 0; ss=1e-30; av=avv=0;
        for(k = 0; k < np; k++) if(wtt[k]){ 
                rms += (data2[k]-data[k])*(data2[k]-data[k]);
                av += data2[k]-data[k];
                avv += data2[k];
                ss++;
                           }
       rms = sqrt(rms/ss); av=av/ss; avv = avv/ss;
        rms1 = 0;
        for(k = 0; k < np; k++) if(wtt[k]){
...
//    if(dmax > 2000) dmax=2000;   // temporary fix
    ...
    if(mode) {dmax = 120; dmin = 0;}
    if(num==5) {
         dmax = 5;
         for(k = 0; k < np; k++) if(data2[k] > dmax) dmax = data2[k];
         if(dmax > 10) dmax = 10;
    }
    scale = dmax-dmin;
//    printf("dmax %f dmin %f\n",dmax,dmin);
    ...
    for (k = 0; k < np; k++) {
     ...
     if(iter==2) {
        ...
        if(k==0 || x < xp) {xp=x;yp=y;}
            if(!iter || (iter==1 && data2[k] < dmax) || iter==2)
            {
             if(iter ==1 && k >0 && wtt[k]==0) fprintf(file, "%d setlinewidth\n",1);
             else if(iter == 1)fprintf(file, "%d setlinewidth\n",2);
              h=s=b=0;
    ...
    } 
   {
    ...
    if(num==0) sprintf(txt, "raw sky spectrum (thin line) vs calibrated spectrum of sky (thick line)");
    if(num==1) sprintf(txt, "hot temperature (thin line) vs calibrated spectrum of hot load(thick line)");
    if(num==2) sprintf(txt, "ambient temperature (thin line) vs calibrated spectrum of ambient load(thick line)");
    if(num==3) sprintf(txt, "cable spectrum (thin line) vs fit to cable spectrum (thick line)");
    if(num==4) sprintf(txt, "calibrated sky spectrum (thin line) vs sky model spectrum (thick line)");
    ...
    if(num==99) sprintf(txt, "calibrated sky spectrum (thin line) vs fit to sky spectrum (thick line) %s",info);
    fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",xoffset, 55.0, txt);
  ...
  // writeout calibrated spectrum 
{
    ...
    for (k = 0; k < np; k++) {
    if(mode==1){
    fprintf(file,"freq %10.6f tantenna %10.6f K skymodel %10.6f K ant_area %8.6f m^2 Tsun_per_SFU %5.3f K lst %5.2f %s\n",
  ...
  // writeout calibrated spectrum 
{
    ...
    for (k = 0; k < np; k++) {
    fprintf(file,"freq %10.6f s11lna %10.6f %10.6f sca %10.6f ofs %10.6f tlnau %10.6f tlnac %10.6f tlnas %10.6f wtcal %1.0f %s\n",
...
{     
      ...
      for (k = 0; k < np; k++) {
         if(k==0) fprintf(file,"%10.6f %10.6f 1 3 // sim %04d:%03d:%02d:%02d:%02d 0  0 0 0 0\n",freqq[k],spant[k],yr,dy,hr,mn,sc);
         else fprintf(file,"%10.6f %10.6f 1\n",freqq[k],spant[k]);
      }
      fclose(file);
}
    ...
    if ((file = fopen(txt, "r")) == NULL) {
        ...
        return;
         ...
         while (fgets(buf, 32768, file) != 0) {
                   sscanf(buf, "%*s %lf %*s %lf %lf %*s %lf %*s %lf %*s %lf %*s %lf %*s %lf %*s %lf",
                      &freq,&re,&im,&ssca,&oofs,&t0,&t1,&t2,&t3);
                    T = re+im*I;
//                    T = T*cexp(-2.0*PI*freq*1e6*(0.0e-12)*I)*pow(10.0,0.05*0.2);     // for test corrections
                 freqq[k]=freq; s11lna[k]=T; sca[k]=ssca; ofs[k]=oofs; tlnau[k]=t0; tlnacc[k]=t1; tlnacs[k]=t2; wtcal[k]=t3;
                 k++;
                 }
    *np = k;
    for (k = 0; k < *np; k++) {
//    printf("freq %10.6f s11lna %10.6f %10.6f sca %10.6f ofs %10.6f tlnau %10.6f tlnac %10.6f tlnas %10.6f wtcal %1.0f\n",
...
void plotvna(int np, double freqq[],complex double data[],double wtt[], int npout,double freqq2[],complex double data2[],double dbdiff[],double pdiff[],int mode,double rmsdb,double rmsphase,int nfit,char *fname)
  // plot the spectrum 
{
    char txt[256];
    ...
    fstop = freqq2[npout-1];
        dmax = -1e99; dmin = 1e99;
        for(k = 0; k < npout; k++) if(10.0*log10(creal(data2[k]*conj(data2[k]))) > dmax) dmax = 10.0*log10(creal(data2[k]*conj(data2[k])));
        for(k = 0; k < npout; k++) if(10.0*log10(creal(data2[k]*conj(data2[k]))) < dmin) dmin = 10.0*log10(creal(data2[k]*conj(data2[k])));
        dscale = pscale = 1e-99;
        for(k = 0; k < np; k++) if(fabs(dbdiff[k]) > dscale) dscale = fabs(dbdiff[k]);
        for(k = 0; k < np; k++) if(fabs(pdiff[k]) > pscale) pscale = fabs(pdiff[k]);
    k = dmin * 0.1 - 1.0;
    dmin = k * 10;
    k = dmax * 0.1 + 0.5;
    dmax = k * 10;
    scale = dmax-dmin;
    if(scale==0.0) {dmax = 1; dmin = -1; scale = 2.0;}
//   printf("dmax %f dmin %f np %d npout %d\n",dmax,dmin,np,npout);
    ...
    if(iter%2)   fprintf(file, "%d setlinewidth\n",1);  // for phase
    else  fprintf(file, "%d setlinewidth\n",0); 
    if(iter<2 || iter > 3) fprintf(file, "%d setlinewidth\n",1);
    yp = 0;
    xp = 0;
    totpp = 0;
    if(iter < 2 || iter > 3) nn = np; else nn = npout;
    nfirst = 1;
    for (k = 0; k < nn; k++) {
        h=s=b=0;
        if(iter < 2 || iter > 3)    x = (freqq[k] - fstart) * 400.0 / (fstop-fstart);
        else    x = (freqq2[k] - fstart) * 400.0 / (fstop-fstart);
...
//             if(fabs(k-0.7*nn) < 1.0) ypos[iter]=y+10.0;
    ...
    for (f = dmin; f <=dmax; f += (dmax - dmin) / 10.0) {
        ...
        fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n 0 0 0 sethsbcolor stroke\n",
           x + xoffset,y+yoffset,x + xoffset + 5,y+yoffset);
           {sprintf(txt, "%4.0f", f); fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",x + xoffset - 25.0,y - 1.0+yoffset, txt);}
    } 
    ...
    { sprintf(txt, "fit %5.4f p-p dB",2.0*dscale); fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",xoffset + 50.0,yoffset + 465.0, txt);}
    { sprintf(txt, "fit %5.4f p-p deg",2.0*pscale); fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",xoffset + 50.0,yoffset + 405.0, txt);}
    { sprintf(txt, "magnitude(thin line)"); fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",xoffset + 280.0,yoffset + 25.0, txt);}
    { sprintf(txt, "phase(thick line)"); fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",xoffset + 280.0,yoffset + 10.0, txt);}
...
//    printf("dmax %f dmin %f\n",dmax,dmin);
    ...
    for (k = 0; k < np; k++) {
        ...
        y = totpp * 480.0;
        if(y > 480.0) y = 480.0;
        if(k==0) {xp=x;yp=y; ypos[iter]=y+10;}
              fprintf(file, "newpath\n %6.2f %6.2f moveto\n %6.2f %6.2f lineto\n %5.2f %5.2f %5.2f sethsbcolor stroke\n",
            xp + xoffset,yp + yoffset,x + xoffset, y + yoffset,h,s,b);
            xp = x;
            yp = y;
    ...
    sprintf(txt, "magnitude of correlated wave"); fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",xoffset + 190.0,yoffset + ypos[1], txt);
    sprintf(txt, "amplitude of uncorrelated wave"); fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",xoffset + 190.0,yoffset + ypos[0], txt);
    sprintf(txt, "phase of correlated wave (thick line)"); fprintf(file, "%6.2f %6.2f moveto\n (%s) show\n",xoffset + 190.0,yoffset + ypos[2], txt);
